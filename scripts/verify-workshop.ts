...
const sessions = db
   .prepare(
      `SELECT COUNT(*) AS n FROM workshop_sessions WHERE status = 'active'`,
   )
   .get() as { n: number };
db.close();
...
const NEEDED_WELCOME = [
   ...
   [
      "ManageMessages (quitar reacciones ajenas)",
...
const NEEDED_CATEGORY = [
   ["ViewChannel", PermissionFlagsBits.ViewChannel],
   [
      "ManageChannels (crear/eliminar canales)",
   ...
   try {
      ...
      if (welcome && welcome.type === ChannelType.GuildText) {
         ...
         for (const [label, bit] of NEEDED_WELCOME) {
            console.log(
               `${welcome.permissionsFor(me).has(bit) ? "✅" : "❌"} ${label}`,
            );
         }
         const msgId = String(settings?.welcome_message_id ?? "");
         const msg = msgId
            ? await welcome.messages.fetch(msgId).catch(() => null)
            : null;
         console.log(
            msg
         ...
         console.log(
            `❌ canal de bienvenida inaccesible: ${welcomeId || "(no configurado)"}`,
         );
      }
      ...
      if (category && category.type === ChannelType.GuildCategory) {
         ...
         for (const [label, bit] of NEEDED_CATEGORY) {
            console.log(
               `${category.permissionsFor(me).has(bit) ? "✅" : "❌"} ${label}`,
            );
         }
      } else {
         console.log(
            `❌ categoría inaccesible: ${categoryId || "(no configurada)"}`,
         );
      }
   } finally {
