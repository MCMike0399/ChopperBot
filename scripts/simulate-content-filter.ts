...
const server = createServer((req, res) => {
  upstreamHits += 1;
  console.log(`  ↳ upstream hit #${upstreamHits}: ${req.method} ${req.url} → 400 high risk`);
...
async function main(): Promise<void> {
  ...
  const { guildProfileFor, REVZ_GUILD_ID } = await import(
    '../src/capabilities/general_chat/profile.js'
  );
  const { renderAssistantPrompt } = await import(
    '../src/capabilities/general_chat/preamble.js'
  );

  const profile = guildProfileFor(REVZ_GUILD_ID);
  if (!profile) throw new Error('RevZ profile missing');
  const system = renderAssistantPrompt(profile, new Date(), [], 'club-de-cine');

  console.log(`fake text backend on ${process.env.KIMI_BASE_URL} (always 400 high risk)\n`);
  const reply = await ask({
    ...
    tools: { tools: [], handle: async () => ({ status: 'error', payload: null }) },
  });
