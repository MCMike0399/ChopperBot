...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log('⚠️  event_intake never migrated — falling back to the built-in defaults.');
  }
  const tokens = effectiveModTokens(configured);
  console.log(`Approver tokens: ${configured.length > 0 ? 'configured' : 'DEFAULTS (nothing configured)'}`);
  ...
  for (const guild of client.guilds.cache.values()) {
    ...
    const hasConfigChannel = guild.channels.cache.has(CONFIGURATION_CHANNEL_ID);
    console.log(`\n${guild.name} (${guild.id})${hasConfigChannel ? '  ← holds the config channel' : ''}`);
    console.log(`  owner: ${guild.ownerId} (always passes — owners hold Administrator)`);
    console.log(
      matched.length > 0
