...
async function fetchText(client: Client, id: string): Promise<TextChannel> {
   ...
   if (!ch?.isTextBased() || ch.isDMBased())
      fail(`channel ${id} not text-based`);
...
function summarizeAttachment(m: Message): string {
   ...
   if (images.length === 0) {
      const names = [...m.attachments.values()]
         .map((a) => `${a.name}:${a.size}b`)
         .join(", ");
   ...
   return images
      .map(
         (img) =>
            `${img.name ?? "?"} ${img.contentType ?? ""} ${img.url.slice(0, 80)}…`,
      )
      .join(" | ");
}

async function dumpChannel(
   label: string,
   ch: TextChannel,
   limit: number,
): Promise<Message[]> {
   ...
   for (const m of msgs) {
      ...
      console.log(
         `  ${m.id} <@${m.author.id}> ${m.author.bot ? "bot" : "user"} att=${att} | ${text}`,
      );
   }
   return msgs;
}
...
async function main(): Promise<void> {
   if (!TICKET_CHANNEL) {
      fail(
         "usage: npx tsx scripts/deliver-event-intake-flyer.ts <ticketChannelId> <imagePath> [--inspect-only]",
      );
   }
   ...
   const created = ticket.created_event_id
      ? calendarStore.get(ticket.created_event_id)
      : null;
   console.log(
      "=== DB calendar (created + siblings sharing discord_event_id) ===",
   );
   const siblings = created?.discord_event_id
      ? calendarStore
           .listAll()
           .filter((e) => e.discord_event_id === created.discord_event_id)
      : created
        ? [created]
        : [];
   ...
   if (created?.discord_event_id && ticket.guild_id) {
      ...
      const ev = await guild.scheduledEvents
         .fetch(created.discord_event_id)
         .catch(() => null);
      ...
      console.log({
         ...
         url: ev
            ? `https://discord.com/events/${ticket.guild_id}/${ev.id}`
            : null,
      });
   }
   ...
   if (st.size < 10_000)
      fail(
         `image looks too small (${st.size} bytes) — refusing to post a stub`,
      );
   ...
   try {
      ...
      const toSweep = [
         ...ticketMsgs,
         ...(await agitprop.messages.fetch({ limit: 20 })).values(),
      ];
      for (const m of toSweep) {
         const isVerify =
            m.content.includes(TAG) ||
            [...m.attachments.values()].some((a) =>
               (a.name ?? "").includes("verify-flyer"),
            );
      ...
      if (ticket.flyer_request_message_id) {
         card = await agitprop.messages
            .fetch(ticket.flyer_request_message_id)
            .catch(() => null);
      }
      ...
      ok(
         `uploaded flyer in Agitprop (${imgs[0]!.name}, ${imgs[0]!.contentType})`,
      );

      if (
         ticket.flyer_status !== "requested" &&
         ticket.flyer_request_message_id
      ) {
         store.markFlyerRequested(
            TICKET_CHANNEL,
            ticket.flyer_request_message_id,
            ticket.flyer_notes,
         );
      ...
      const fulfilled = await flyerService.fulfillFlyer(
         TICKET_CHANNEL,
         imageMsg,
         "agitprop",
      );
      ...
      if (after.flyer_status !== "delivered")
         fail(`expected delivered, got ${after.flyer_status}`);
      if (after.flyer_image_message_id !== imageMsg.id) {
         fail(
            `pointer mismatch: db=${after.flyer_image_message_id} posted=${imageMsg.id}`,
         );
      }
      ok(
         `DB pointer → ${after.flyer_image_channel_id}/${after.flyer_image_message_id}`,
      );

      const recentTicket = await ticketCh.messages.fetch({ limit: 8 });
      const mirror = [...recentTicket.values()].find(
         (m) =>
            m.content.includes("Flyer del evento") &&
            listImageAttachments(m).length > 0,
      );
      if (!mirror) fail("no mirrored flyer with an image in the ticket");
      const mirrored = listImageAttachments(mirror)[0]!;
      ok(
         `ticket mirror ${mirror.id} (${mirrored.name}, ${mirrored.contentType})`,
      );
