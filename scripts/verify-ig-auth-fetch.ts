...
try {
   ...
   for (const p of posts.slice(0, 5)) {
      console.log(
         `  igPostId=${p.igPostId} takenAtMs=${p.takenAtMs} caption=${(p.caption ?? "").slice(0, 50).replace(/\n/g, " ")}`,
      );
   }
} catch (err) {
   ...
   if (/HTTP 429/.test(msg)) {
      console.error(
         `THROTTLED (429) — auth path reached IG but was rate-limited, NOT an auth failure: ${msg}`,
      );
      process.exit(3);
   }
