...
async function main(): Promise<void> {
   if (!toolName || !jsonArg) {
      console.error(
         "Uso: npx tsx scripts/calendar-tool.ts <toolName> '<json payload>' [--sync-discord]",
      );
      process.exit(1);
   }
   ...
   const onlyId = Object.keys(payload).every(
      (k) => k === "id" || k === "event_id",
   );
   if (!onlyId) {
   ...
   if (SYNC) {
      const synced = await source.handle("calendar_sync_discord_event", {
         event_id: id,
      });
