...
function commitRelease(section: ReleaseSection, push: boolean): void {
   ...
   if (!dirty) {
      ...
      const msg =
         `Release v${section.version}` + (headline ? `\n\n${headline}` : "");
...
async function main(): Promise<void> {
   ...
   const channelId =
      process.env.RELEASE_NOTES_CHANNEL_ID ?? DEFAULT_RELEASE_CHANNEL_ID;
   ...
   try {
      ...
      if (
         ...
         throw new Error(
            `channel ${channelId} is not a postable guild text/announcement channel`,
         );
      }
      const sent = await channel.send(post);
      console.log(
         `✅ Published v${section.version} → #${channel.name} (message ${sent.id})`,
      );
   } finally {
