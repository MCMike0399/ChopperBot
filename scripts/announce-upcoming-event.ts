...
async function main(): Promise<void> {
  if (!Number.isInteger(eventId)) {
    console.error('Uso: npx tsx scripts/announce-upcoming-event.ts <eventId> [--date YYYY-MM-DD] [--post]');
    process.exit(1);
  }
  ...
  if (dateArg) {
    picked = occurrences.find((o) => {
      ...
      return `${p.year}-${String(p.month).padStart(2, '0')}-${String(p.day).padStart(2, '0')}` === dateArg;
    });
  }
  if (!picked) {
    console.error(
  ...
  console.log(`modo              : ${POST ? 'PUBLICAR' : 'PREVIEW (no publica)'}`);
  ...
  if (!discordEvent) {
    console.log('aviso            : sin evento de Discord enlazado — el aviso saldría SIN enlace');
  }
  ...
  if (body.length < 20) {
    ...
    console.error(`El modelo devolvió un texto demasiado corto (${body.length} chars): "${body}"`);
    await client.destroy();
    memory.close();
    process.exit(1);
  }
  ...
  if (target.discordEvent?.imageUrl) console.log(`  │ [adjunta la portada del evento: ${target.discordEvent.imageUrl}]`);
  ...
  if (POST) {
    const sent = await (channel as {
      ...
      content: text,
      allowedMentions: { parse: mentions.everyone ? ['everyone'] : [], roles: mentions.roleIds },
      files: target.discordEvent?.imageUrl ? [target.discordEvent.imageUrl] : undefined,
    ...
    console.log('(sin fila en el ledger — el anuncio automático del día sigue programado)');
  } else {
