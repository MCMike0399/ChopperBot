...
async function main(): Promise<void> {
  ...
  const reply1 = await ask({
    system,
    messages: [
      { role: 'user', content: 'oye, el conversatorio de DataCenters se cae, ya no se va a hacer' },
      { role: 'assistant', content: '¿Te refieres a **Conversatorio: DataCenters**? Lo cancelo y lo saco del calendario.' },
  ...
  const reply2 = await ask({
    system,
    messages: [{ role: 'user', content: 'esta semana no hay cine club, cancela solo esa sesión porfa' }],
    tools,
    effort: 'high',
  });
  ...
  const reply3 = await ask({
    system,
    messages: [
      { role: 'user', content: 'esta semana no hay cine club, cancela solo esa sesión porfa' },
  ...
  console.log('serie sigue viva:', series !== null, '| one-off borrado:', store.get(event.id) === null);
  console.log(
    'ocurrencias canceladas:',
    JSON.stringify(store.listOverridesForMaster(weekly.id).map((o) => ({ at: o.occurrence_start_at, cancelled: o.cancelled }))),
  );
  mem.close();
}
