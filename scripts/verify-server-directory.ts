...
const users = userArgs.length > 0 ? userArgs : ['187289179871248384', '678730759309099019'];
...
client.once('clientReady', async () => {
  try {
    for (const userId of users) {
      const provider = createDiscordDirectoryProvider(() => client, REVZ_GUILD_ID, userId);
      const source = new ServerDirectoryToolSource(provider);
      const channels = await provider.listViewableChannels();
      const info = await source.handle('server_channel_info', { channel: query });
      console.log(`\nusuario ${userId}: ve ${channels.length} canales`);
      console.log(`  server_channel_info("${query}") → ${info.status}:`, JSON.stringify(info.payload));
    }
  } catch (err) {
