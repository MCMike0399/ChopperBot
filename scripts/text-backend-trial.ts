...
const RUNS =
   runsArg >= 0 ? Math.max(1, Number(process.argv[runsArg + 1]) || 2) : 2;
...
async function send(
   ...
   try {
      ...
      return {
         reply: `[ERR ${err.name}: ${String(err.message).slice(0, 90)}]`,
         tools,
         ms: Date.now() - t0,
      };
   }
...
async function battery(): Promise<Battery> {
   ...
   // create — recurring series, right weekday + 20:00 + location, exactly one row
   {
      ...
      const r = await send(
         cap,
         'crea el evento "Club de cine" todos los jueves a las 8 pm en la sala de cine, para ver Andor',
      );
      ...
      const w = store
         ...
         .filter(
            (e) =>
               e.recurrence_freq === "weekly" && /cine|andor/i.test(e.title),
         );
      if (
         w.length === 1 &&
         /Thursday/.test(localStr(w[0].start_at)) &&
         /20:00/.test(localStr(w[0].start_at)) &&
         (w[0].location ?? "").length > 0
      )
         b.create = 1;
      memory.close();
   }
   // override — edit ONE occurrence, series intact
   {
      ...
      const r = await send(
         cap,
         'En la serie "Club de cine", SOLO el jueves 25 de junio cambia el título a "Club de cine: tanta verdad". Las demás semanas quedan igual.',
      );
      replies.push(r.reply);
      b.ms += r.ms;
   ...
   // dedup — do NOT duplicate an event already on the calendar
   {
      ...
      const r = await send(
         cap,
         'crea el evento "Círculo de Estudio: Repensar la Pobreza", todos los domingos a las 8 pm en Salón de Círculo de Estudio',
      );
      replies.push(r.reply);
      b.ms += r.ms;
   ...
   // oneoff — "mañana" at the right date/time, no recurrence
   {
      ...
      const r = await send(
         cap,
         'crea un evento único para mañana: "Asamblea ordinaria", a las 8 pm, en sala de juntas',
      );
      replies.push(r.reply);
      b.ms += r.ms;
      const o = store
         .listAll()
         .filter((e) => !e.recurrence_freq && /asamblea/i.test(e.title));
      if (
         o.length === 1 &&
         /Tuesday/.test(localStr(o[0].start_at)) &&
         /20:00/.test(localStr(o[0].start_at))
      )
         b.oneoff = 1;
      memory.close();
   }
   // voice — the 94% case: plain Spanish answer, in character, no scaffolding
   {
      ...
      const noTools: ComposedTools = {
         tools: [],
         handle: async () => ({ status: "error", content: "no tools" }),
      };
      const t0 = Date.now();
      let reply = "";
      try {
         ...
         reply = await ask({
            ...
            messages: [
               {
                  role: "user",
                  content: "¿de qué va este servidor y qué puedo hacer aquí?",
               },
            ],
            tools: noTools,
            effort: "medium",
      ...
      // voice regression.
      const inCharacter =
         /(comunidad|servidor|server|puedes|aquí|revoluci)/i.test(reply);
      ...
      if (reply.length > 80 && !/^\[ERR/.test(reply) && inCharacter && clean)
         b.voice = 1;
   }
...
async function filterProbe(): Promise<{
   refused: number;
   classified: number;
   empty: number;
   lines: string[];
   ...
   for (const [label, prompt] of FILTER_PROMPTS) {
      try {
         ...
         if (text.trim().length === 0) {
            empty++;
            lines.push(
               `   ${RED}EMPTY   ${RST}  ${label.padEnd(20)} ${DIM}no content — reasoning-only or a silent refusal; inconclusive${RST}`,
            );
         } else {
            lines.push(
               `   ${GREEN}answered${RST}  ${label.padEnd(20)} ${DIM}${text.slice(0, 70).replace(/\s+/g, " ")}${RST}`,
            );
         }
      } catch (err) {
         refused++;
...
async function main(): Promise<void> {
   ...
   console.log(
      `backend : ${textBackend.provider} · ${textBackend.modelId} @ ${textBackend.baseUrl}`,
   );
   ...
   console.log(
      `scope   : ${filterOnly ? "risk filter only" : `${RUNS} run(s) of the full battery`}   now(local): ${localStr(NOW.getTime())}\n`,
   );
   ...
   if (!filterOnly) {
      ...
      console.log(
         `tool-calling : ${tool}/${toolMax}   ${DIM}(create ${agg.create} override ${agg.override} dedup ${agg.dedup} oneoff ${agg.oneoff})${RST}`,
      );
      console.log(
         `voice        : ${agg.voice}/${RUNS}${agg.voice === RUNS ? ` ${GREEN}✓${RST}` : ` ${RED}✗${RST}`}`,
      );
      console.log(
         `leaks        : ${agg.leaks}${agg.leaks === 0 ? ` ${GREEN}✓${RST}` : ` ${RED}✗ scaffolding reached the reply${RST}`}`,
      );
      ...
      console.log(
         `avg latency  : ${DIM}${(agg.ms / (RUNS * 5) / 1000).toFixed(1)}s per turn${RST}`,
      );
   }
   console.log(
      `refusals     : ${f.refused}/${FILTER_PROMPTS.length} refused · ${f.classified} classified as content_filter · ${f.empty} empty`,
   );
   ...
   if (uncaught) {
      ...
      console.log(
         `${DIM}  Effect: the turn is classified as a config error → admin channel gets paged and the${RST}`,
      );
      console.log(
         `${DIM}  member sees an English error string instead of the Nova fallback. Widen the regex in${RST}`,
      );
      console.log(
         `${DIM}  src/llm/health.ts (and add a case to src/llm/__tests__/) BEFORE switching .env.${RST}`,
      );
   }
   if (agg.leaks > 0)
      console.log(
