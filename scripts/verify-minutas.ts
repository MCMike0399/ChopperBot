...
import {
   MINUTAS_MIGRATIONS,
   MinutasStore,
} from "../src/capabilities/minutas/store.js";
...
import {
   ...
   pcmToWav,
} from "../src/capabilities/minutas/audio.js";
...
const CHAT_LINES = [
   {
      userId: "e2e-voz-3",
      author: "Carla (chat prueba)",
      content: "yo confirmo el sonido el viernes 👍",
   },
   {
      userId: "e2e-voz-3",
      author: "Carla (chat prueba)",
      content: "y yo apoyo lo de los volantes",
   },
];
...
async function main(): Promise<void> {
   console.log(
      "— minutas e2e — preparando fixtures de voz (gTTS + ffmpeg + opus)…",
   );
   ...
   for (const [i, speaker] of SPEAKERS.entries()) {
      ...
      fixtures.push({
         speaker,
         frames,
         seconds: (frames.length * 960) / 48000,
      });
   }
   check(
      "fixtures TTS generados",
      fixtures.every((f) => f.frames.length > 100),
   );
   ...
   const proofSegments = await transcriber.transcribe(
      proofWav,
      join(FIXTURE_DIR, "proof"),
   );
   const proofText = proofSegments
      .map((s) => s.text)
      .join(" ")
      .toLowerCase();
   check(
   ...
   const mem = new SqliteMemoryStore({
      path: resolve(config.CHOPPERBOT_DATA_DIR, "chopperbot.db"),
   });
   ...
   check(
      "canal de minutas configurado",
      store.getOutputChannelId() === MINUTAS_CHANNEL_ID,
      store.getOutputChannelId() ?? "unset",
   );
   ...
   if (!storage)
      throw new Error(
         "MinIO no configurado — la prueba exige el almacenamiento real",
      );
   ...
   check(
      "sesión abierta y conexión de voz Ready",
      sessions.getActive(REVZ_GUILD_ID)?.id === row.id,
      row.id,
   );
   ...
   console.log(
      `— inyectando voz 1 (${fixtures[0]!.seconds.toFixed(1)}s de audio)…`,
   );
   ...
   await new Promise((r) =>
      setTimeout(r, Math.ceil(fixtures[0]!.seconds * 1000) + 800),
   );
   sessions.recordChatLine(REVZ_GUILD_ID, CHAT_LINES[0]!);

   console.log(
      `— inyectando voz 2 (${fixtures[1]!.seconds.toFixed(1)}s de audio)…`,
   );
   ...
   await new Promise((r) =>
      setTimeout(r, Math.ceil(fixtures[1]!.seconds * 1000) + 800),
   );
   sessions.recordChatLine(REVZ_GUILD_ID, CHAT_LINES[1]!);

   console.log(
      "— cerrando la sesión y corriendo finalize REAL (whisper + DeepSeek + MinIO + Discord)…",
   );
   ...
   check(
      "DB: minio_prefix estructurado",
      /^minutas\/\d+\/\d{4}-\d{2}-\d{2}\/.+\/$/.test(done.minio_prefix ?? ""),
      done.minio_prefix ?? "unset",
   );
   check(
      "DB: summary_message_id registrado",
      !!done.summary_message_id,
      done.summary_message_id ?? "unset",
   );
   ...
   const listed = await s3.send(
      new ListObjectsV2Command({
         ...
         Prefix: done.minio_prefix!,
      }),
   );
   const keys = (listed.Contents ?? [])
      .map((o) => o.Key!.replace(done.minio_prefix!, ""))
      .sort();
   ...
   check(
      "MinIO contiene 2 WAV de audio",
      keys.filter((k) => k.startsWith("audio/") && k.endsWith(".wav"))
         .length === 2,
   );
   check(
      "MinIO contiene 2 JSON crudos de whisper",
      keys.filter((k) => k.startsWith("transcript/") && k.endsWith(".json"))
         .length === 2,
   );
   ...
   check(
      ...
      draftText.includes("Voz Uno (prueba)") &&
         draftText.includes("Voz Dos (prueba)"),
   );
   check(
      "borrador incluye los comentarios del chat",
      (draftText.match(/💬/g) ?? []).length === 2,
   );
   check(
      "borrador transcribe contenido esperado",
      draftText.toLowerCase().includes("foro") &&
         draftText.toLowerCase().includes("asamblea"),
   );
   ...
   check(
      "minuta.md tiene estructura de acta",
      minutaText.includes("## Resumen") && minutaText.includes("## Acuerdos"),
   );

   const posted = await await client.channels
      .fetch(MINUTAS_CHANNEL_ID)
      .then((c) =>
         (
            c as {
               isTextBased(): boolean;
               messages: {
                  fetch(id: string): Promise<{ content: string; url: string }>;
               };
            }
         ).messages.fetch(done.summary_message_id!),
      );
   check(
      "mensaje publicado en #minutas-de-asambleas con encabezado",
      posted.content.includes("# 📜 Minuta") &&
         posted.content.includes("PRUEBA e2e"),
   );
   console.log(`\n🔗 Minuta publicada: ${posted.url}`);
   console.log(
      `📦 Borradores en MinIO: ${config.MINIO_BUCKET}/${done.minio_prefix}`,
   );
   console.log(`⏱️  Resultado finalize: empty=${result.empty}`);

   const localDir = join(sessionsDir, row.id);
   check(
      "directorio local limpiado tras subida completa",
      !existsSync(localDir),
   );
