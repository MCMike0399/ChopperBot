...
import {
   monthKeyOfUtc,
   monthWindowUtc,
} from "../src/capabilities/calendar/grid.js";
...
const verdict = monthPublishAction(
   current,
   (k) => store.getPublished(k) !== null,
);
...
console.log(
   `→ DELETE      : [${tracked.filter((m) => !desired.includes(m)).join(", ") || "—"}]`,
);
...
for (const key of desired) {
   ...
   console.log(
      `  ${key}: ${store.listOccurrences(startMs, endMs - 1).length} occurrence(s) will render`,
   );
}
...
if (process.argv.includes("--render")) {
   ...
   for (const monthKey of desired) {
      ...
      if (!file) {
         console.log(`  ${monthKey}: no template, skipped`);
         continue;
      }
      const templateBytes = new Uint8Array(
         await readFile(resolve("calendar", file)),
      );
      const pdf = await renderMonthPdf({
         monthKey,
         events,
         overrides,
         templateBytes,
      ...
      console.log(
         `  ${monthKey}: ${outDir}/${monthKey}.png (${(png.length / 1024).toFixed(0)} KB)`,
      );
   }
}
db.close();
