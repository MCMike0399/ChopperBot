...
import {
   ...
   extractRequesterId,
   type MessageLike,
} from "../src/capabilities/event_intake/parse.js";
...
async function main(): Promise<void> {
   ...
   try {
      ...
      if (!channel || !channel.isTextBased())
         throw new Error(`channel ${CHANNEL_ID} not text-based`);
      ...
      if (!formMsg) {
         console.log(
            "❌ No Ticket Tool form message detected in this channel.",
         );
         return;
      }
      console.log(
         `✅ Detected form from ${formMsg.author?.tag} (${formMsg.author?.id})`,
      );
      ...
      console.log("\n--- parsed form ---");
      console.log(
         JSON.stringify({ ...parsed, pairs: parsed.pairs.length }, null, 2),
      );
