...
} from "../src/capabilities/file_scanner/store.js";
...
async function main(): Promise<void> {
   if (!config.VIRUSTOTAL_API_KEY) {
      console.error(
         "✗ VIRUSTOTAL_API_KEY not set in .env — cannot run the live smoke.",
      );
      process.exit(1);
   }

   const mem = new SqliteMemoryStore({ path: ":memory:" });
   await new NamespacedMemory(mem, "file_scanner").migrate(
      "file_scanner",
      FILE_SCANNER_MIGRATIONS,
   );
   ...
   const t1 = Date.now();
   const out1 = await scanner.scanBytes(rnd, {
      fileName: "chopperbot-smoke-random.bin",
      uploader: null,
   });
   console.log(
      `took ${((Date.now() - t1) / 1000).toFixed(1)}s · budget used=${limiter.usedInWindow()}`,
   );
   console.log(
      renderScanMessage([
         { fileName: "chopperbot-smoke-random.bin", status: out1 },
      ]),
   );
   ...
   console.log(
      `took ${((Date.now() - t2) / 1000).toFixed(1)}s · budget used=${limiter.usedInWindow()}`,
   );
