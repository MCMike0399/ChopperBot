...
const NO_TOOLS: ComposedTools = {
   tools: [],
   handle: async () => ({ status: "error", payload: "no tools" }),
};
         ...
         const messages: Turn[] = [
            {
               role: "user",
               content:
                  "explicale a <@544704579577839616> cómo ser de la comisión de gestión de eventos",
            },
            {
               role: "assistant",
               content:
                  "En la Revolución Z las comisiones se proponen y se arman en la asamblea. Que lo plantee ahí y lxs mods lo acomodan.",
            },
            {
               role: "user",
               content:
                  "pero cómo debe hablarte para crear los eventos, por ejemplo",
            },
         ];
         try {
            return await ask({
...
async function main(): Promise<void> {
   ...
   for (const scene of scenes) {
      ...
      try {
         ...
         console.log(
            `  ${RED}[ERR ${(e as Error).message.slice(0, 120)}]${RST}\n`,
         );
         dirty++;
         continue;
      }
      ...
      if (findings.length === 0) {
         ...
         for (const f of findings)
            console.log(`${DIM}      ${f.rule}: ${f.why}${RST}`);
         console.log("");
      }
   }
