...
const bill = (
   ...
   if (!u) return;
   const cached =
      u.input_tokens_details?.cached_tokens ?? u.prompt_cache_hit_tokens ?? 0;
   const input = u.input_tokens ?? u.prompt_tokens ?? 0;
   spentIn += input - cached;
   spentCached += cached;
   spentOut += u.output_tokens ?? u.completion_tokens ?? 0;
};
...
async function responsesBasic(): Promise<void> {
   ...
   try {
      const r = (await client.responses.create({
         model: MODEL,
         instructions:
            "Eres ChopperBot, el asistente de una comunidad de izquierda. Responde en español, breve.",
      ...
      console.log(
         ok(
            `${Date.now() - t0}ms · status=${r.status} · store=${String(r.store)}`,
         ),
      );
      console.log(
         `   in=${u?.input_tokens} (cached ${u?.input_tokens_details?.cached_tokens ?? 0}) out=${u?.output_tokens} (reasoning ${u?.output_tokens_details?.reasoning_tokens ?? 0})`,
      );
      console.log(
         `   ${D}${String(r.output_text).slice(0, 120).replace(/\s+/g, " ")}${X}`,
      );
   } catch (e) {
      console.log(
         bad(
            `${(e as { status?: number }).status} ${(e as Error).message.slice(0, 160)}`,
         ),
      );
   }
}
...
async function responsesEffort(): Promise<void> {
   await section(
      "2. Responses API — reasoning.effort tiers (ChopperBot uses medium/high)",
   );
   for (const effort of ["low", "medium", "high"] as const) {
      ...
      try {
         ...
         console.log(
            ok(
               `effort=${effort.padEnd(6)} ${String(Date.now() - t0).padStart(6)}ms · out=${u?.output_tokens} (reasoning ${u?.output_tokens_details?.reasoning_tokens ?? 0}) · ${D}${String(r.output_text).slice(0, 60).replace(/\s+/g, " ")}${X}`,
            ),
         );
      } catch (e) {
         console.log(
            bad(
               `effort=${effort} → ${(e as { status?: number }).status} ${(e as Error).message.slice(0, 120)}`,
            ),
         );
      }
   }
...
async function responsesTools(): Promise<void> {
   ...
   try {
      ...
      if (calls.length === 0) {
         ...
         console.log(
            `   ${D}output types: ${(r.output ?? []).map((o) => o.type).join(", ")}${X}`,
         );
         return;
      }
      for (const c of calls) {
         ...
         console.log(
            ok(
               `${Date.now() - t0}ms · ${c.name} · args ${parsed ? "parse OK" : `${R}UNPARSEABLE${X}`}`,
            ),
         );
         ...
         if (parsed) {
            ...
            const wd = Number.isNaN(d.getTime())
               ? "?"
            ...
            const good =
               /Thursday/.test(wd) &&
               /20:00/.test(wd) &&
               parsed.recurrence_freq === "weekly";
            console.log(
               `   ${good ? ok("semantics: Thursday 20:00 weekly") : bad(`semantics: got "${wd}" freq=${String(parsed.recurrence_freq)}`)}`,
            );
         }
      }
   } catch (e) {
...
async function responsesStream(): Promise<void> {
   ...
   try {
      ...
      } as never)) as never as AsyncIterable<{
         type: string;
         delta?: string;
      ...
      console.log(
         ok(
            `${deltas} text deltas · first at ${firstDelta}ms · total ${Date.now() - t0}ms`,
         ),
      );
      console.log(
         `   terminal event: ${[...seen].find((t) => /response\.(completed|incomplete|failed)/.test(t)) ?? `${R}none${X}`}`,
      );
...
async function chatCompletions(): Promise<void> {
   await section(
      "5. chat/completions — the surface src/llm/client.ts already speaks",
   );
   const t0 = Date.now();
   try {
      ...
      console.log(
         ok(
            `${Date.now() - t0}ms · finish=${r.choices[0]?.finish_reason} · tool_calls=${calls.length}`,
         ),
      );
      ...
      console.log(
         `   prompt=${u?.prompt_tokens} (cache hit ${u?.prompt_cache_hit_tokens ?? "n/a"}) completion=${u?.completion_tokens}`,
      );
      for (const c of calls) {
         ...
         console.log(
            `   ${parsed ? ok(`${fn?.name} args parse OK`) : bad(`${fn?.name} UNPARSEABLE`)}  ${D}${JSON.stringify(parsed ?? fn?.arguments)}${X}`,
         );
      }
      // Does it also emit reasoning_content, like Kimi K2.7 Thinking does?
      const rc = (msg as { reasoning_content?: string } | undefined)
         ?.reasoning_content;
      console.log(
         `   reasoning_content: ${rc ? `${Y}present (${rc.length} chars) — the degenerate-output guard must keep handling this${X}` : "absent"}`,
      );
   } catch (e) {
      console.log(
         bad(
...
async function chatEffortAndLoop(): Promise<void> {
   await section(
      "5b. chat/completions — reasoning_effort + tool round-trip (the real loop shape)",
   );
   ...
   for (const effort of ["low", "high"] as const) {
      ...
      try {
            ...
            messages: [
               {
                  role: "user",
                  content:
                     "Un evento semanal empieza el jueves 25 de junio de 2026. ¿Qué fecha es la cuarta repetición? Solo la fecha.",
               },
            ],
         } as never);
         bill(r.usage as never);
         console.log(
            ok(
               `reasoning_effort=${effort.padEnd(4)} ${String(Date.now() - t0).padStart(6)}ms · ${D}${(r.choices[0]?.message?.content ?? "").slice(0, 50).replace(/\s+/g, " ")}${X}`,
            ),
         );
      } catch (e) {
         console.log(
            bad(
               `reasoning_effort=${effort} → ${(e as { status?: number }).status} ${(e as Error).message.slice(0, 110)}`,
            ),
         );
      }
   }
   ...
   try {
      ...
      if (!call) {
         console.log(bad("round-trip: no tool call to feed back"));
         return;
      }
      ...
      const closed =
         text.trim().length > 0 && second.choices[0]?.finish_reason === "stop";
      console.log(
         closed
            ? ok(
                 `round-trip closed · finish=${second.choices[0]?.finish_reason} · ${D}${text.slice(0, 70).replace(/\s+/g, " ")}${X}`,
              )
            : bad(
                 `round-trip did NOT close · finish=${second.choices[0]?.finish_reason} · content=${JSON.stringify(text.slice(0, 60))}`,
              ),
      );
   } catch (e) {
      console.log(
         bad(
            `round-trip: ${(e as { status?: number }).status} ${(e as Error).message.slice(0, 140)}`,
         ),
      );
   }
}
...
async function cacheProbe(): Promise<void> {
   await section(
      "6. Context cache — cache-hit input is $0.0028/M vs $0.14/M (50×)",
   );
   ...
   const prefix =
      ...
      Array.from(
         { length: 120 },
         (_, i) =>
            `${i + 1}. Regla operativa número ${i + 1}: responde en español, con criterio propio, sin tecnicismos, y respeta los estatutos de la comunidad.`,
      ).join("\n");
   for (const attempt of [1, 2]) {
      ...
      try {
         ...
         const pct = u?.prompt_tokens
            ? Math.round((hit / u.prompt_tokens) * 100)
            : 0;
         console.log(
            `   call ${attempt}: ${String(Date.now() - t0).padStart(5)}ms · prompt=${u?.prompt_tokens} · cache hit=${hit} (${pct}%) miss=${u?.prompt_cache_miss_tokens ?? "n/a"}`,
         );
      } catch (e) {
         console.log(
            bad(`call ${attempt}: ${(e as Error).message.slice(0, 120)}`),
         );
      }
   }
   console.log(
      `   ${D}a hit on call 2 means our long capability prompts bill at 1/50th on every repeat turn${X}`,
   );
}
...
async function filterProbe(): Promise<void> {
   await section(
      "7. Risk filter — does V4-Flash refuse RevZ-shaped political prompts?",
   );
   ...
   const prompts: Array<[string, string]> = [
      ...
      [
         "anti-imperialism",
         "explícame por qué el antiimperialismo es parte de los estatutos de una comunidad de izquierda",
      ],
   ...
   for (const [label, prompt] of prompts) {
      try {
         ...
         if (!text.trim()) {
            empty++;
            console.log(
               `   ${Y}EMPTY   ${X} ${label.padEnd(20)} ${D}no content — inconclusive${X}`,
            );
         } else
            console.log(
               `   ${G}answered${X} ${label.padEnd(20)} ${D}${text.slice(0, 62).replace(/\s+/g, " ")}${X}`,
            );
      } catch (e) {
         refused++;
         ...
         if (!caught) uncaught++;
         console.log(
            `   ${R}refused ${X} ${label.padEnd(20)} status=${(e as { status?: number }).status} kind=${caught ? `${G}${kind}${X}` : `${R}${kind} ← UNCAUGHT${X}`} ${D}${(e as Error).message.slice(0, 60)}${X}`,
         );
      }
   }
   console.log(
   ...
   if (uncaught > 0)
      console.log(
         `   ${R}BLOCKER: widen the regex in src/llm/health.ts before switching .env${X}`,
      );
}
...
async function preflight(): Promise<boolean> {
   try {
      ...
      if (status === 402) {
         ...
         console.log(
            `${D}Top up at https://platform.deepseek.com/top_up, then re-run this probe.${X}`,
         );
         console.log(
            `${D}Nothing below can be measured until then, so stopping here rather than printing misleading results.${X}`,
         );
         return false;
      }
      if (status === 401) {
...
function redPng(size = 64): Uint8Array {
   ...
   const parts = [
      sig,
      chunk("IHDR", ihdr),
      chunk("IDAT", new Uint8Array(deflateSync(rawPx))),
...
async function visionProbe(): Promise<void> {
   await section(
      "8. Vision — could DeepSeek replace Nova Lite on image turns?",
   );
   ...
   for (const model of [MODEL, "deepseek-v4-pro"]) {
      try {
            ...
            messages: [
               {
                  ...
                  content: [
                     {
                        type: "text",
                        text: "Responde SOLO con el color dominante de esta imagen, en una palabra.",
                     },
                     {
                        type: "image_url",
         ...
         console.log(
            sees
               ? ok(
                    `${model} — SEES the image · ${D}"${txt.slice(0, 60).replace(/\s+/g, " ")}"${X}`,
                 )
         ...
         console.log(
            bad(
               `${model} — ${(e as { status?: number }).status} ${(e as Error).message.slice(0, 120)}`,
            ),
         );
      }
   }
   console.log(
      `   ${D}no vision here ⇒ Nova Lite must stay for image turns and the IG classifier${X}`,
   );
}
...
async function main(): Promise<void> {
   ...
   const cost =
      (spentIn * RATE.miss + spentCached * RATE.hit + spentOut * RATE.out) /
      1e6;
   console.log(`\n${"─".repeat(72)}`);
   console.log(
      `tokens: ${spentIn} uncached-in · ${spentCached} cached-in · ${spentOut} out`,
   );
