...
} from "../src/capabilities/instagram_monitor/classifier.js";
...
async function makeFlyerPng(): Promise<{ bytes: Uint8Array; format: "png" }> {
   ...
   const lines: Array<{
      ...
      {
         t: "Ayudanos a eliminar este grupo",
         size: 30,
         f: reg,
         color: rgb(1, 1, 1),
      },
      {
         t: 'Grupo: "Mostrando mi mujer real"',
         size: 34,
         f: bold,
         color: rgb(1, 0.4, 0.45),
      },
      { t: "Plataforma: Facebook", size: 32, f: bold, color: rgb(0.5, 0.8, 1) },
      {
         t: "Es un grupo donde se comparten fotos",
         size: 24,
         f: reg,
         color: rgb(0.9, 0.9, 0.9),
      },
      {
         t: "intimas de mujeres SIN su consentimiento.",
         size: 24,
         f: reg,
         color: rgb(0.9, 0.9, 0.9),
      },
      {
         t: "Pasos para reportarlo:",
         size: 26,
         f: bold,
         color: rgb(1, 0.85, 0.2),
      },
      {
         t: "1. Entra al grupo en Facebook",
         size: 22,
         f: reg,
         color: rgb(0.85, 0.85, 0.85),
      },
      {
         t: '2. Toca los tres puntos y "Reportar grupo"',
         size: 22,
         f: reg,
         color: rgb(0.85, 0.85, 0.85),
      },
      {
         t: "3. Comparte esta publicacion",
         size: 22,
         f: reg,
         color: rgb(0.85, 0.85, 0.85),
      },
      {
         t: "#NiUnaMas  #BastaDeViolenciaDigital",
         size: 22,
         f: bold,
         color: rgb(1, 0.4, 0.45),
      },
   ...
   try {
      ...
      await execFileP(
         "pdftoppm",
         ["-png", "-r", "110", "-singlefile", p, join(dir, "f")],
         { maxBuffer: 64 << 20 },
      );
...
async function experimentVision() {
   ...
   console.log(
      `${D}(the moderator's exact failure: facts are in the image, not the caption)${X}\n`,
   );
   ...
   for (const m of VISION_CANDIDATES) {
      const reply = await askModel(
         m,
         "Eres un asistente que lee imágenes con precisión.",
         {
            role: "user",
            content: prompt,
            attachments: [img],
      ...
      console.log(
         `${verdict}  ${m}   grupo:${grupo ? "✓" : "✗"} plataforma:${plat ? "✓" : "✗"}`,
      );
...
async function experimentClassify() {
   ...
   const gold = samples.map((s) =>
      parseClassificationReply(s.classification_json),
   );
   console.log(
      `${samples.length} real posts (${gold.filter((g) => g?.relevant).length} relevant per stored labels)\n`,
   );
   ...
   for (const m of CLASSIFY_CANDIDATES) {
      ...
      for (let i = 0; i < samples.length; i++) {
         ...
         const raw = await askModel(m, SYSTEM_PROMPT, {
            role: "user",
            content: userText,
         });
...
async function experimentChat() {
   ...
   const turn: Turn = {
      role: "user",
      content:
         "oye chopper, ando bajoneado hoy :( cuéntame algo que me suba el ánimo",
   };
...
async function main() {
   console.log(
      "ChopperBot effort-tier bake-off · real Bedrock · real DB captions",
   );
...
main().catch((e) => {
   console.error("experiment crashed:", e);
   process.exit(1);
});
