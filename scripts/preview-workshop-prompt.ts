...
import {
   SessionWorkspace,
   workspaceDirFor,
} from "../src/capabilities/workshop/workspace.js";
...
if (!channelId) {
   console.error(
      "usage: npx tsx scripts/preview-workshop-prompt.ts <channelId> [--full]",
   );
   process.exit(1);
}
...
function main(): void {
   ...
   const prompt = renderWorkshopPrompt({
      ...
      deliveredPaths: new Set(
         store.fileManifest(channelId).map((f) => f.rel_path),
      ),
   });
   ...
   const block =
      prompt.split("# Workspace de la sesión")[1]?.split("\n# ")[0] ??
      "(sin bloque)";
