...
const positional = args.filter((a, i) => !a.startsWith('--') && i !== sayIdx + 1);
...
async function main(): Promise<void> {
  ...
  const channel = await client.channels.fetch(CALENDAR_CHANNEL_ID).catch(() => null);
  const guildId = channel && 'guildId' in channel ? (channel.guildId as string | null) : null;
  if (!guildId) {
    console.error('No pude resolver el servidor desde el canal del calendario.');
  ...
  console.log(`menciones permit. : ${settings.mentions.length > 0 ? settings.mentions.join(', ') : '(ninguna)'}`);
  console.log(`modo              : ${SEND ? 'PUBLICAR DE VERDAD' : 'solo redactar (no publica)'}`);

  const source = new CalendarToolSource(store, 'verify-script', Date.now(), undefined, {
    broadcaster: createBroadcaster({ client, guildId }),
    writeAnnouncement: (system) =>
      ask({ system, messages: [{ role: 'user', content: 'Escribe el anuncio.' }], tools: composeToolSources([]) }),
  ...
  const targets = p.channels as Array<{ name: string; mention: string; posts_as?: string; post_title?: string }>;
  console.log(
  ...
  if (p.problems) console.log(`sin resolver      : ${JSON.stringify(p.problems)}`);
  console.log(`enlace del evento : ${p.has_event_link ? 'sí' : 'NO (saldría sin botón de apuntarse)'}`);
  ...
  if (!SEND) {
    ...
    console.log(`Para publicarlo de verdad: repite con --send, o en Discord confirma el token ${p.token}.`);
    await client.destroy();
    memory.close();
    return;
  }

  const sent = await source.handle('calendar_send_announcement', { token: p.token });
  ...
  const again = await source.handle('calendar_send_announcement', { token: p.token });
  console.log(`reintento         : ${again.status} — ${JSON.stringify(again.payload)}`);
