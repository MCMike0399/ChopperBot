...
const db = new Database(
   process.argv[2] ?? resolve(ROOT, "data/chopperbot.db"),
   { readonly: true },
);
const events = db
   .prepare("SELECT * FROM calendar_events ORDER BY start_at")
   .all() as any[];
...
for (const key of availableMonthKeys()) {
   ...
   const has = events.some((e) => {
      if (e.recurrence_freq)
         return (
            e.start_at <= endMs &&
            (e.recurrence_until == null || e.recurrence_until >= startMs)
         );
