...
function extractWords(pdfPath: string): {
   pageW: number;
   pageH: number;
   words: Word[];
} {
...
function detectDayColumns(
   ...
   for (let y = yLo; y < yHi; y++)
      for (let x = 0; x < W; x++) colAvg[x] += px[y * W + x];
   let max = 0;
   ...
   for (let x = 0; x < W; x++) {
      if (colAvg[x] >= thresh) run.push(x);
         ...
         lines.push(run.reduce((a, b) => a + b, 0) / run.length / scale);
         run = [];
      }
   }
   if (run.length)
...
function calibrate(file: string): {
   ...
   if (dayWords.length < 28) {
      throw new Error(
         `${file}: only found ${dayWords.length} day-number words (expected ≥28)`,
      );
   }
   ...
   if (detected) {
      ...
      const colGroups = cluster(
         dayWords.map((w) => ({ key: w.xMax, w })),
         25,
      )
      ...
      if (colGroups.length !== 7) {
         throw new Error(
            `${file}: expected 7 columns, got ${colGroups.length}: ${colGroups}`,
         );
      }
      const colPitch = median(
         colGroups.slice(1).map((r, i) => r - colGroups[i]),
      );
   ...
   const rowGroups = cluster(
      dayWords.map((w) => ({ key: w.yMin, w })),
      25,
   )
      .map((g) => ({
   ...
   const rowPitch = median(
      rowGroups.slice(1).map((r, i) => r.numberTop - rowGroups[i].numberTop),
   );
...
const geometries = files.map((f) => {
   ...
   // eslint-disable-next-line no-console
   console.log(
      `${g.monthKey}  ${g.rows.length} rows  pitch=${g.rowPitch}  cols=${g.columns.length}`,
   );
   return g;
});
...
{
   const fromGrid = geometries.filter(
      (g) => g.columns[0].left > 119 && g.columns[0].left < 126,
   );
