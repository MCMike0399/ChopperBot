...
const GUILD_ID =
   process.argv.find((a) => /^\d{17,20}$/.test(a)) ?? "1435843683541979248";
const RUN_MODEL = !process.argv.includes("--no-model");

async function main(): Promise<void> {
   const db = new Database(
      resolve(ROOT, config.CHOPPERBOT_DATA_DIR, "chopperbot.db"),
      {
         readonly: true,
      },
   );
   ...
   try {
      ...
      const canMentionAny = me.permissions.has(
         PermissionFlagsBits.MentionEveryone,
      );
      console.log(
         `Guild: ${guild.name} — bot MentionEveryone: ${canMentionAny ? "yes" : "no"}`,
      );
         ...
         messages: [
            {
               role: "user",
               content:
                  "ah fah pued pedirle a lxs mods que me ayuden con el flyer no le sé a eso",
            },
         ],
         tools: {
            tools: [],
            handle: async () => ({ status: "error", payload: {} }),
         },
      });
      console.log(`\n${reply}\n`);
      const pinged = mentions.notifyIds.filter((id) =>
         reply.includes(`<@&${id}>`),
      );
      console.log(
         pinged.length > 0
