...
import type {
   CalendarPublisher,
   PublishSummary,
} from "../src/capabilities/calendar/publisher.js";
...
class FakePublisher implements CalendarPublisher {
   ...
   outputChannelId() {
      return "1518328211165941912";
   }
   monthsForEvent(e: RenderEvent) {
      return monthsWithOccurrences(e, availableMonthKeys());
   }
   async publishAll() {
      return this.run(availableMonthKeys());
   }
   async publish(monthKeys: string[]) {
      return this.run(monthKeys);
   }
      ...
      for (const key of [...new Set(monthKeys)].sort()) {
         ...
         const bytes = await renderMonthPdf({
            ...
            templateBytes: new Uint8Array(
               readFileSync(resolve(TPL, templateFileFor(key)!)),
            ),
         });
...
const src = new CalendarToolSource(
   store,
   "MOD_alice",
   NOW,
   new FakePublisher(store),
);
...
async function call(tool: string, input: unknown) {
   ...
   console.log(
      `• ${tool} → ${res.status}${pub ? `  published=${JSON.stringify({ posted: pub.posted, skipped: pub.skipped, ok: pub.ok })}` : ""}`,
   );
   return res;
}
...
await call("calendar_create_event", {
   title: "Asamblea constituyente",
   start_at_iso: "2026-06-21T02:00:00Z",
   end_at_iso: "2026-06-21T04:00:00Z",
   location: "Asamblea-Z",
});
// 2) Weekly Sunday circle.
const repensar = await call("calendar_create_event", {
   title: "Círculo: Repensar la Pobreza",
   start_at_iso: "2026-06-15T02:00:00Z",
   recurrence_freq: "weekly",
   location: "Sala de eventos",
});
// 3) Weekly Monday circle.
await call("calendar_create_event", {
   title: "Círculo: Las venas abiertas de América Latina",
   start_at_iso: "2026-06-16T02:00:00Z",
   recurrence_freq: "weekly",
   location: "Sala de eventos",
});
...
await call("calendar_update_event", {
   id: repensarId,
   start_at_iso: "2026-06-15T01:00:00Z",
});
// 5) A one-off to delete.
const doomed = await call("calendar_create_event", {
   title: "Evento de prueba",
   start_at_iso: "2026-06-25T18:00:00Z",
});
await call("calendar_delete_event", {
   id: (doomed.payload as { event: { id: number } }).event.id,
});
...
for (const r of rows) {
   console.log(
      `  #${r.id} "${r.title}" ${new Date(r.start_at).toISOString()} freq=${r.recurrence_freq ?? "-"} by=${r.created_by} @${r.location ?? "-"}`,
   );
}
...
const checks: [string, boolean][] = [
   ...
   [
      "VEVENT count == rows",
      ics.split("BEGIN:VEVENT").length - 1 === rows.length,
   ],
