...
import { WorkshopToolSource, type SessionActions } from '../src/capabilities/workshop/tools.js';
...
const question = process.argv[3] ?? '¿Qué dice este documento sobre su tema principal?';
if (!source) {
  console.error('usage: npx tsx scripts/e2e-workshop-doc.ts <file|minio:channel/relPath> "pregunta"');
  process.exit(1);
}
...
try {
  ...
  console.log(`doc: ${rel} (${(bytes.length / 1024 / 1024).toFixed(1)} MB) — pregunta: ${question}\n`);
  const t0 = Date.now();
  ...
  const noRawFullRead = !toolCalls.includes('workshop_read_file') || toolCalls.filter((t) => t === 'workshop_read_file').length <= 2;
