...
function newChain() {
   ...
   return async function say(
      ...
      if (tools.length)
         console.log(`${dim}     tools: ${tools.join(", ")}${rst}`);
      return { reply, tools };
   };
}
...
async function main() {
   ...
   console.log(
      `Model: ${config.BEDROCK_MODEL_ID}   now(local): ${localStr(NOW.getTime())}\n`,
   );

   // ── Scene 1: create a recurring series, with a missing-location clarification ──
   console.log(
      "── Scene 1: crear serie semanal (Club de cine), aclarando lugar ──",
   );
   {
      const say = newChain();
      await say(
         "crea el evento de club de cine todos los jueves a las 8:00 para ver andor de star wars",
      );
      ...
      if (
         byTitle(/cine|andor/i).filter((e) => e.recurrence_freq === "weekly")
            .length === 0
      ) {
      ...
      check(
         weekly.length >= 1,
         "serie semanal de cine creada",
         `${weekly.length} match(es): ${cine.map((e) => `#${e.id} "${e.title}"`).join("; ")}`,
      );
      if (weekly[0]) {
         const ls = localStr(weekly[0].start_at);
         check(
            /Thursday/.test(ls) && /20:00/.test(ls),
            "arranca jueves 20:00",
            ls,
         );
         ...
         if (/cine|sala/i.test(loc)) check(true, "lugar capturado", loc);
            ...
            warn(
               "lugar no capturado como se esperaba",
               `location=${JSON.stringify(loc)}`,
            );
         if (cine.length > 1)
            warn("se creó más de un evento de cine", `total=${cine.length}`);
      }
   }
   ...
   {
      const master = byTitle(/cine|andor/i).find(
         (e) => e.recurrence_freq === "weekly",
      );
      const before = master ? overrideCountFor(master.id) : 0;
      const say = newChain();
      await say(
         "el evento de andor ahora veremos un poquito de tanta verdad solo el 24 de junio",
      );
      await say("sí el 25 me equivoqué xd");
      if (!master) {
         check(false, "no hay serie de cine para editar");
      } else {
         const after = overrideCountFor(master.id);
         if (after > before)
            check(
               true,
               "override de una sola ocurrencia creado",
               `master #${master.id}, overrides=${after}`,
            );
         else
            warn(
               "no se registró un override de ocurrencia",
               `overrides=${after} (¿editó la serie completa?)`,
            );
         // The whole series must NOT have been retitled/retimed.
         const fresh = store.get(master.id)!;
         check(
            fresh.recurrence_freq === "weekly",
            "la serie sigue siendo semanal (no se rompió)",
            `freq=${fresh.recurrence_freq}`,
         );
      }
   }
   ...
   console.log("\n── Scene 3: republicar el calendario ──");
   {
      ...
      check(
         tools.includes("calendar_publish"),
         "llamó a calendar_publish",
         tools.join(", ") || "(ninguna)",
      );
   }
   ...
   );
   {
      const say = newChain();
      await say(
         "publica un nuevo evento unico mañana de asamblea ordinaria a las 8pm en sala de juntas",
      );
      const asamblea = byTitle(/asamblea/i);
      const oneOff = asamblea.filter((e) => !e.recurrence_freq);
      check(
         oneOff.length === 1,
         "evento único de asamblea creado",
         asamblea
            .map(
               (e) =>
                  `#${e.id} "${e.title}" freq=${e.recurrence_freq ?? "one-off"}`,
            )
            .join("; ") || "(ninguno)",
      );
      if (oneOff[0])
         check(
            /Tuesday/.test(localStr(oneOff[0].start_at)) &&
               /20:00/.test(localStr(oneOff[0].start_at)),
            "es martes 23 a las 20:00",
            localStr(oneOff[0].start_at),
         );
   }
   ...
   console.log("\n── Scene 5: borrar la asamblea de mañana ──");
   {
      ...
      check(
         byTitle(/asamblea/i).length === 0,
         "la asamblea fue eliminada",
         byTitle(/asamblea/i)
            .map((e) => `#${e.id}`)
            .join(", ") || "sin eventos de asamblea",
      );
   }
   ...
   );
   {
      ...
      check(
         after === before,
         "no creó un duplicado",
         `antes=${before} después=${after}`,
      );
      if (/exist|ya (está|hay)|duplicad/i.test(reply))
         check(true, "avisó que ya existe");
   ...
   console.log("\n── Scene 7: charla simple (sin tocar el calendario) ──");
   {
      ...
      const touched = [...a.tools, ...b.tools].filter((t) =>
         mutators.includes(t),
      );
      check(
         touched.length === 0,
         "no ejecutó tools que muten el calendario",
         touched.join(", ") || "ninguna",
      );
      check(
         a.reply.length > 0 && b.reply.length > 0,
         "respondió en ambos turnos",
      );
   }
   ...
   console.log("\n── Scene 8: serie acotada por número de sesiones ──");
   {
      const say = newChain();
      await say(
         "crea un taller de serigrafía cada jueves a las 7 pm, son 6 sesiones, empieza este jueves",
      );
      const taller = byTitle(/serigraf/i);
      check(
         taller.length === 1,
         "creó UNA sola fila (no una por sesión)",
         taller
            .map((e) => `#${e.id} ${e.recurrence_freq ?? "one-off"}`)
            .join("; ") || "(ninguna)",
      );
      ...
      if (!s)
         check(
            false,
            "es una serie semanal",
            taller.map((e) => e.recurrence_freq ?? "one-off").join(", "),
         );
      else {
         check(
            s.recurrence_until !== null,
            "la serie quedó ACOTADA (no indefinida)",
            s.recurrence_until
               ? localStr(s.recurrence_until)
               : "recurrence_until=null",
         );
         const n = countOccurrencesUntil(
            s.start_at,
            "weekly",
            s.recurrence_until,
         );
   ...
   );
   {
      const say = newChain();
      await say(
         'crea el círculo de lectura "Raíz que no desaparece" todos los martes de julio a las 8pm',
      );
      const circ = byTitle(/ra[íi]z que no desaparece/i);
      check(
         circ.length === 1,
         "creó UNA sola fila, no una por martes",
         circ
            .map(
               (e) =>
                  `#${e.id} ${e.recurrence_freq ?? "one-off"} ${localStr(e.start_at)}`,
            )
            .join("; ") || "(ninguna)",
      );
      ...
      if (!s)
         check(
            false,
            "es una serie semanal",
            circ.map((e) => e.recurrence_freq ?? "one-off").join(", "),
         );
      else {
         check(
            s.recurrence_until !== null,
            '"de julio" se interpretó como RANGO',
            s.recurrence_until
               ? `hasta ${localStr(s.recurrence_until)}`
               : "recurrence_until=null (quedó indefinida)",
         );
         const n = countOccurrencesUntil(
            s.start_at,
            "weekly",
         ...
         if (n === 4 || n === 5)
            check(true, "cubre los martes de julio", `n=${n}`);
         else warn("número de martes inesperado", `n=${n}`);
         if (!/Tuesday/.test(localStr(s.start_at)))
            warn("no arranca en martes", localStr(s.start_at));
      }
   }
   ...
   );
   {
      ...
      const first = await say(
         "crea la asamblea permanente todos los sábados a las 8pm",
      );
      const askedRange = /hasta cu[áa]ndo|cu[áa]ntas sesiones|indefinid/i.test(
         first.reply,
      );
      if (askedRange) check(true, "preguntó por el rango (una vez)");
      else
         warn(
            "no preguntó por el rango",
            "aceptable si la creó indefinida de una",
         );
      ...
      if (!s)
         check(
            false,
            "es una serie semanal",
            perm.map((e) => e.recurrence_freq ?? "one-off").join(", "),
         );
      else
         check(
            s.recurrence_until === null,
            "quedó INDEFINIDA (no la bloqueó ni la acotó sola)",
            `until=${s.recurrence_until}`,
         );
   }
   ...
   for (const e of store.listAll()) {
      console.log(
         `  #${e.id} ${dim}|${rst} "${e.title}" ${dim}|${rst} ${e.recurrence_freq ?? "one-off"} ${dim}|${rst} ${localStr(e.start_at)} ${dim}|${rst} ${e.location ?? "—"} ${dim}| overrides=${overrideCountFor(e.id)}${rst}`,
      );
   }

   console.log();
   if (failures === 0)
      console.log(`${g}✓ All calendar smoke checks passed.${rst}`);
   else
      console.log(
         `${r}✗ ${failures} hard check(s) failed${rst} (see ~ warnings for soft model-behavior notes).`,
      );
   memory.close();
