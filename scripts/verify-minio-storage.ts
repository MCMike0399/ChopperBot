...
async function main(): Promise<void> {
   ...
   if (!storage) {
      fail(
         "MINIO_ACCESS_KEY/MINIO_SECRET_KEY no están configurados — el storage está deshabilitado.",
      );
   }
   console.log(
      `backend=${storage!.backend} endpoint=${config.MINIO_ENDPOINT} bucket=${config.MINIO_BUCKET}`,
   );

   if (!(await storage!.ensureReady())) {
      fail(
         "El bucket no existe y no pudo crearse (¿MinIO caído? ¿credenciales sin permiso?).",
      );
   }
   ...
   if (
      !back ||
      Buffer.from(back).toString("utf-8") !==
