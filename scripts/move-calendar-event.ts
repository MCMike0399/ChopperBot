...
import { formatInTimezone, WALL_CLOCK_OFFSET_MS } from '../src/capabilities/calendar/time.js';

const [idArg, dateArg, timeArg] = process.argv.slice(2).filter((a) => !a.startsWith('--'));
...
async function main(): Promise<void> {
  ...
  if (!Number.isInteger(eventId) || !dateMatch || !timeMatch) {
    console.error('Uso: npx tsx scripts/move-calendar-event.ts <eventId> <YYYY-MM-DD> <HH:MM> [--no-publish]');
    process.exit(1);
  }
  ...
  const newStartMs =
    Date.UTC(Number(ys), Number(mos) - 1, Number(ds), Number(hs), Number(mis)) - WALL_CLOCK_OFFSET_MS;
  ...
  const updated = store.update(eventId, {
    ...
    ...(event.recurrence_until != null ? { recurrence_until: event.recurrence_until + delta } : {}),
  });
  console.log(`  guardado      : ${formatInTimezone(updated!.start_at)} (recurrencia: ${updated!.recurrence_freq ?? 'no'})`);
