...
function fmtTime(m: Message): string {
  return m.createdAt.toLocaleString('es-MX', { timeZone: 'America/Mexico_City', hour12: false });
}
...
async function sampleChannel(ch: GuildBasedChannel, limit: number, trunc: number): Promise<void> {
  ...
  try {
    ...
    out(`_(no se pudo leer: ${err instanceof Error ? err.message : String(err)})_`);
  }
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    out(`_Generado: ${new Date().toLocaleString('es-MX', { timeZone: 'America/Mexico_City' })} (hora CDMX)_`);
    out();

    out('## Guilds donde está el bot');
    for (const g of client.guilds.cache.values()) out(`- ${g.name} (\`${g.id}\`)`);
    out();
    ...
    for (const r of sortedRoles) {
      ...
      out(`- ${r.name} (\`${r.id}\`) — mencionable: ${r.mentionable ? 'sí' : 'no'}, miembros: ${r.members.size}`);
    }
    out();
    ...
    try {
      ...
      out(`_(no se pudo leer la DB: ${err instanceof Error ? err.message : String(err)})_`);
    }
    out();

    const channels = await guild.channels.fetch();
    const cats = [...channels.values()]
      .filter((c): c is GuildBasedChannel => c != null && c.type === ChannelType.GuildCategory)
      .sort((a, b) => a.position - b.position);
    const orphan = [...channels.values()].filter(
      (c): c is GuildBasedChannel => c != null && c.parentId == null && c.type !== ChannelType.GuildCategory,
    );
    ...
    if (orphan.length > 0) {
      out('### (sin categoría)');
      orphan.sort((a, b) => a.position - b.position).forEach(renderChannelLine);
    }
    for (const cat of cats) {
      out(`### 📁 ${cat.name} (\`${cat.id}\`)`);
      const children = [...channels.values()]
        .filter((c): c is GuildBasedChannel => c != null && c.parentId === cat.id)
        .sort((a, b) => a.position - b.position);
    ...
    for (const cat of cats) {
      ...
      for (const ch of children) {
        if (ch.type === ChannelType.GuildForum || ch.type === ChannelType.GuildMedia) {
          ...
          try {
            const threads = await (ch as typeof ch & { threads: { fetchActive: () => Promise<{ threads: Map<string, { name: string; id: string }> }> } }).threads.fetchActive();
            for (const t of threads.threads.values()) out(`  - "${t.name}" (\`${t.id}\`)`);
            if (threads.threads.size === 0) out('  _(sin hilos activos)_');
          } catch (err) {
            out(`  _(no se pudieron listar hilos: ${err instanceof Error ? err.message : String(err)})_`);
          }
          out();
          continue;
        }
        if (!ch.isTextBased() || ch.type === ChannelType.GuildVoice || ch.type === ChannelType.GuildStageVoice) continue;
        const isGeneral = ch.id === GENERAL_CHANNEL_ID;
        out(`### #${ch.name} (\`${ch.id}\`)${isGeneral ? ' — GENERAL, últimas ~1–2 h' : ''}`);
        await sampleChannel(ch, isGeneral ? MSG_LIMIT_GENERAL : MSG_LIMIT_PER_CHANNEL, isGeneral ? MSG_TRUNC_GENERAL : MSG_TRUNC);
        out();
      }
    }
