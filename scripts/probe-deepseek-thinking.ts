...
async function once(extra: Record<string, unknown>) {
   ...
   try {
         ...
         messages: [
            {
               role: "system",
               content:
                  "Eres ChopperBot. Usa las herramientas cuando corresponda.",
            },
      ...
      console.log(
         `   ERROR ${e.status ?? ""} ${String(e.message).slice(0, 110)}`,
      );
      return null;
   }
}
...
async function main() {
   ...
   await variant("thinking enabled/high", {
      thinking: { type: "enabled", reasoning_effort: "high" },
   });
   await variant("thinking enabled/low", {
      thinking: { type: "enabled", reasoning_effort: "low" },
   });
   await variant("thinking enabled/max", {
   ...
   await variant("top-level reasoning_effort=high", {
      reasoning_effort: "high",
   });
   await variant("bogus knob (control)", {
