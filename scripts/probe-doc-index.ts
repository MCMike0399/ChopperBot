...
if (!source) {
   console.error(
      "usage: npx tsx scripts/probe-doc-index.ts <file|minio:channel/relPath> [query]",
   );
   process.exit(1);
}
...
try {
   ...
   if (source.startsWith("minio:")) {
      ...
      if (!storage)
         throw new Error("MinIO no configurado (MINIO_ACCESS_KEY/SECRET).");
   ...
   console.log(
      `source: ${rel} (${(bytes.length / 1024 / 1024).toFixed(1)} MB)`,
   );
   ...
   console.log(
      `indexed in ${((Date.now() - t0) / 1000).toFixed(1)}s → pages=${built.meta.pages} chars=${built.meta.chars} chunks=${built.meta.chunks}`,
   );
   ...
   for (const h of hits) {
      console.log(
         `  [pp. ${h.chunk.page_start}–${h.chunk.page_end}] score=${h.score.toFixed(2)} ${h.chunk.heading ?? ""}`,
      );
      console.log(
         `    ${excerptAround(h.chunk.text, query, 220).replace(/\n/g, " ")}`,
      );
   }

   const mid = Math.floor(built.meta.pages / 2);
   const range = chunks.filter(
      (c) => c.page_end >= mid && c.page_start <= mid + 3,
   );
   console.log(
