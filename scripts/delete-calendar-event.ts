...
async function main(): Promise<void> {
   ...
   console.log(
      `Eliminando #${event.id} ${event.title} (${formatInTimezone(event.start_at)}, recurrencia: ${event.recurrence_freq ?? "no"})`,
   );
   console.log(
      `  evento de Discord enlazado: ${event.discord_event_id ?? "(ninguno)"}`,
   );
   ...
   const outChannel = outputChannelId
      ? await client.channels.fetch(outputChannelId).catch(() => null)
      : null;
   const guildId =
      outChannel && "guildId" in outChannel
         ? (outChannel.guildId as string)
         : null;
   if (!guildId) {
      console.error(
         "No pude resolver el servidor (canal de salida inaccesible).",
      );
   ...
   if (msgArg) {
      ...
      try {
         ...
         if (ch && ch.isTextBased()) {
            await (
               ch as { messages: { delete(id: string): Promise<unknown> } }
            ).messages.delete(messageId!);
            console.log(`  mensaje ${messageId}: eliminado ✅`);
         }
      } catch (err) {
         console.log(
            `  ⚠️ no pude eliminar el mensaje ${messageId}: ${err instanceof Error ? err.message : err}`,
         );
      }
   }
