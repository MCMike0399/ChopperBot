...
const check = (ok: boolean, label: string, detail = '') => {
  ...
  else { failures++; console.log(`  ${r}✗ ${label}${rst}${detail ? `  ${detail}` : ''}`); }
};
...
const fakeGuild = { id: 'G1', name: 'Revolución Z', memberCount: 300, channels: { cache: new Map() } } as unknown as Guild;
const client = {
...
configCap.recordSkippedCapabilities([{ id: 'file_scanner', error: 'VIRUSTOTAL_API_KEY not set' }]);
router = buildRouter(new Map([['CH_CAL', 'calendar'], ['CH_IG', 'instagram_monitor']]));
...
async function say(user: string, history: Turn[] = []): Promise<{ reply: string; tools: string[]; inputs: unknown[] }> {
  ...
  const spied: ComposedTools = {
    ...
    handle: (n, i) => { tools.push(n); inputs.push(i); return bundle.tools.handle(n, i); },
  };
...
const actionsOf = (inputs: unknown[]) =>
  inputs.map((i) => (i as { action?: string } | null)?.action).filter(Boolean) as string[];
...
{
  const { reply, tools, inputs } = await say('oye, ¿cómo va el bot? dame el estado general');
  check(tools.includes('config_system'), 'llamó a config_system', tools.join(', ') || '(ninguna)');
  check(actionsOf(inputs).includes('health'), 'usó action:health', actionsOf(inputs).join(', ') || '(sin action)');
  const perCap = tools.filter((t) => ['config_instagram', 'config_filescanner', 'config_eventintake'].includes(t));
  check(perCap.length === 0, 'no encadenó los status por capability', perCap.join(', ') || 'ninguno');
  // The whole point is a human summary, not a JSON dump.
  check(!reply.includes('"uptime_ms"') && !reply.includes('{"'), 'resumió en vez de volcar JSON');
  if (/file_scanner/i.test(reply)) check(true, 'reportó el capability que no arrancó');
  else warn('no mencionó file_scanner', 'estaba en skipped — se esperaba que lo reportara');
}

// ── Scene 2: a real problem must be surfaced, with the fix ───────────────────
console.log('\n── Scene 2: kill-switch de IG activo → lo reporta y dice cómo arreglarlo ──');
{
  new InstagramMonitorStore(memory.db()).tripGlobalStop('require_login', NOW.getTime());
  const { reply, inputs } = await say('¿todo bien con el monitor de instagram?');
  check(/detenid|parad|stop|kill/i.test(reply), 'dijo que el monitor está detenido');
  check(/require_login|sesi[óo]n|cookies/i.test(reply), 'nombró la causa');
  if (/resume_monitor/i.test(reply)) check(true, 'indicó el comando para reanudar');
  else warn('no citó resume_monitor', 'aceptable si explicó el procedimiento');
  console.log(`${dim}     actions: ${actionsOf(inputs).join(', ') || '(ninguna)'}${rst}`);
}
...
console.log('\n── Scene 3: "¿con qué modelo piensa?" → Kimi (texto) + Nova (imágenes) ──');
{
  ...
  check(!/sonnet|claude-3|anthropic\.claude/i.test(reply), 'NO citó el modelo legacy (Sonnet/Claude)');
}

console.log();
if (failures === 0) console.log(`${g}✓ All admin health smoke checks passed.${rst}`);
