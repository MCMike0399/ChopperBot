...
async function probe(label: string, system: string | null, user: string): Promise<void> {
  try {
...
async function main(): Promise<void> {
  console.log(`text backend: ${config.KIMI_MODEL_ID} @ ${config.KIMI_BASE_URL}\n`);
  ...
  if (withBedrock) {
    ...
    try {
      const text = await ask({
        ...
        tools: { tools: [], handle: async () => ({ ok: false, error: 'no tools' }) },
        effort: 'low',
      });
