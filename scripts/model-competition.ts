...
async function send(
   ...
   try {
      ...
      return {
         reply: `[ERR ${(e as Error).name}: ${String((e as Error).message).slice(0, 80)}]`,
         tools,
      };
   }
}
...
async function battery(): Promise<Check> {
   ...
   // vision (gating)
   {
      ...
      const { reply } = await send(
         cap,
         "Responde SOLO con el color dominante de esta imagen, en una palabra.",
         new ImageAttachable("r.png", "image/png", RED, "png"),
      );
      replies.push(reply);
...
async function main() {
   ...
   console.log(
      `now(local): ${localStr(NOW.getTime())} · ${RUNS} runs/model · scores summed over runs\n`,
   );
   const dim = "\x1b[2m",
      rst = "\x1b[0m";
   ...
   eligible.sort(
      (a, b) => b.total - a.total || a.leaks - b.leaks || b.vision - a.vision,
   );
   console.log();
   if (eligible.length === 0) {
      console.log(
         "\x1b[31mNo open-weight contender passed the vision gate.\x1b[0m",
      );
   } else {
      const w = eligible[0];
      const ref = results.find((r) => r.ref);
      console.log(
         `🏆 Winner (open-weight, image-capable): \x1b[32m${w.id}\x1b[0m`,
      );
      console.log(
         `   calendar ${w.total}/${4 * RUNS}, vision ${w.vision}/${RUNS}, leaks ${w.leaks}`,
      );
      if (ref)
         console.log(
            `   ${dim}reference Haiku: calendar ${ref.total}/${4 * RUNS}, vision ${ref.vision}/${RUNS}, leaks ${ref.leaks}${rst}`,
         );
   }
   process.exit(0);
}
...
main().catch((e) => {
   console.error("competition crashed:", e);
   process.exit(1);
});
