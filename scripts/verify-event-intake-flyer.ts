...
if (!TICKET_CHANNEL) {
   fail(
      "usage: npx tsx scripts/verify-event-intake-flyer.ts <ticketChannelId> [--dry-run]",
   );
}
...
async function main(): Promise<void> {
   ...
   if (parsed.flyerSelf !== false) {
      fail(
         `ticket ${TICKET_CHANNEL} has flyerSelf=${String(parsed.flyerSelf)} — pick a flyerSelf=false ticket`,
      );
   }
   ...
   if (ticket.flyer_status === "requested") {
      fail(
         `ticket already has an open flyer job — cancel it first or pick another channel`,
      );
   }
   ...
   try {
      ...
      if (afterOpen.flyer_status !== "requested")
         fail(`expected flyer_status=requested, got ${afterOpen.flyer_status}`);
      ok(
         `DB flyer_status=requested (card id ${afterOpen.flyer_request_message_id})`,
      );

      const agitprop = await fetchText(client, agitpropId);
      const cardMsg = await agitprop.messages.fetch(
         afterOpen.flyer_request_message_id!,
      );
      if (!cardMsg.content.includes("Solicitud de flyer"))
         fail("Agitprop card missing expected text");
      ...
      const notice = [...recentTicket.values()].find((m) =>
         m.content.includes("Comisión de Agitprop"),
      );
      ...
      if (afterFulfill.flyer_status !== "delivered") {
         fail(
            `expected flyer_status=delivered, got ${afterFulfill.flyer_status}`,
         );
      }
      if (!afterFulfill.flyer_image_message_id)
         fail("flyer_image_message_id not set");
      ok(
         `DB flyer_status=delivered (image msg ${afterFulfill.flyer_image_message_id})`,
      );

      const cardAfter = await agitprop.messages.fetch(
         afterOpen.flyer_request_message_id!,
      );
      if (!cardAfter.content.includes("Flyer entregado"))
         fail("Agitprop card not marked delivered");
      ok("Agitprop card edited to delivered");

      const recentAfter = await ticketCh.messages.fetch({ limit: 10 });
      const mirror = [...recentAfter.values()].find((m) =>
      ...
      if (finalRow.flyer_status === "delivered") {
         ok(
            "Flyer delivered — job correctly stays delivered (cancel only applies while requested)",
         );
      } else {
