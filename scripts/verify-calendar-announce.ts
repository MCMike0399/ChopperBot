...
async function main(): Promise<void> {
   ...
   console.log(
      `canal de anuncios     : ${announceChannelId ?? "(sin configurar)"}`,
   );
   console.log(
      `hora de anuncio        : ${settings.hour}:00 (America/Mexico_City)`,
   );
   console.log(`menciones              : ${JSON.stringify(settings.mentions)}`);
   console.log(
      `modo                   : ${POST ? (REPOST ? "PUBLICAR (re-anunciando)" : "PUBLICAR") : "PREVIEW (no publica)"}`,
   );
   ...
   const guildId =
      channel && "guildId" in channel
         ...
         : null;
   console.log(
      `canal                  : ${channel && "name" in channel ? `#${channel.name}` : "(no accesible)"}`,
   );
   if (!guildId) {
      console.log(
         "El canal de anuncios no es de un servidor accesible — abortando.",
      );
   ...
   console.log(
      `puede CREAR eventos    : ${diag.canManageEvents ? "✅ sí" : '❌ no (falta "Gestionar eventos")'}`,
   );
   if (diag.hiddenEventChannels.length > 0) {
      ...
      console.log(
         "   (los eventos que viven en esos canales NO aparecen en mi lista)",
      );
   }
   for (const p of diag.problems)
      console.log(`   ⚠️  ${p.replace(/\*\*/g, "")}`);
   ...
   for (const e of events ?? []) {
      console.log(
         `  ${e.id}  ${formatInTimezone(e.startAtMs)}  ${e.recurring ? "🔁" : "  "}  ${e.name}`,
      );
   }
   ...
   for (const a of report.announced) {
      ...
      console.log(
         `  evento Discord: ${a.discordEventId ?? "(ninguno)"} [${a.link}]`,
      );
      if (a.discordEventUrl)
         console.log(`  enlace        : ${a.discordEventUrl}`);
      console.log(
         `  publicado     : ${a.posted ? `✅ ${a.messageId}` : POST ? `❌ ${a.error ?? "no"}` : "— (preview)"}`,
      );
      console.log("  ──── mensaje ────");
      console.log(
         a.text
   ...
   if (report.nudged.length > 0) {
      ...
      for (const n of report.nudged)
         console.log(`  #${n.eventId} ${n.title} — ${n.startAtLocal}`);
      console.log(
         "  → en producción esto avisa a lxs mods en el canal de gestión del calendario.",
      );
   }
