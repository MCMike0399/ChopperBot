...
async function main(): Promise<void> {
   ...
   const pending = mem
      .db()
      .prepare(
         "SELECT * FROM workshop_files WHERE storage_key IS NULL ORDER BY channel_id, rel_path",
      )
   ...
   try {
      for (const rec of pending) {
         ...
         const localPath = join(
            workspaceDirFor(dataDir, rec.channel_id),
            rec.rel_path,
         );
         ...
         if (DRY_RUN) {
            console.log(
               `  • ${label} — ${bytes.length} B desde ${source} (se subiría)`,
            );
            skipped += 1;
            continue;
         }
