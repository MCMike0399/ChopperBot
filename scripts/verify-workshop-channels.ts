...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!category || category.type !== ChannelType.GuildCategory) {
      console.error(`❌ La categoría ${CATEGORY_ID} no existe o no es categoría.`);
      process.exit(1);
    }
    const children = category.guild.channels.cache
      .filter((c) => c.parentId === CATEGORY_ID)
      .sort((a, b) => a.createdTimestamp - b.createdTimestamp);
    console.log(`Categoría "${category.name}" — ${children.size} canal(es):\n`);
    ...
    for (const channel of children.values()) {
      ...
      if (!session) {
        ...
        problems.push(`sesión en estado '${session.status}' pero el canal existe`);
      }

      const manifest = session ? store.fileManifest(channel.id) : [];
      for (const rec of manifest) {
        const fv: FileVerdict = { rec, local: 'no', minio: 'no-key', carrier: 'unreachable', problems: [] };
        // Local cache
        const localPath = join(workspaceDirFor(dataDir, channel.id), rec.rel_path);
        if (existsSync(localPath)) {
          fv.local = 'yes';
          if (statSync(localPath).size !== rec.bytes) {
            fv.problems.push(`tamaño local ${statSync(localPath).size} ≠ manifiesto ${rec.bytes}`);
          }
        }
        // MinIO
        if (rec.storage_key) {
          ...
          if (bytes === null) {
            ...
            fv.problems.push(`MinIO tiene ${bytes.length} B ≠ manifiesto ${rec.bytes} B`);
          } else {
            fv.minio = 'ok';
          }
        } else {
        ...
        if (channel.type === ChannelType.GuildText) {
          ...
          if (!msg) {
            ...
            if (!att) fv.problems.push('el mensaje portador no tiene el adjunto');
          }
        }
      ...
      const ok =
        problems.length === 0 && fileVerdicts.every((f) => f.problems.length === 0);
      if (!ok) failures += 1;
      const icon = ok ? '✅' : problems.length || fileVerdicts.some((f) => f.minio === 'missing') ? '❌' : '⚠️';
      console.log(
        `${icon} #${channel.name} (${channel.id})${session ? ` — ${session.user_tag}, ${session.status}` : ''}`,
      );
      if (session && manifest.length === 0) console.log('    sin archivos en el manifiesto');
