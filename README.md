...
| `calendar` | A **per-user** calendar — every Discord user has their own events, visible from *any* channel bound to this capability (no channel scoping). Create, list, search, update, and delete events, with daily / weekly / monthly recurrence and an optional end date. |
