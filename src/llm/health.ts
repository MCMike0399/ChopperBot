...
export function classifyLlmError(err: unknown): LlmErrorKind {
   ...
   if (
      /Throttling|ServiceUnavailable|InternalServer|ModelTimeout|ModelNotReady/i.test(
         name,
      )
   ) {
      return "transient";
   ...
   if (
      /Validation|AccessDenied|ResourceNotFound|UnrecognizedClient|InvalidSignature/i.test(
         name,
      )
   ) {
...
export class LlmHealthMonitor {
   ...
   reportFailure(err: unknown, nowMs = Date.now()): void {
      ...
      const shouldAlert =
         kind === "deterministic" ||
         this.consecutiveFailures >= TRANSIENT_ALERT_THRESHOLD;
      if (!shouldAlert) return;
      if (
         this.lastAlertAtMs !== null &&
         nowMs - this.lastAlertAtMs < ALERT_COOLDOWN_MS
      )
         return;
   ...
   private post(lines: string[]): void {
      if (!this.sink) return;
      void this.sink(lines).catch((err) =>
         log.warn({ err }, "llm.health.sink_failed"),
      );
   }
}
