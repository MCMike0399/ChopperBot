...
function baseInput() {
   return {
      system: "system",
      messages: [
         { role: "user" as const, content: "explícame el libro por capítulos" },
      ],
      tools: TOOLS,
   };
}
...
describe("askKimi — forcing pass after the iteration cap", () => {
   test("a forcing pass that still returns tool_calls gets one nudged retry", async () => {
      ...
      createMock.mockImplementation(async () => {
         ...
         if (n <= config.MAX_TOOL_ITERATIONS + 1)
            return toolCallsResponse(`call_${n}`);
      ...
      const forcingCall2 =
         createMock.mock.calls[config.MAX_TOOL_ITERATIONS + 1][0];
   ...
   test("a forcing pass that answers text on the first try needs no retry", async () => {
      ...
      createMock.mockImplementation(async () => {
         n += 1;
         if (n <= config.MAX_TOOL_ITERATIONS)
            return toolCallsResponse(`call_${n}`);
