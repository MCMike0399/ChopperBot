...
describe("isDegenerateOutput", () => {
   ...
   test("flags a bare tool-call envelope", () => {
      expect(
         isDegenerateOutput(
            '{"name": "workshop_read_file", "arguments": {"path": "a.txt"}}',
         ),
      ).toBe(true);
      expect(isDegenerateOutput('<tool_call>{"name":"x"}</tool_call>')).toBe(
         true,
      );
   });
   ...
   test("does NOT flag ordinary answers, even technical ones", () => {
      expect(
         isDegenerateOutput("¡Listo! Te envié el Excel con tres hojas."),
      ).toBe(false);
      expect(
         isDegenerateOutput(
      ...
      expect(
         isDegenerateOutput(
            'El JSON que me pasaste tiene "name" y "arguments" mal anidados.',
         ),
      ).toBe(false);
...
describe("ask() — degenerate output is never returned", () => {
   test("scaffolding is discarded and the retry text is used instead", async () => {
      createMock
         ...
         .mockResolvedValueOnce(res("Aquí va tu resumen."));
      const out = await ask({
         system: "s",
