...
function fakeTools(
  ...
  return {
    ...
    handle: vi.fn(handle ?? (async () => ({ status: 'success', payload: { id: 33 } }))),
  };
}

/** The Converse request we sent for the i-th send() call. */
function bedrockReqAt(i: number): { messages: Array<{ role: string; content: unknown[] }> } {
...
function baseInput(tools: ComposedTools) {
  return {
    system: 'system',
    messages: [{ role: 'user' as const, content: 'agenda esto (cartel adjunto)' }],
    tools,
...
describe('askBedrock — empty-response retry', () => {
  test('empty end_turn after a tool run retries and returns the later text', async () => {
    ...
    sendMock
      .mockResolvedValueOnce(bedrockToolUse('t1', 'calendar_create_event', { title: 'X' }))
  ...
  test('a retry that re-emits the same tool call is served from cache — no double write', async () => {
    ...
    sendMock
      ...
      .mockResolvedValueOnce(bedrockToolUse('t2', 'calendar_create_event', { title: 'X' }))
