...
function highRiskError(): Error {
   ...
   ) as Error & { status: number; param: string; code: number };
   err.param = "prompt";
   err.code = 400;
   return err;
}
...
describe("classifyLlmError", () => {
   test("protocol/config 4xx are deterministic", () => {
      expect(classifyLlmError(apiError(400, "invalid temperature"))).toBe(
         "deterministic",
      );
      ...
      expect(classifyLlmError(apiError(404, "no such model"))).toBe(
         "deterministic",
      );
   });
   ...
   test("a provider moderation refusal is its own kind, not a config error", () => {
      ...
      expect(
         classifyLlmError(apiError(403, "blocked by the safety system")),
      ).toBe("content_filter");
      expect(classifyLlmError(apiError(400, "content_filter triggered"))).toBe(
         "content_filter",
      );
   });
   ...
   test("genuine bad-parameter 4xx keep their deterministic classification", () => {
      ...
      expect(
         classifyLlmError(
            apiError(400, "invalid temperature: only 1 is allowed"),
         ),
      ).toBe("deterministic");
      expect(classifyLlmError(apiError(401, "Invalid Authentication"))).toBe(
         "deterministic",
      );
      expect(
         classifyLlmError(
      ...
      expect(
         isContentFilterRejection(apiError(429, "high risk of throttling")),
      ).toBe(false);
      expect(
         isContentFilterRejection(apiError(500, "content filter service down")),
      ).toBe(false);
   });
});
...
describe("LlmHealthMonitor", () => {
   test("deterministic error alerts on the FIRST failure (the temperature case)", () => {
      ...
      monitor.reportFailure(
         apiError(400, "400 invalid temperature: only 1 is allowed"),
         1_000,
      );
   ...
   test("transient errors alert only at the consecutive threshold", () => {
      ...
      monitor.reportFailure(
         apiError(429, "throttled"),
         TRANSIENT_ALERT_THRESHOLD * 1_000,
      );
      expect(alerts).toHaveLength(1);
   ...
   test("a content-filter rejection never alerts and never touches the health state", () => {
      ...
      expect(snap.content_filter_rejections).toBe(
   ...
   test("a rejecting or missing sink never throws into the caller", () => {
      const monitor = new LlmHealthMonitor();
      expect(() =>
         monitor.reportFailure(apiError(400, "no sink"), 1_000),
      ).not.toThrow();
