...
describe("ask — routing between Kimi (text) and Bedrock (vision)", () => {
   ...
   describe("Kimi text path (effort high, no images)", () => {
      ...
      test("runs multiple tool calls in one turn (one tool message each, in order)", async () => {
         ...
         expect(kimiToolMsgs(1).map((m) => m.tool_call_id)).toEqual([
            "t1",
            "t2",
         ]);
      });
      ...
      test("hits MAX_TOOL_ITERATIONS, then forces a final answer without tools", async () => {
         ...
         kimiMock.mockResolvedValueOnce(
            kimiEnd("forced synthesis based on partial context"),
         );
         const tools = fakeTools();
         const out = await ask({
            system: "p",
      ...
      test("text-only turn sends content as a plain string (no image parts)", async () => {
         ...
         expect(kimiReqAt(0).messages[1]).toEqual({
            role: "user",
            content: "hi",
         });
         expect(sendMock).not.toHaveBeenCalled();
   ...
   describe("Bedrock vision path (images, or effort low)", () => {
      test("an attached image on the vision tier routes to Bedrock as a Converse image block", async () => {
         ...
         const imgPart = parts[1] as {
            image: { source: { bytes: Uint8Array } };
         };
         expect(imgPart.image.source.bytes).toEqual(
            new Uint8Array([137, 80, 78, 71]),
         );
      });
   });
   ...
   describe("bedrock text backend (LLM_TEXT_BACKEND=bedrock)", () => {
      ...
      test("text turn runs tool calls over Converse in bedrock mode", async () => {
         ...
         try {
            ...
            const tools = fakeTools(async (name) => ({
               status: "success",
               payload: { tool: name, rowCount: 1 },
            }));
            const out = await ask({
               system: "p",
            ...
            expect(tools.handle).toHaveBeenCalledWith("logs_query", {
               env: "dev",
               query: "stats count() | limit 1",
            });
