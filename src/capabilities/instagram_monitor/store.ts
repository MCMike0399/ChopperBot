...
export function computeCadenceInterval(
   ...
   if (span < CADENCE_MIN_SPAN_MS)
      return { intervalMs: null, postsPerDay: null };
   ...
   if (silence > CADENCE_DECAY_START * medianGap) {
      const mult = Math.min(
         silence / medianGap / CADENCE_DECAY_START,
   ...
   interval = Math.min(
      Math.max(interval, CADENCE_MIN_INTERVAL_MS),
...
export function nextDueAtMs(
   ...
   return (
      ...
      Math.min(
         base * 2 ** Math.min(account.consecutive_failures, 10),
         Math.max(MAX_BACKOFF_MS, base),
      )
   );
}
...
export class InstagramMonitorStore {
   ...
   upsertAccount(input: AddAccountInput): {
      account: MonitoredAccount;
      created: boolean;
   } {
   ...
   setPaused(username: string, paused: boolean): MonitoredAccount | null {
      ...
      this.db
         .prepare(
            `UPDATE instagram_monitor_accounts SET paused = ? WHERE username = ?`,
         )
   ...
   listAccounts(): MonitoredAccount[] {
      return this.db
         .prepare(
            `SELECT * FROM instagram_monitor_accounts ORDER BY username ASC`,
         )
   ...
   dueAccounts(
      ...
      for (const r of rows) {
         ...
         const dueAt =
            (r.last_polled_at ?? 0) +
            Math.min(
               base * 2 ** Math.min(r.consecutive_failures, 10),
               Math.max(MAX_BACKOFF_MS, base),
            ) +
            pollJitterMs(
               r.id,
               r.last_polled_at,
               Math.floor(base * POLL_JITTER_FRACTION),
            );
   ...
   recomputeCadence(id: number, nowMs: number): void {
      const row = this.db
         .prepare(
            `SELECT username FROM instagram_monitor_accounts WHERE id = ?`,
         )
      ...
      const posts = this.db
         ...
         .all(row.username, since, CADENCE_MAX_SAMPLES) as {
         posted_at: number;
      }[];
      ...
      return this.db.transaction(() => {
         ...
         for (const a of this.listAccounts()) {
            if (
               ...
               nowMs - a.cadence_updated_at >= CADENCE_TTL_MS
            ) {
   ...
   private appendEvent(
      column: "recent_auth_json" | "recent_429_json",
      nowMs: number,
   ): number {
      const runtime = this.getRuntime();
      const raw =
         column === "recent_auth_json"
            ? runtime.recent_auth_json
            : runtime.recent_429_json;
      let arr: number[] = [];
      if (raw) {
         try {
            const parsed = JSON.parse(raw);
            if (Array.isArray(parsed))
               arr = parsed.filter((n) => typeof n === "number");
         } catch {
            arr = [];
         }
      }
      ...
      this.db
         .prepare(
            `UPDATE instagram_monitor_runtime SET ${column} = ? WHERE id = 1`,
         )
   ...
   writeLastDigestAt(nowMs: number): void {
      this.db
         .prepare(
            `UPDATE instagram_monitor_runtime SET last_digest_at = ? WHERE id = 1`,
         )
         .run(nowMs);
   }
}
