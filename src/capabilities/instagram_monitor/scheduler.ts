...
function quietBoundaryJitterMin(
   nowMs: number,
   tz: string,
   salt: string,
): number {
...
export function inQuietHours(nowMs: number, tz = QUIET_HOURS_TZ): boolean {
   ...
   const start =
      QUIET_HOURS_START_HOUR * 60 + quietBoundaryJitterMin(nowMs, tz, "start");
   const end =
      QUIET_HOURS_END_HOUR * 60 + quietBoundaryJitterMin(nowMs, tz, "end");
...
export class InstagramMonitorScheduler {
   ...
   private lastBlockReason: "none" | "killswitch" | "auth" | "rate" | "budget" =
      "none";
   ...
   private scheduleNextTick(): void {
      ...
      const factor =
         TICK_SPREAD_MIN + Math.random() * (TICK_SPREAD_MAX - TICK_SPREAD_MIN);
   ...
   private async maybeAnnounceResume(now: number): Promise<void> {
      ...
      if (
         ...
         log.debug(
            { reason, pausedForMs },
            "instagram_monitor.tick.resumed_silent",
         );
      }
   }
   ...
   private recomputeStretch(now: number): void {
      ...
      try {
         ...
         if (
            this.lastLoggedStretch === 0 ||
            Math.abs(r.stretch - this.lastLoggedStretch) /
               this.lastLoggedStretch >=
               0.1
         ) {
            this.lastLoggedStretch = r.stretch;
            log.info(
   ...
   async tickOnce(): Promise<void> {
      ...
      try {
         ...
         if (
            this.tickSkipProbability > 0 &&
            Math.random() < this.tickSkipProbability
         ) {
            ...
            if (used >= this.dailyRequestBudget) {
               ...
               if (
                  ...
                  try {
                     ...
                     log.warn(
                        { err },
                        "instagram_monitor.budget_notify_failed",
                     );
                  }
               }
               return;
   ...
   private async processAccount(acc: MonitoredAccount): Promise<void> {
      ...
      try {
         ...
         if (err instanceof InstagramHardAccountError) {
            ...
            if (
               streak === HARD_PAUSE_THRESHOLD &&
               this.deps.notifyAccountAutoPaused
            ) {
               try {
                  ...
                  log.warn(
                     { err: notifyErr },
                     "instagram_monitor.hard_pause_notify_failed",
                  );
               }
            }
            return;
      ...
      if (acc.last_post_id === null) {
         log.info(
            {
               account: acc.username,
               seeded_to: newest.igPostId,
               posts: posts.length,
            },
            "instagram_monitor.first_poll_seed",
         );
         this.deps.store.markPollSuccess(
            acc.id,
            Date.now(),
            newest.igPostId,
            newest.takenAtMs,
      ...
      if (anchorIdx >= 0) {
         ...
         if (acc.last_post_at !== null) {
            ...
            newPostsNewestFirst = newPostsNewestFirst.filter(
               (p) => p.takenAtMs > floor,
            );
         }
      ...
      if (boundChannels.length === 0) {
         log.info(
            {
               account: acc.username,
               new_posts: newPostsNewestFirst.length,
               advanced_to: next.id,
            },
      ...
      for (const post of chronological) {
         ...
         if (
            boundChannels.every((channelId) =>
               this.deps.store.hasSeen(channelId, post.igPostId),
            )
         ) {
            continue;
         }
            ...
            if (pushedHere >= MAX_PUSHES_PER_ACCOUNT_PER_TICK_PER_CHANNEL) {
               this.deps.store.recordSeen({
                  ...
                  classification_json: JSON.stringify({
                     skipped: "rate_limited_per_channel",
                  }),
                  pushed: false,
                  discord_message_id: null,
               });
      ...
      try {
         ...
         log.debug(
            { err, account: acc.username },
            "instagram_monitor.cadence_recompute_failed",
         );
      }
   }
}
