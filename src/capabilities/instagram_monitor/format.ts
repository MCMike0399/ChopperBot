...
export function formatStatusDigest(input: StatusDigestInput): string[] {
   ...
   head.push(
      `Cuentas: ${accounts.length} (pausadas ${pausedCount} · bloqueadas ${blockedCount})`,
   );
...
function describeState(r: RuntimeState, now: number): string {
   ...
   if (rate)
      return `🟡 pausado — límite de peticiones (429), reanuda en ${formatAgeEs(rate - now)}`;
   if (budget)
      return `🟡 pausado — presupuesto diario, reanuda en ${formatAgeEs(budget - now)}`;
   if (auth)
      return `🟡 pausado — sesión, reanuda en ${formatAgeEs(auth - now)}`;
...
function cadenceCell(a: MonitoredAccount): string {
   ...
   if (ppd >= 1)
      return `${ppd < 10 ? ppd.toFixed(1) : String(Math.round(ppd))}/d`;
   const days = 1 / ppd;
...
function stCell(a: MonitoredAccount): string {
   ...
   if (a.consecutive_failures > 0)
      return `!${Math.min(a.consecutive_failures, 9)}`;
   return "·";
}
