...
export class InstagramMonitorToolSource implements ToolSource {
   ...
   tools(): ToolSpec[] {
      ...
      return this.deps.isMod
         ? specs
         : specs.filter((t) => !MUTATING_TOOLS.has(t.name));
   }
   ...
   async handle(toolName: string, input: unknown): Promise<ToolHandlerResult> {
      ...
      try {
         ...
         if (MUTATING_TOOLS.has(toolName) && !this.deps.isMod) {
            log.warn(
               { tool: toolName, user: this.deps.userId },
               "instagram_monitor.tool_denied",
            );
            return {
               status: "error",
               payload: {
                  error: "Solo moderación puede modificar la lista global de cuentas vigiladas (afecta a todos los canales). Pídeselo a un mod.",
               },
            };
         }
         ...
         switch (toolName) {
            case "monitor_add_account": {
               ...
               log.info(
                  { tool: toolName, username, created, ms: Date.now() - t0 },
                  "tool_call",
               );
               return {
                  status: "success",
            ...
            case "monitor_remove_account": {
               ...
               if (!removed) {
                  return {
                     status: "error",
                     payload: {
                        error: `La cuenta @${username} no está en la lista global.`,
                     },
                  };
               }
               return {
                  status: "success",
                  payload: { removed: serializeAccount(removed) },
               };
            }
            ...
            case "monitor_pause_account": {
               ...
               if (typeof obj.paused !== "boolean") {
                  return {
                     status: "error",
                     payload: { error: "paused debe ser boolean." },
                  };
               }
               ...
               return {
                  status: "success",
                  payload: { account: serializeAccount(updated) },
               };
            }
            ...
            case "monitor_recent_pushed": {
               ...
               const rows = this.deps.store.recentPushed(
                  this.deps.channelId,
                  limit,
               );
               return {
                  status: "success",
                  payload: { posts: rows.map(serializeSeen) },
               };
            }
   ...
   return {
      ...
      last_polled_at_iso:
         a.last_polled_at !== null
            ? new Date(a.last_polled_at).toISOString()
            : null,
      last_post_id: a.last_post_id,
      consecutive_failures: a.consecutive_failures,
      consecutive_auth_failures: a.consecutive_auth_failures,
      consecutive_hard_failures: a.consecutive_hard_failures,
      poll_interval_min:
         a.poll_interval_ms !== null
            ? Math.round(a.poll_interval_ms / 60000)
            : null,
      posts_per_day:
         a.posts_per_day !== null ? Number(a.posts_per_day.toFixed(2)) : null,
      cadence_updated_at_iso:
         a.cadence_updated_at !== null
            ? new Date(a.cadence_updated_at).toISOString()
            : null,
   };
}
...
function serializeSeen(p: SeenPost) {
   ...
   if (p.classification_json) {
      try {
         ...
         classification = {
            parse_error: true,
            raw: p.classification_json.slice(0, 80),
         };
      }
   }
   return {
      ...
      posted_at_iso:
         p.posted_at !== null ? new Date(p.posted_at).toISOString() : null,
