...
function fakeFetcher(
   byUsername: Record<string, RecentPost[]>,
): InstagramFetcher {
   return {
...
beforeEach(() => {
   vi.useFakeTimers({
      now: new Date("2026-08-16T12:00:00-06:00"),
      toFake: ["Date"],
   });
});
...
describe("InstagramMonitorScheduler", () => {
   ...
   test("publishes only LLM-relevant posts; records pushed=0 for skipped", async () => {
      ...
      const classify = vi.fn(
         async (_account: string, p: RecentPost): Promise<Classification> => {
            if (p.igPostId === "P2") {
               return {
                  relevant: true,
                  type: "evento",
      ...
      const publishedShortcodes = publish.mock.calls.map(
         (c) => (c[3] as RecentPost).shortcode,
      );
      ...
      expect(store.recentPushed(CHAN, 10).map((s) => s.ig_post_id)).toEqual([
         "P2",
      ]);
      mem.close();
   });
   ...
   test("processes posts oldest-first so Discord sees them in posted order", async () => {
      ...
      const sch = new InstagramMonitorScheduler({
         store,
         fetcher: fakeFetcher({
            foo: [post("P3"), post("P2"), post("P1"), post("P0")],
         }),
         client: fakeClient,
         getBoundChannels: ONE_CHANNEL,
         classify: async () => ({
   ...
   test("caps pushes per account per tick per channel", async () => {
      ...
      const publish = vi.fn(
         async () => ({ ok: true, messageId: "m" }) as PublishResult,
      );
      const sch = new InstagramMonitorScheduler({
         store,
      ...
      for (let i = 1; i <= 10; i++)
         expect(store.hasSeen(CHAN, `P${i}`)).toBe(true);
   ...
   test("new channel binding gets no backfill — only posts detected after the bind", async () => {
      ...
      const publish = vi.fn(
         async (
            ...
            ok: true,
            messageId: `${channelId}:${p.igPostId}`,
         }),
      );
      const sch = new InstagramMonitorScheduler({
         store,
      ...
      (sch as unknown as { deps: { fetcher: InstagramFetcher } }).deps.fetcher =
         fakeFetcher({
            foo: [post("P2"), post("P1")],
         });
      await sch.tickOnce();
      ...
      const got = publish.mock.calls
         .map((c) => `${c[1]}:${(c[3] as RecentPost).igPostId}`)
         .sort();
   ...
   test("anchor missing but genuinely-newer posts present: time-gate publishes them", async () => {
      ...
      expect(
         publish.mock.calls.map((c) => (c[3] as RecentPost).igPostId),
      ).toEqual(["NEW1", "NEW2"]);
   ...
   test("seeding records the anchor capture time", async () => {
      ...
      const sch = new InstagramMonitorScheduler({
         ...
         fetcher: fakeFetcher({
            foo: [
               post("P2", { takenAtMs: 9_000 }),
               post("P1", { takenAtMs: 8_000 }),
            ],
         }),
         client: fakeClient,
         getBoundChannels: ONE_CHANNEL,
...
describe("InstagramMonitorScheduler — guardrails", () => {
      ...
      const sch = new InstagramMonitorScheduler({
         ...
         fetcher: {
            async fetchRecentPosts(u: string) {
               if (u === "aaa")
                  throw new InstagramRateLimitError("throttled (HTTP 429)");
               return [post("B1")];
            },
         },
         client: fakeClient,
   ...
   test("a session-level auth failure (checkpoint) trips the breaker immediately", async () => {
      ...
      const sch = new InstagramMonitorScheduler({
         store,
         fetcher: spyFetcher(
            new InstagramAuthError(
               "Instagram rejected ...",
               "HTTP 400 checkpoint_required",
            ),
         ),
         client: fakeClient,
         getBoundChannels: ONE_CHANNEL,
   ...
   test("an account-specific 401 does NOT trip the breaker or halt other accounts", async () => {
      ...
      const { store, mem } = await newStore();
      const now = Date.now();
      const bad = store.upsertAccount({
         username: "restricted",
         added_by: "U",
      });
      const good = store.upsertAccount({ username: "healthy", added_by: "U" });
      store.markPollSuccess(bad.account.id, now - 25 * 60 * 60 * 1000, "R0");
            ...
            async fetchRecentPosts(u: string) {
               if (u === "restricted") {
                  throw new InstagramAuthError(
                     "rejected feed (HTTP 401)",
                     "HTTP 401",
                  );
               }
               return [
                  post("H1", { takenAtMs: 2_000 }),
                  post("H0", { takenAtMs: 1_000 }),
               ];
            },
         },
         client: fakeClient,
   ...
   test("a deterministic hard failure counts separately and auto-pauses at HARD_PAUSE_THRESHOLD", async () => {
      // Mirrors the 2026-07 laser.provider incident: IG rejects one account's
      // pk-resolve the SAME way on every poll. The scheduler counts these on
      // their own counter, alerts once at the crossing, and dueAccounts then
      // skips the account — retrying forever is pure automation-signature
      // traffic for zero yield.
      const { store, mem } = await newStore();
      const now = Date.now();
      ...
      // Seed the streak one below the threshold, with the last poll far enough
      // back that the account is due despite backoff + worst-case jitter.
      for (let i = 0; i < HARD_PAUSE_THRESHOLD - 1; i++) {
         store.markPollFailure(
            broken.account.id,
            now - (26 - i) * 60 * 60 * 1000,
            { hard: true },
         );
      ...
      const fetchPosts = vi.fn(async (u: string) => {
         if (u === "broken") {
            throw new InstagramHardAccountError(
               "deterministic 400 resolving @broken (laser.provider)",
               "laser.provider",
            );
         }
         return [post("H1", { takenAtMs: 2_000 })];
      });
      const notifyAccountAutoPaused = vi.fn(async () => {});
      const sch = new InstagramMonitorScheduler({
         store,
         fetcher: { fetchRecentPosts: fetchPosts },
         client: fakeClient,
         getBoundChannels: ONE_CHANNEL,
         classify: async () => ({
            relevant: true,
         ...
         fetchCover: async () => null,
         notifyAccountAutoPaused,
      });

      await sch.tickOnce(); // 'healthy' is oldest-due → polls fine; streak untouched
      expect(store.getAccount("broken")?.consecutive_hard_failures).toBe(
         HARD_PAUSE_THRESHOLD - 1,
      );
      expect(notifyAccountAutoPaused).not.toHaveBeenCalled();

      await sch.tickOnce(); // 'broken' → 5th deterministic failure → auto-pause + one alert
      expect(store.getAccount("broken")?.consecutive_hard_failures).toBe(
         HARD_PAUSE_THRESHOLD,
      );
      expect(store.getAccount("broken")?.consecutive_auth_failures).toBe(0); // NOT an auth failure
      expect(notifyAccountAutoPaused).toHaveBeenCalledTimes(1);
      expect(notifyAccountAutoPaused.mock.calls[0][0]).toMatchObject({
         account: "broken",
         failures: HARD_PAUSE_THRESHOLD,
      });

      await sch.tickOnce(); // gated out of dueAccounts → nothing else fetches
      expect(fetchPosts).toHaveBeenCalledTimes(2);
      expect(notifyAccountAutoPaused).toHaveBeenCalledTimes(1); // no alert flapping
      mem.close();
   });
   ...
   test("opportunistic recompute fires when new posts are detected", async () => {
      ...
      const sch = new InstagramMonitorScheduler({
         ...
         fetcher: {
            async fetchRecentPosts() {
               return [
                  post("NEW", { takenAtMs: T + 3_600_000 }),
                  post("H0", { takenAtMs: T }),
               ];
            },
         },
      ...
      (
         sch as unknown as { lastCadenceSweepAtMs: number }
      ).lastCadenceSweepAtMs = Date.now();
      await sch.tickOnce();
...
describe("InstagramMonitorScheduler — status digest", () => {
   function digestScheduler(
      store: InstagramMonitorStore,
      notifyStatusDigest: () => Promise<void>,
   ) {
...
describe("InstagramMonitorScheduler — undecided classifications", () => {
   ...
   const UNDECIDED: Classification = {
      ...DECIDED_IRRELEVANT,
      reason:
         "ask_failed: 500 The server had an error while processing your request",
      undecided: true,
   };
   ...
   test("does not record an undecided post, and holds the anchor below it", async () => {
      ...
      const classify = vi.fn(
         async (_a: string, p: RecentPost): Promise<Classification> =>
   ...
   test("retries the undecided post on the next poll and publishes it once decided", async () => {
      ...
      const p1Calls = classify.mock.calls.filter(
         (c) => (c[1] as RecentPost).igPostId === "P1",
      );
