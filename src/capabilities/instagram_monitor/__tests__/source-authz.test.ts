...
const READ_ONLY = ['monitor_list_accounts', 'monitor_recent_pushed', 'monitor_test_classify'];
...
describe('InstagramMonitorToolSource — watch-list authorization', () => {
  ...
  test('handle() refuses a mutating tool for a non-mod even if it is dispatched anyway', async () => {
    ...
    for (const tool of MUTATING) {
      const res = await source.handle(tool, { username: 'cuenta_atacante', paused: true });
      expect(res.status).toBe('error');
      expect((res.payload as { error: string }).error).toMatch(/moderación/i);
    }
  ...
  test('a moderator can still add an account (the gate is not a wall)', async () => {
    ...
    const res = await source.handle('monitor_add_account', { username: '@RevolucionZ' });
    expect(res.status).toBe('success');
    expect(store.listAccounts().map((a) => a.username)).toEqual(['revolucionz']);
    mem.close();
  });
});
