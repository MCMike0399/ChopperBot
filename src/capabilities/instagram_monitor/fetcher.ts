 ...
 */
export function detectHardAccountBlock(
   status: number,
   body: string,
): string | null {
...
export function clientHintsFromUserAgent(
   userAgent: string,
...
export class DirectInstagramFetcher implements InstagramFetcher {
   ...
   private async maybeWarmup(
      ...
      if (
         this.warmupProbability <= 0 ||
         Math.random() >= this.warmupProbability
      )
         return;
      const headers: Record<string, string> = {
         "User-Agent": this.userAgent,
   ...
   private async resolvePk(
      ...
      try {
         ...
         throw new Error(
            `web_profile_info returned non-JSON resolving @${username}`,
         );
      }
   ...
   private fetchAuthedFeed(
      pk: string,
      auth: InstagramAuth,
   ): Promise<RecentPost[]> {
      return this.requestAuthedFeed(
         IG_FEED_URL(pk, randomFeedCount()),
         `feed/user/${pk}`,
         auth,
      );
   }
...
function bestImageUrl(node: Record<string, unknown>): string {
   const candidates = (
      node.image_versions2 as { candidates?: { url?: unknown }[] } | undefined
   )?.candidates;
...
function firstVideoUrl(node: Record<string, unknown>): string | undefined {
   const url = (node.video_versions as { url?: unknown }[] | undefined)?.[0]
      ?.url;
...
function normalizeFeedItem(it: Record<string, unknown>): RecentPost {
   ...
   const captionText = (it.caption as { text?: unknown } | null | undefined)
      ?.text;
   ...
   if (mt === 8) {
      ...
      for (const child of children) {
         ...
         carouselVideoUrls.push(
            child.media_type === 2 ? (firstVideoUrl(child) ?? null) : null,
         );
      }
   } else if (mt === 2) {
      mediaType = "video";
...
function normalizeNode(node: Record<string, unknown>): RecentPost {
   ...
   const videoUrl =
      node.video_url !== undefined
         ? asString(node.video_url, "video_url")
         : undefined;

   const captionEdges =
      (
         node.edge_media_to_caption as
            { edges?: { node?: { text?: unknown } }[] } | undefined
      )?.edges ?? [];
   const caption =
   ...
   if (isCarousel) {
      ...
      const children =
         (
            node.edge_sidecar_to_children as
               { edges?: { node?: Record<string, unknown> }[] } | undefined
         )?.edges ?? [];
      carouselUrls = [];
      carouselVideoUrls = [];
