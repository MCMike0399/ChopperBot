...
export class WorkshopToolSource implements ToolSource {
      ...
      return [
         {
            ...
            inputSchema: {
               ...
               properties: {
                  code: {
                     type: "string",
                     description: "El script Python completo a ejecutar.",
                  },
                  timeout_s: {
                     type: "number",
            ...
            inputSchema: {
               ...
               properties: {
                  path: {
                     type: "string",
                     description:
                        'Ruta relativa dentro del workspace, p. ej. "notas.md".',
                  },
                  content: { type: "string" },
               },
            ...
            inputSchema: {
               ...
               properties: {
                  path: {
                     type: "string",
                     description:
                        'Ruta relativa del documento, p. ej. "uploads/libro.pdf".',
                  },
               },
               required: ["path"],
            },
         },
         {
            ...
            inputSchema: {
               ...
               properties: {
                  ...
                  query: {
                     type: "string",
                     description: "Qué buscar (palabras clave o una frase).",
                  },
                  max_results: {
                     type: "number",
                     description:
                        "Fragmentos a devolver (por defecto 4, máximo 8).",
                  },
               },
               required: ["path", "query"],
            },
         },
         {
            ...
            inputSchema: {
               ...
               properties: {
                  ...
                  from_page: {
                     type: "number",
                     description: "Primera página (1-based, inclusive).",
                  },
                  to_page: {
                     type: "number",
                     description: "Última página (inclusive).",
                  },
               },
         ...
         },
         {
            ...
            inputSchema: {
               ...
               properties: {
                  path: {
                     type: "string",
                     description: "Ruta relativa dentro del workspace.",
                  },
                  caption: {
                     type: "string",
                     description:
                        "Texto breve opcional que acompaña al archivo.",
                  },
               },
               required: ["path"],
            },
            ...
            inputSchema: {
               ...
               properties: {
                  confirm: {
                     type: "boolean",
                     description:
                        "Debe ser true; confirma primero con el usuario.",
                  },
               },
               required: ["confirm"],
            },
      ...
      try {
         switch (toolName) {
            ...
            case "workshop_read_file": {
               ...
               if (!this.deps.workspace.exists(path)) {
                  return {
                     status: "error",
                     payload: { error: `No existe: ${path}` },
                  };
               }
               if (!this.budgetAllows(maxBytes))
                  return this.budgetExceededResult(`el contenido de ${path}`);
            ...
            case "workshop_list_files": {
               ...
               const undelivered = files
                  .filter((f) => !f.entregado)
                  .map((f) => f.path);
               return {
                  status: "success",
                  payload: {
            ...
            case "workshop_send_file": {
               ...
               if (!this.deps.workspace.exists(path)) {
                  return {
                     status: "error",
                     payload: {
                        error: `No existe: ${path}. Usa workshop_list_files.`,
                     },
                  };
               }
               ...
               if (bytes > MAX_SEND_FILE_BYTES) {
                  return {
                     status: "error",
                     payload: {
                        error: `El archivo pesa ${bytes} bytes; el máximo para Discord es ~9.5 MB. Compréndelo (zip) o divídelo.`,
                     },
                  };
               }
               const caption =
               ...
               return {
                  ...
                  payload: {
                     queued: path,
                     bytes,
                     note: "Se adjuntará al canal junto con tu respuesta.",
                  },
               };
            }
            case "workshop_rename_session": {
               ...
               return res.ok
                  ...
                  : {
                       status: "error",
                       payload: {
                          error: res.error ?? "No pude renombrar el canal.",
                       },
                    };
            }
            ...
            case "workshop_close_session": {
               if (obj.confirm !== true) {
                  return {
                     status: "error",
                     payload: {
                        error: "Confirma primero con el usuario y vuelve a llamar con confirm=true. Cerrar elimina el canal.",
                     },
                  };
               }
               ...
               return {
                  ...
                  payload: {
                     closing: true,
                     note: "El canal se despedirá y se eliminará unos segundos después de tu respuesta.",
                  },
               };
            }
            default:
         ...
         if (err instanceof PathEscapeError) {
            return {
               status: "error",
               payload: {
                  error: "Ruta inválida: usa rutas relativas dentro del workspace.",
               },
            };
         }
   ...
   private async handleDocIndex(
      obj: Record<string, unknown>,
   ): Promise<ToolHandlerResult> {
      const path = String(obj.path ?? "");
      const built = await this.ensureIndex(path);
      if ("error" in built)
         return { status: "error", payload: { error: built.error } };
      const { meta } = built;
      ...
      const payload = {
         ...
         chunks: meta.chunks,
         outline,
         ...(meta.outline.length > 60
            ? { outline_note: `… y ${meta.outline.length - 60} secciones más` }
            : {}),
         note:
   ...
   private async handleDocSearch(
      obj: Record<string, unknown>,
   ): Promise<ToolHandlerResult> {
      const path = String(obj.path ?? "");
      const query = String(obj.query ?? "").trim();
      if (!query)
         return { status: "error", payload: { error: "Falta query." } };
      const k = clampInt(obj.max_results, 1, 8, 4);
      const perHitChars = 1200;
      if (!this.budgetAllows(k * perHitChars))
         return this.budgetExceededResult("más fragmentos del documento");
      ...
      const results = hits.map((h) => ({
         pages:
            h.chunk.page_start === h.chunk.page_end
   ...
   private async handleDocRead(
      ...
      if (remaining < 2_000)
         return this.budgetExceededResult(`las páginas ${from}–${to}`);
      ...
      if (from > built.meta.pages) {
         return {
            status: "error",
            payload: {
               error: `El documento tiene ${built.meta.pages} páginas.`,
            },
         };
      }
   ...
   private async handleRunPython(
      obj: Record<string, unknown>,
   ): Promise<ToolHandlerResult> {
      const code = String(obj.code ?? "");
      if (!code.trim())
         return { status: "error", payload: { error: "Falta el código." } };
      if (!sandboxAvailable()) {
         return {
            status: "error",
            payload: {
               error: "La ejecución de código no está disponible en este host (falta bubblewrap).",
            },
         };
      }
      const timeoutMs =
         clampInt(
            obj.timeout_s,
            1,
            Math.floor(this.deps.maxTimeoutMs / 1000),
            30,
         ) * 1000;

      this.deps.workspace.ensure();
      const before = new Map(
         this.deps.workspace.list().map((f) => [f.path, f.modifiedAt]),
      );
