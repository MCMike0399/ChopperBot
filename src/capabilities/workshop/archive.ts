...
export function renderArchiveContent(
   ...
   const generated = plan.attach.filter(
      (r) => !r.rel_path.startsWith("uploads/"),
   );
   const lines: string[] = [
      "📁 **Archivos del taller** — sobrevivieron la limpieza:",
   ];
   if (uploads.length > 0)
      lines.push("📥 **Subidos por ti**", ...listLines(uploads));
   if (generated.length > 0)
      lines.push("🛠️ **Generados aquí**", ...listLines(generated));
...
export interface ArchiveDeps {
   ...
   store: Pick<
      WorkshopStore,
      "fileManifest" | "recordFile" | "removeFileRecord"
   >;
...
export async function archiveSessionFiles(
   ...
   if (plan.attach.length > 0) {
      ...
      for (
         let i = 0;
         i < plan.attach.length;
         i += MAX_ATTACHMENTS_PER_MESSAGE
      ) {
         const batch = plan.attach.slice(i, i + MAX_ATTACHMENTS_PER_MESSAGE);
         const content =
            i === 0
               ? renderArchiveContent(plan, maxSendBytes)
               : "📁 **Archivos del taller** (cont.)";
         const files = batch.map(
            (rec) =>
               new AttachmentBuilder(workspace.absolute(rec.rel_path), {
                  name: basename(rec.rel_path),
               }),
         );
      ...
      for (const oldId of oldCarrierIds) {
         if (
            stillReferenced.has(oldId) ||
            result.postedMessageIds.includes(oldId)
         )
            continue;
