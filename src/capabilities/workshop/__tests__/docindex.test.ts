...
describe("parseIndexerOutput", () => {
   test("finds the result line among other stdout noise", () => {
      ...
      expect(parseIndexerOutput(out)).toEqual({
         ok: true,
         pages: 3,
         chars: 900,
         chunks: 2,
...
describe("searchChunks (lexical retrieval)", () => {
   const corpus = [
      chunk(
         0,
         "La caza de brujas fue un instrumento de disciplinamiento del cuerpo femenino.",
         "CAPÍTULO 4",
         120,
      ),
      chunk(
         1,
         "El feudalismo entró en crisis por las revueltas campesinas y la peste negra.",
         "CAPÍTULO 1",
         20,
      ),
      chunk(
         2,
         "La acumulación primitiva requirió la separación del campesinado de la tierra común.",
         "CAPÍTULO 2",
         60,
      ),
      chunk(
         3,
         "Recetario: mezclar harina y agua hasta obtener una masa homogénea.",
         null,
         200,
      ),
   ];
   ...
   test("ranks the on-topic chunk first, accent- and case-insensitive", () => {
      const hits = searchChunks(
         corpus,
         "¿Qué dice sobre la caza de BRUJAS?",
         2,
      );
   ...
   test("respects k", () => {
      const hits = searchChunks(
         corpus,
         "campesinado revueltas tierra crisis",
         1,
      );
...
describe.skipIf(!sandboxAvailable())("ensureDocIndex (real sandbox)", () => {
   test("indexes a txt document end-to-end: meta, chunks, outline, freshness", async () => {
      ...
      try {
         ...
         expect(ws.list().every((f) => !f.path.startsWith(".docindex"))).toBe(
            true,
         );
   ...
   test("unsupported extension and missing file are clean Spanish errors", async () => {
      ...
      try {
         ...
         const bad = await ensureDocIndex({
            workspace: ws,
            sourceRel: "datos.xlsx",
            venvDir: null,
            timeoutMs: 10_000,
         });
         expect(bad).toHaveProperty("error");
         const missing = await ensureDocIndex({
            workspace: ws,
            sourceRel: "no-existe.pdf",
            venvDir: null,
            timeoutMs: 10_000,
         });
