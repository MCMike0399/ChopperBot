...
import { renderWorkshopPrompt, type WorkshopPromptContext } from '../preamble.js';
import type { WorkspaceFile } from '../workspace.js';

function ctx(overrides: Partial<WorkshopPromptContext> = {}): WorkshopPromptContext {
  return {
...
const file = (path: string, bytes = 100): WorkspaceFile => ({ path, bytes, modifiedAt: 0 });
...
describe('renderWorkshopPrompt', () => {
  test('workspace listing marks each file with its delivery status', () => {
    const prompt = renderWorkshopPrompt(
      ctx({
        files: [file('enviado.docx'), file('pendiente.pdf'), file('uploads/apuntes.pdf'), file('extracto.txt')],
        deliveredPaths: new Set(['enviado.docx', 'uploads/apuntes.pdf']),
      }),
    );
    expect(prompt).toContain('enviado.docx (100 B) — ✅ ya entregado al usuario');
    expect(prompt).toContain('pendiente.pdf (100 B) — ⚠️ NO entregado');
    expect(prompt).toContain('uploads/apuntes.pdf (100 B) — 📥 subido por el usuario');
