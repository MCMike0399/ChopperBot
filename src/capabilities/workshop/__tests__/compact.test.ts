...
import {
   shouldCompact,
   COMPACT_MIN_OLD_TURNS,
   COMPACT_MIN_OLD_CHARS,
} from "../compact.js";
...
describe("shouldCompact", () => {
   ...
   test("compacts from the turn-count threshold", () => {
      const few = Array.from({ length: COMPACT_MIN_OLD_TURNS - 1 }, () =>
         turn("corto"),
      );
   ...
   test("compacts from the char threshold even with few turns", () => {
      expect(shouldCompact([turn("x".repeat(COMPACT_MIN_OLD_CHARS))])).toBe(
         true,
      );
