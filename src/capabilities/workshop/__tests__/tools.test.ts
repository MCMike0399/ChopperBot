...
import { WorkshopToolSource, capOutput, type SessionActions } from '../tools.js';
...
describe('workshop file tools', () => {
  test('write → list → read round-trip', async () => {
    const src = newSource();
    const w = await src.handle('workshop_write_file', { path: 'ensayo.md', content: '# Hola' });
    expect(w.status).toBe('success');

    const l = await src.handle('workshop_list_files', {});
    expect(l.status).toBe('success');
    expect((l.payload as { files: Array<{ path: string }> }).files.map((f) => f.path)).toEqual([
      'ensayo.md',
    ]);
  ...
  test('read of a missing file and traversal paths are clean errors', async () => {
    const src = newSource();
    const missing = await src.handle('workshop_read_file', { path: 'nope.txt' });
    expect(missing.status).toBe('error');
    const escape = await src.handle('workshop_read_file', { path: '../../etc/passwd' });
  ...
  test('send_file queues an existing file and rejects a missing/oversized one', async () => {
    ...
    const ok = await src.handle('workshop_send_file', { path: 'reporte.txt', caption: 'aquí está' });
...
describe('workshop session tools', () => {
  ...
  test('close_session requires confirm=true', async () => {
    ...
    const confirmed = await src.handle('workshop_close_session', { confirm: true });
  ...
  test('rename_session delegates to the action', async () => {
    const src = newSource();
    const res = await src.handle('workshop_rename_session', { name: 'tesis' });
    expect(res.status).toBe('success');
