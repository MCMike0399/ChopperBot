...
describe("SessionWorkspace — symlink containment", () => {
   ...
   test("writing through a symlink cannot modify a file outside the workspace", () => {
      ...
      expect(() => ws.writeText("payload.js", "malicious")).toThrow(
         PathEscapeError,
      );
      expect(() =>
         ws.writeBytes("payload.js", new Uint8Array([1, 2, 3])),
   ...
   test("a symlinked DIRECTORY component is refused too (the whole path is walked)", () => {
      ...
      expect(() => ws.readText("escape/nota.txt", 1000)).toThrow(
         PathEscapeError,
      );
      expect(() => ws.writeText("escape/nuevo.txt", "x")).toThrow(
         PathEscapeError,
      );
...
describe("listUndeliveredDeliverables", () => {
   const file = (path: string, modifiedAt: number, bytes = 100) => ({
      path,
      bytes,
      modifiedAt,
   });
   ...
   test("returns deliverables created or rewritten this turn, minus skips", () => {
      ...
      const found = listUndeliveredDeliverables(before, after, skip).map(
         (f) => f.path,
      );
