...
describe("workshop storage glue", () => {
   test("storageKeyFor is deterministic and session-scoped", () => {
      expect(storageKeyFor("123", "uploads/libro.pdf")).toBe(
         "workshop/123/uploads/libro.pdf",
      );
   });
   ...
   test("uploadToStorage stores the bytes and records the key", async () => {
      ...
      expect(ok).toBe(true);
      expect(storage.objects.get("workshop/c1/a.txt")).toEqual(
         new Uint8Array([7, 8]),
      );
   ...
   test("restoreFromStorage: no key, missing object or a down backend → null (caller falls back)", async () => {
      ...
      // NULL storage_key
      expect(
         await restoreFromStorage(storage, ws, store.fileManifest("c1")[0]!),
      ).toBeNull();
...
describe("storageBackfill (heals the MinIO boot race)", () => {
   ...
   async function backfillSetup() {
      ...
      const ws = new SessionWorkspace(
         join(dataDir, "workshop", "sessions", CH),
      );
      ws.ensure();
      const storage = new FakeStorage();
   ...
   test("uploads NULL-storage_key files from the local cache and records the key", async () => {
      ...
      expect([...storage.objects.get(`workshop/${CH}/uploads/a.txt`)!]).toEqual(
         [1, 2, 3],
      );
      expect(store.fileManifest(CH)[0]!.storage_key).toBe(
         `workshop/${CH}/uploads/a.txt`,
      );
