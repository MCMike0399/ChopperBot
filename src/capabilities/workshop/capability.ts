...
export class WorkshopCapability implements Capability {
   ...
   private messageListener:
      ((m: OmitPartialGroupDMChannel<Message>) => void) | null = null;
   ...
   private channelDeleteListener:
      ((c: DMChannel | NonThreadGuildBasedChannel) => void) | null = null;
   ...
   async init({ memory, projectRoot }: CapabilityInitDeps): Promise<void> {
      ...
      if (this.storage && !(await this.storage.ensureReady())) {
         log.warn(
            { capability: this.id, backend: this.storage.backend },
            "workshop.storage_unavailable",
         );
      }
      log.info(
         {
   ...
   async start({ client }: CapabilityStartDeps): Promise<void> {
      if (!this.store)
         throw new Error("WorkshopCapability.start() before init()");
   ...
   async dispose(): Promise<void> {
      ...
      if (this.messageListener)
         this.boundClient.off(Events.MessageCreate, this.messageListener);
      if (this.reactionListener)
         this.boundClient.off(Events.MessageReactionAdd, this.reactionListener);
      if (this.interactionListener)
         this.boundClient.off(
   ...
   isSessionChannel(channelId: string): boolean {
      ...
      if (
         !this.sessionCache ||
         now - this.sessionCache.at > SESSION_CACHE_TTL_MS
      ) {
         this.sessionCache = {
            ids: new Set(this.store.activeChannelIds()),
            at: now,
         };
      }
   ...
   async adminCloseSession(channelId: string): Promise<boolean> {
      const ok =
         (await this.watcher?.closeSession(channelId, "admin")) ?? false;
