...
import { EVENT_INTAKE_MIGRATIONS, EventIntakeStore } from '../../event_intake/store.js';
...
function stubCapability(id: string): Capability {
  return {
    ...
    description: `${id} (stub)`,
    init: async () => {},
    buildTurn: async () => ({ system: '', tools: { tools: [], handle: async () => ({ status: 'success', payload: {} }) } }),
...
async function healthyDeps(over: Partial<HealthDeps> = {}) {
  ...
  for (const id of ['configuration', 'calendar', 'instagram_monitor', 'file_scanner', 'event_intake', 'general_chat']) {
    registry.register(stubCapability(id));
  }
  const router = buildRouter(new Map([['CH_CAL', 'calendar'], ['CH_IG', 'instagram_monitor']]));
  const client = { guilds: { cache: new Map([['G1', { id: 'G1', name: 'Revolución Z' }]]) } };
...
describe('collectHealth', () => {
  ...
  test('a failing LLM is "down" — the bot cannot answer at all', async () => {
    ...
    expect((r.llm as { health: { last_error: string } }).health.last_error).toContain('invalid api key');
    memory.close();
  });
  ...
  test('a capability that failed init is named WITH its reason', async () => {
    ...
    expect((r.capabilities as { skipped: unknown[] }).skipped).toHaveLength(1);
    memory.close();
  });
  ...
  test('the IG kill-switch surfaces as a problem with the fix command', async () => {
    ...
    expect((r.instagram_monitor as { polling_stopped: boolean }).polling_stopped).toBe(true);
    memory.close();
  });
  ...
  test('an unpublished current month is flagged — the Aug 1 miss', async () => {
    ...
    expect((r.calendar as { months_missing: string[] }).months_missing).toEqual(['2026-08']);
    memory.close();
  });
  ...
  test('a calendar with no output channel is flagged (DB null AND no env fallback)', async () => {
    ...
    try {
      ...
      expect((r.calendar as { output_channel_id: string | null }).output_channel_id).toBeNull();
    } finally {
  ...
  test('the env output channel is used when the DB setting is empty', async () => {
    ...
    try {
      const r = collectHealth(deps);
      expect((r.calendar as { output_channel_id: string | null }).output_channel_id).toBe('12345678901234567890');
  ...
  test('a channel bound to an unregistered capability is caught', async () => {
    ...
    const r = collectHealth(deps);
    expect(r.status).toBe('degraded');
    expect((r.capabilities as { orphan_bindings: unknown[] }).orphan_bindings).toEqual([
      { channel_id: 'CH_GONE', capability: 'retired_capability' },
    ]);
    memory.close();
  });
  ...
  test('an un-migrated subsystem degrades ITS block only — the report still returns', async () => {
    ...
    const memory = new SqliteMemoryStore({ path: ':memory:' });
    const registry = new CapabilityRegistry();
    for (const id of ['configuration', 'instagram_monitor', 'file_scanner', 'event_intake']) {
      registry.register(stubCapability(id));
    }
