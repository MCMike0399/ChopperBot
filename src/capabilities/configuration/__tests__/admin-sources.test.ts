...
describe("ConfigInstagramAdminSource", () => {
   ...
   beforeEach(async () => {
      ...
      src = new ConfigInstagramAdminSource({
         db: memory.db(),
   ...
   test("add then list surfaces the account", async () => {
      const added = ok(
         await src.handle("config_instagram", {
            action: "add",
            username: "NASA",
         }),
      );
      ...
      expect((list.accounts as Payload[]).map((a) => a.username)).toEqual([
         "nasa",
      ]);
   });
   ...
   test("unknown action and missing username produce clear errors", async () => {
      expect(
         err(await src.handle("config_instagram", { action: "frobnicate" })),
      ).toMatch(/must be one of/);
      expect(
         err(await src.handle("config_instagram", { action: "add" })),
      ).toMatch(/username/);
   });
   ...
   test("resume_monitor requires confirm and clears the persistent kill-switch", async () => {
      ...
      expect(
         err(
            await src.handle("config_instagram", { action: "resume_monitor" }),
         ),
      ).toMatch(/confirm/);
      const resumed = ok(
         await src.handle("config_instagram", {
   ...
   test("status reports the kill-switch state and account aggregates", async () => {
      ...
      const status = ok(
         await src.handle("config_instagram", { action: "status" }),
      );
...
describe("ConfigCalendarAdminSource", () => {
   ...
   beforeEach(async () => {
      ...
      src = new ConfigCalendarAdminSource({
         db: memory.db(),
   ...
   test("update edits any event but requires confirm", async () => {
      ...
      const ev = cal.create({
         created_by: ALICE,
         title: "Old",
         start_at: Date.parse("2026-06-01T15:00:00Z"),
      });
      expect(
         err(
            await src.handle("config_calendar", {
   ...
   test("output channel can be set + read, and rejects a bad snowflake", async () => {
      ...
      ok(
         await src.handle("config_calendar", {
            ...
            channel_id: "1518328211165941912",
         }),
      );
      expect(
         (
...
describe("ConfigDbSource — read-only safety", () => {
   ...
   beforeEach(async () => {
      ...
      new InstagramMonitorStore(memory.db()).upsertAccount({
         username: "nasa",
         added_by: OPERATOR,
      });
   ...
   test("writes are rejected and leave data intact", async () => {
      for (const sql of [
         ...
         expect(
            err(await src.handle("config_db", { action: "query", sql })),
         ).toBeTruthy();
      }
      ...
      const r = ok(
         await src.handle("config_db", {
            action: "query",
            sql: "SELECT COUNT(*) AS n FROM instagram_monitor_accounts",
         }),
      );
   ...
   test("multi-statement queries are rejected", async () => {
      expect(
         err(
            await src.handle("config_db", {
               action: "query",
               sql: "SELECT 1; DROP TABLE instagram_monitor_accounts",
            }),
         ),
      ).toBeTruthy();
   });
   ...
   test("describe_schema returns columns for a known table", async () => {
      const r = ok(
         await src.handle("config_db", {
            ...
            table: "instagram_monitor_accounts",
         }),
      );
...
function makeClientWithChannel(
   channelId: string,
   perms: bigint[] | null,
): Client {
   const guild = {
...
describe("ConfigurationToolSource — grouped bindings + permissions", () => {
   ...
   test("by_capability groups channels under each capability", async () => {
      ...
      const r = ok(
         await src.handle("config_bindings", { action: "by_capability" }),
      );
      const caps = r.capabilities as Payload[];
      const ig = caps.find((c) => c.capability_id === "instagram_monitor")!;
      expect(ig.channel_count).toBe(2);
      expect(
         (ig.channels as Payload[]).map((c) => c.channel_id).sort(),
      ).toEqual([CH_A, CH_B].sort());
   ...
   test("check_permissions reports flags and can_push verdict", async () => {
      ...
      const src = makeSource(client, []);
      const r = ok(
         await src.handle("config_discovery", {
   ...
   test("binding to instagram_monitor warns when Attach Files is missing", async () => {
      ...
      const src = makeSource(client, []);
      const r = ok(
         await src.handle("config_bindings", {
            action: "bind",
            channel_id: CH_A,
