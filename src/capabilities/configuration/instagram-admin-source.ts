            ...
            inputSchema: {
               ...
               properties: {
                  ...
                  confirm: {
                     type: "boolean",
                     description:
                        'Must be true for "remove", "reset_anchor", and "resume_monitor".',
                  },
               },
               required: ["action"],
            },
   ...
   async handle(toolName: string, input: unknown): Promise<ToolHandlerResult> {
      ...
      try {
         ...
         if (action === "resume_monitor") {
            ...
            log.info(
               {
                  tool: "config_instagram.resume_monitor",
                  was_stopped: was,
                  ms: Date.now() - t0,
               },
               "tool_call",
         ...
         switch (action) {
            ...
            case "pause":
            case "resume": {
               const updated = this.store.setPaused(
                  username,
                  action === "pause",
               );
            ...
            case "reset_anchor": {
               ...
               log.info(
                  {
                     tool: "config_instagram.reset_anchor",
                     username,
                     ms: Date.now() - t0,
                  },
                  "tool_call",
               ...
               return {
                  status: "success",
                  payload: { account: serializeAccount(reset) },
               };
            }
         }
      } catch (err) {
   ...
   private buildStatus(): Record<string, unknown> {
      ...
      return {
         kill_switch: {
            ...
            resume_with:
               r.global_stop === 1
                  ? "config_instagram action:resume_monitor confirm:true"
                  : null,
         },
         cooldowns: {
         ...
         accounts: {
            ...
            auth_blocked: accounts.filter(
               (a) => a.consecutive_auth_failures >= 5,
            ).length,
            with_failures: accounts.filter((a) => a.consecutive_failures > 0)
               .length,
         },
         adaptive: {
...
function notFound(username: string): ToolHandlerResult {
   return {
      status: "error",
      payload: { error: `No monitored account "${username}".` },
   };
}
