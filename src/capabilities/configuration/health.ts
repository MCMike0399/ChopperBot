 ...
 */
function overallStatus(signals: { llmDegraded: boolean; problems: string[] }): 'ok' | 'degraded' | 'down' {
...
export function collectHealth(deps: HealthDeps): HealthReport {
  ...
  if (uptimeMs < 10 * 60_000) problems.push(`Reinicio reciente: lleva ${humanAge(uptimeMs)} arriba.`);
  ...
  const capabilities = {
    ...
    orphan_bindings: [...deps.router.getAllBindings().entries()]
      ...
      .map(([channelId, capId]) => ({ channel_id: channelId, capability: capId })),
  };
  for (const s of deps.skipped) problems.push(`Capability "${s.id}" no arrancó: ${s.error}`);
  if (capabilities.orphan_bindings.length > 0) {
    problems.push(`${capabilities.orphan_bindings.length} canal(es) bindeados a una capability no registrada.`);
  }

  const discord = safe(() => ({
    guilds: [...deps.client.guilds.cache.values()].map((g) => ({ id: g.id, name: g.name })),
  ...
  const instagram = safe(() => {
    ...
    const authBlocked = accounts.filter((a) => a.consecutive_auth_failures >= 5);
    const budget = config.IG_DAILY_REQUEST_BUDGET;
    const used = rt.requests_24h ?? 0;
    if (rt.global_stop === 1) {
      problems.push(`Monitor de Instagram DETENIDO (kill-switch): ${rt.stop_reason ?? 'sin razón registrada'}. Reanuda con \`config_instagram action:resume_monitor confirm:true\`.`);
    }
    if (rt.budget_pause_until !== null && rt.budget_pause_until > now) {
      problems.push(`Monitor de Instagram en pausa por presupuesto hasta ${iso(rt.budget_pause_until)}.`);
    }
    if (rt.rate_cooldown_until !== null && rt.rate_cooldown_until > now) {
      problems.push(`Monitor de Instagram en cooldown por rate-limit hasta ${iso(rt.rate_cooldown_until)}.`);
    }
    if (authBlocked.length > 0) {
      problems.push(`${authBlocked.length} cuenta(s) de IG auto-pausadas por fallos de autenticación.`);
    }
    return {
      ...
      accounts_cold_start: accounts.filter((a) => a.poll_interval_ms === null).length,
    ...
    return {
      ...
      series_bounded: series
        ...
        .map((e) => ({
          id: e.id,
          title: e.title,
          occurrences: countOccurrencesUntil(e.start_at, e.recurrence_freq!, e.recurrence_until),
  ...
  const fileScanner = safe(() => {
    ...
    if (!enabled) {
      return { enabled: false, reason: 'No registrada (falta VIRUSTOTAL_API_KEY o falló el init).' };
    }
    ...
    if (used >= budget) problems.push('File scanner sin presupuesto de VirusTotal en la ventana de 24 h.');
    return {
      enabled: true,
  ...
  const eventIntake = safe(() => {
    ...
    if (!enabled) return { enabled: false, reason: 'No registrada (falló el init).' };
    ...
    for (const t of recent) byStatus[t.status] = (byStatus[t.status] ?? 0) + 1;
    if (categories.length === 0) {
      problems.push('event_intake no tiene categorías de tickets configuradas — está inactiva.');
    }
    ...
    return {
      ...
      mod_roles_effective: configuredRoles.length > 0 ? configuredRoles : [...DEFAULT_MOD_ROLES],
      mod_roles_source: configuredRoles.length > 0 ? 'configured' : 'default (roles.ts)',
