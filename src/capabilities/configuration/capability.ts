...
import {
   ConfigWorkshopAdminSource,
   type ConfigWorkshopAdminDeps,
} from "./workshop-admin-source.js";
import {
   ConfigMinutasAdminSource,
   type ConfigMinutasAdminDeps,
} from "./minutas-admin-source.js";
import { ConfigDbSource } from "./db-source.js";
import {
   renderConfigurationPrompt,
   renderUnauthorizedConfigurationPrompt,
} from "./preamble.js";
...
export class ConfigurationCapability implements Capability {
   ...
   bootStore(): ConfigurationStore {
      if (!this.store)
         throw new Error("ConfigurationCapability not initialized");
      return this.store;
   }
    ...
    */
   attachWorkshopAdmin(
      hooks: NonNullable<ConfigWorkshopAdminDeps["workshop"]>,
   ): void {
    ...
    */
   attachMinutasAdmin(
      hooks: NonNullable<ConfigMinutasAdminDeps["minutas"]>,
   ): void {
   ...
   async buildTurn(ctx: CapabilityTurnContext): Promise<CapabilityTurnBundle> {
      if (!this.store || !this.db) {
         throw new Error(
            "ConfigurationCapability.buildTurn called before init",
         );
      }
