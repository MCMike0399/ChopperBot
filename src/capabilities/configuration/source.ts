...
export class ConfigurationToolSource implements ToolSource {
      ...
      return [
         {
            ...
            inputSchema: {
               ...
               properties: {
                  action: {
                     type: "string",
                     enum: ["list", "by_capability", "bind", "unbind"],
                  },
                  channel_id: {
                     type: "string",
                     description:
                        "Discord channel snowflake (17–20 digits). Required for bind/unbind.",
                  },
                  capability: {
                     type: "string",
                     description:
                        'Registered capability id, e.g. "calendar" or "instagram_monitor". Required for bind.',
                  },
               },
               required: ["action"],
            },
         },
         {
            ...
            inputSchema: {
               ...
               properties: {
                  ...
                  guild_id: {
                     type: "string",
                     description: 'Required for "guild_channels".',
                  },
                  channel_id: {
                     type: "string",
                     description: 'Required for "check_permissions".',
                  },
               },
               required: ["action"],
            },
            ...
            inputSchema: {
               ...
               properties: {
                  ...
                  limit: {
                     ...
                     minimum: 1,
                     maximum: 100,
                     description: 'For "list_users".',
                  },
                  capability: {
                     type: "string",
                     description: 'For "purge_channel_data".',
                  },
                  channel_id: {
                     type: "string",
                     description: 'For "purge_channel_data".',
                  ...
                  confirm: {
                     type: "boolean",
                     description: 'Must be true for "purge_channel_data".',
                  },
               },
               required: ["action"],
            },
      ...
      switch (action) {
         ...
         case "purge_channel_data":
            return this.handlePurge(
               obj,
               t0,
               "config_system.purge_channel_data",
            );
      }
   }
   ...
   private handleListBindings(t0: number, toolName: string): ToolHandlerResult {
      ...
      log.info(
         { tool: toolName, count: bindings.length, ms: Date.now() - t0 },
         "tool_call",
      );
   ...
   private handleBindChannel(
      ...
      if (channelId === CONFIGURATION_CHANNEL_ID) {
         return {
            status: "error",
            payload: {
               error: "Cannot rebind the configuration channel — it is hardcoded to the configuration capability.",
            },
         };
      }
      if (capability === CONFIGURATION_CAPABILITY_ID) {
         return {
            status: "error",
            payload: {
               error: "The 'configuration' capability is bound to a single hardcoded channel and cannot be assigned elsewhere.",
            },
         };
      }
      if (capability === GENERAL_CHAT_CAPABILITY_ID) {
         return {
            status: "error",
            payload: {
               error: "The 'general_chat' capability is the bot's baseline fallback and cannot be bound to a specific channel — it runs automatically wherever no other capability is bound.",
            },
         };
      }
      ...
      log.info(
         {
            ...
            channelId,
            capability,
            previous: previous?.capability_id ?? null,
            ms: Date.now() - t0,
         },
         "tool_call",
      );
   ...
   private handleUnbindChannel(
      ...
      return {
         status: "success",
         payload: { channel_id: channelId, removed: true },
      };
   }
   ...
   private handleListGuildChannels(
      obj: Record<string, unknown>,
   ): ToolHandlerResult {
      ...
      if (!guild) {
         return {
            status: "error",
            payload: { error: `Guild ${guildId} not found or not joined.` },
         };
      }
   ...
   private handlePurge(
      ...
      if (obj.confirm !== true) {
         return {
            status: "error",
            payload: {
               error: "Refusing destructive purge without `confirm: true`. Re-run the tool with confirm set explicitly.",
            },
         };
      }
      if (capability === CONFIGURATION_CAPABILITY_ID) {
         return {
            status: "error",
            payload: { error: "Refusing to purge configuration_* tables." },
         };
      }
   ...
   private handleBindingsByCapability(t0: number): ToolHandlerResult {
      ...
      log.info(
         {
            tool: "config_bindings.by_capability",
      ...
      return {
         status: "success",
         payload: { capabilities, orphan_bindings: orphans },
      };
   }
   ...
   private handleCheckPermissions(
      ...
      log.info(
         {
            tool: "config_discovery.check_permissions",
            channelId,
            ms: Date.now() - t0,
         },
         "tool_call",
      ...
      return {
         status: "success",
         payload: { channel_id: channelId, ...result },
      };
   }
   ...
   private computePushPermissions(channelId: string): PushPermissionReport {
      const channel = this.deps.client.channels.cache.get(
         channelId,
      ) as unknown as ChannelPermProbe | undefined;
      if (!channel) {
         return {
            resolved: false,
