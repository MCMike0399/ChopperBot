      ...
      return [
         {
            ...
            inputSchema: {
               ...
               properties: {
                  action: {
                     type: "string",
                     enum: ["status", "set_output_channel", "end_session"],
                  },
                  channel_id: {
                     type: "string",
                     description: "For set_output_channel: text channel id.",
                  },
               },
               required: ["action"],
            },
         ...
         switch (action) {
            case "status": {
               ...
               const active = this.deps.guildId
                  ? (this.deps.minutas?.activeSession(this.deps.guildId) ??
                    null)
                  : null;
               return {
                  status: "success",
                  payload: {
                     ...
                     transcriber_available:
                        this.deps.minutas?.transcriberAvailable() ?? null,
            ...
            case "set_output_channel": {
               ...
               if (!SNOWFLAKE.test(id)) {
                  return {
                     status: "error",
                     payload: {
                        error: "channel_id debe ser un snowflake de canal de texto.",
                     },
                  };
               }
               this.store.setOutputChannelId(id);
               log.info(
                  { tool: toolName, channel: id, by: this.deps.callerUserId },
                  "minutas.set_output_channel",
               );
               return {
                  status: "success",
                  payload: {
                     output_channel_id: id,
                     note: `Las minutas se publican ahora en <#${id}>.`,
                  },
               };
            }
            case "end_session": {
               if (!this.deps.guildId) {
                  return {
                     status: "error",
                     payload: {
                        error: "No puedo resolver el servidor actual.",
                     },
                  };
               }
               if (!this.deps.minutas) {
                  return {
                     status: "error",
                     payload: {
                        error: "La capacidad minutas no está registrada (falló al iniciar).",
                     },
                  };
               }
               const ack = await this.deps.minutas.endActiveSession(
                  this.deps.guildId,
               );
