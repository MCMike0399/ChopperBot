      ...
      try {
         switch (action) {
            case "status": {
               ...
               const venv = existsSync(
                  join(this.deps.dataDir, "workshop", "venv", "bin", "python3"),
               );
               return {
                  status: "success",
                  payload: {
                     ...
                     max_sessions_per_user:
                        config.WORKSHOP_MAX_SESSIONS_PER_USER,
                     ...
                     note:
                        !settings.welcome_channel_id || !settings.category_id
            ...
            case "list_sessions": {
               ...
               return {
                  status: "success",
                  payload: { active: sessions.length, sessions },
               };
            }
            case "close_session": {
               const channelId = String(obj.channel_id ?? "").replace(
                  /[<#>]/g,
                  "",
               );
               if (!/^\d{17,20}$/.test(channelId)) {
                  return {
                     status: "error",
                     payload: { error: "channel_id inválido." },
                  };
               }
               if (!this.deps.workshop) {
                  return {
                     status: "error",
                     payload: {
                        error: "La capacidad workshop no está registrada.",
                     },
                  };
               }
               const ok = await this.deps.workshop.adminCloseSession(channelId);
               log.info(
                  { channelId, by: this.deps.callerUserId, ok },
                  "workshop.admin_close",
               );
               return ok
                  ...
                  : {
                       status: "error",
                       payload: {
                          error: "No pude cerrar esa sesión (¿existe y está activa?).",
                       },
                    };
            }
            case "set_channels": {
               ...
               this.store.setChannels(
                  welcome ?? cur.welcome_channel_id,
                  category ?? cur.category_id,
               );
               if (this.deps.workshop)
            ...
            case "repost_welcome": {
               ...
               return {
                  status: "success",
                  payload: { welcome_message_id: settings.welcome_message_id },
               };
            }
            default:
               return {
