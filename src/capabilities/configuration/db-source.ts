...
export class ConfigDbSource implements ToolSource {
            ...
            inputSchema: {
               ...
               properties: {
                  ...
                  table: {
                     type: "string",
                     description:
                        'For "describe_schema" (omit for all tables).',
                  },
                  name: {
                     type: "string",
                     description: 'Table name for "inspect_table".',
                  },
                  limit: {
                     type: "integer",
                     minimum: 1,
                     maximum: 100,
                     description: 'For "inspect_table".',
                  },
                  sql: {
                     type: "string",
                     description: 'Read-only SQL for "query".',
                  },
                  params: {
                     type: "array",
                     description:
                        'Optional positional bind values for ? placeholders in "query".',
                     items: {},
                  },
               },
               required: ["action"],
      ...
      try {
         ...
         switch (action) {
            case "list_tables":
               return {
                  status: "success",
                  payload: { tables: this.deps.store.listTables() },
               };
            case "migrations":
               return {
                  status: "success",
   ...
   private handleDescribeSchema(
      ...
      if (obj.table !== undefined && obj.table !== null && obj.table !== "") {
         ...
         if (!createRow)
            return {
               status: "error",
               payload: { error: `Table "${name}" does not exist.` },
            };
         const columns = (
         ...
         return {
            status: "success",
            payload: { table: name, columns, sql: createRow.sql },
         };
      }
   ...
   private handleQuery(
      ...
      if (!READ_ONLY_LEADERS.has(leader)) {
         log.warn(
            { tool: "config_db.query", rejected: "leader", leader },
            "config_db.query_rejected",
         );
         return {
            status: "error",
            payload: {
      ...
      if (leader === "PRAGMA" && sql.includes("=")) {
         log.warn(
            { tool: "config_db.query", rejected: "assignment_pragma" },
            "config_db.query_rejected",
         );
         return {
            status: "error",
            payload: {
               error: "Assignment PRAGMAs are not allowed (read-only).",
            },
         };
      }
      ...
      try {
         ...
         log.warn(
            { tool: "config_db.query", rejected: "prepare_failed" },
            "config_db.query_rejected",
         );
         return {
            status: "error",
            payload: {
               error: `Could not prepare query (one statement only): ${err instanceof Error ? err.message : String(err)}`,
            },
         };
      }
      if (!stmt.readonly) {
         log.warn(
            { tool: "config_db.query", rejected: "not_readonly" },
            "config_db.query_rejected",
         );
         return {
            status: "error",
            payload: {
               error: "Query rejected: it writes to the database. This tool is read-only.",
            },
         };
      }
      ...
      return {
         status: "success",
         payload: { row_count: rows.length, truncated, rows },
      };
   }
}
...
function parseParams(
   v: unknown,
): Array<string | number | bigint | Buffer | null> {
   if (v === undefined || v === null) return [];
   if (!Array.isArray(v))
      throw new Error("params: must be an array of bind values");
   return v.map((p, i) => {
      ...
      if (
         ...
         return p;
      throw new Error(
         `params[${i}]: bind values must be string, number, bigint, or null`,
      );
   });
}
