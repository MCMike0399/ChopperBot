      ...
      try {
         switch (action) {
            ...
            case "list_channels":
               return {
                  status: "success",
                  payload: { channels: this.store.getWatchedChannels() },
               };
            case "set_channels": {
               const raw = (
               ...
               if (invalid.length > 0) {
                  return {
                     status: "error",
                     payload: {
                        error: `No reconozco estos valores: ${invalid.join(", ")} (usa ids de canal, "guild:<idServidor>", "este servidor" o "todos").`,
                     },
                  };
               }
