...
function opusFrame(encoder: InstanceType<typeof OpusEncoder>, seed: number): Buffer {
  const pcm = Buffer.alloc(FRAME_SAMPLES * OPUS_CHANNELS * 2);
  for (let i = 0; i < FRAME_SAMPLES; i++) {
    const v = Math.round(6000 * Math.sin((2 * Math.PI * 220 * (seed * FRAME_SAMPLES + i)) / OPUS_SAMPLE_RATE));
...
describe('burstCapturedNothing', () => {
  it('flags a burst that received packets but wrote no audio', () => {
    expect(burstCapturedNothing({ bytes: 0, packets: 40, decodeErrors: 40 })).toBe(true);
  });

  it('does not flag a silent speaker who sent nothing at all', () => {
    expect(burstCapturedNothing({ bytes: 0, packets: 0, decodeErrors: 0 })).toBe(false);
  });

  it('does not flag a burst that produced audio despite some bad packets', () => {
    expect(burstCapturedNothing({ bytes: 64000, packets: 100, decodeErrors: 7 })).toBe(false);
  });
});
...
describe('recordOpusStreamToPcm', () => {
  ...
  it('survives corrupt packets mid-stream and keeps the rest of the burst', async () => {
    ...
    for (let i = 0; i < 150; i++) {
      ...
      if (i % 10 === 0) packets.push(Buffer.from([0xff, 0xff, 0xff, 0xff, 0xff]));
    }
  ...
  it('keeps working on a later burst after a fully corrupt one (no shared state)', async () => {
    const enc = new OpusEncoder(OPUS_SAMPLE_RATE, OPUS_CHANNELS);
    const garbage = Array.from({ length: 60 }, () => Buffer.from([0xfe, 0x01, 0x02, 0x03]));
    const poisoned = await recordOpusStreamToPcm(streamOf(garbage), join(dir, 'poison.pcm'));
    ...
    const after = await recordOpusStreamToPcm(streamOf(good), join(dir, 'after-poison.pcm'));
  ...
  it('reports an all-garbage burst as having captured nothing', async () => {
    const garbage = Array.from({ length: 40 }, () => Buffer.from([0xfe, 0xaa, 0xbb]));
    const result = await recordOpusStreamToPcm(streamOf(garbage), join(dir, 'allbad.pcm'));
  ...
  it('resolves cleanly for a speaker who sent no packets at all', async () => {
    const result = await recordOpusStreamToPcm(streamOf([]), join(dir, 'empty.pcm'));
