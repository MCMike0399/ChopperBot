...
class FakeTranscriber implements Transcriber {
  ...
  async transcribe(wavPath: string, outBase: string) {
    ...
    if (name.startsWith('batch-002-'))
      return [{ startMs: 0, endMs: 1500, text: 'propongo hacer el foro el sábado' }];
    return [];
  }
}
    ...
    channels: {
      fetch: async () => ({
        ...
        send: async (payload: Record<string, unknown>) => {
          sent.push(payload);
          return { id: `msg-${sent.length}`, url: `https://discord.com/channels/g/c/msg-${sent.length}` };
        },
      }),
    },
...
async function newHarness() {
  ...
  await new NamespacedMemory(mem, 'minutas').migrate('minutas', MINUTAS_MIGRATIONS);
...
async function fabricateSessionDir(sessionsDir: string, opts: { withAudio?: boolean; withChat?: boolean }) {
  ...
  if (opts.withAudio) {
    ...
    await writeFile(
      ...
      [
        JSON.stringify({ seq: 1, userId: 'u1', speaker: 'Ana', file: 'audio/001-Ana.pcm', startedAtMs: 0 }),
        JSON.stringify({ seq: 2, userId: 'u2', speaker: 'Beto', file: 'audio/002-Beto.pcm', startedAtMs: 5_000 }),
      ].join('\n') + '\n',
    );
  }
  if (opts.withChat) {
    await writeFile(
      join(dir, 'chat.jsonl'),
      JSON.stringify({ t: T0 + 12_000, userId: 'u3', author: 'Carla', content: 'yo apoyo lo del foro' }) + '\n',
    );
  }
  return dir;
}
...
describe.skipIf(!hasFfmpeg)('minutas finalize pipeline', () => {
  ...
  test('full path: PCM → WAV → draft → LLM → Discord post → MinIO layout → DB done → local cleanup', async () => {
    const { store, sessionsDir, storage, sent, finalizeSession, deps } = harness;
    store.createSession(sessionRow());
    const dir = await fabricateSessionDir(sessionsDir, { withAudio: true, withChat: true });

    const result = await finalizeSession(deps, SESSION_ID);

    // Result + DB lifecycle.
    expect(result.empty).toBe(false);
    expect(result.publishedUrl).toBe('https://discord.com/channels/g/c/msg-1');
    ...
    expect(stats).toMatchObject({ bursts: 2, chatLines: 1, empty: false, minioUploaded: true });

    // The Discord post: header + body, files attached, no mentions allowed.
    expect(sent).toHaveLength(1);
    const post = sent[0]!;
    expect(String(post.content)).toContain('# 📜 Minuta — Asamblea de prueba');
    ...
    const draft = await readFile(join(dir, 'draft.md'), 'utf8').catch(() => null);
    ...
    expect(draftText).toContain('[00:05] Beto: propongo hacer el foro el sábado');
    expect(draftText).toContain('[00:12] 💬 Carla (chat): yo apoyo lo del foro');
  ...
  test('live-transcribed bursts are consumed from the ledger — whisper only runs for the remainder', async () => {
    ...
    store.createSession(sessionRow());
    const dir = await fabricateSessionDir(sessionsDir, { withAudio: true, withChat: false });
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        speaker: 'Ana',
        startedAtMs: 0,
        segments: [{ startMs: 0, endMs: 2000, text: 'abro la asamblea de hoy' }],
      }) + '\n',
    );
  ...
  test('empty session: no LLM call, no post, DB done with empty stats', async () => {
    const { store, sessionsDir, storage, sent, finalizeSession, deps } = harness;
    store.createSession(sessionRow());
    await fabricateSessionDir(sessionsDir, { withAudio: false, withChat: false });
    ...
    const row = store.getSession(SESSION_ID)!;
    expect(row.status).toBe('done');
    expect(JSON.parse(row.stats_json!)).toMatchObject({ empty: true, bursts: 0, chatLines: 0 });
  ...
  test('storage disabled: minutes still publish; local dir kept as the only copy', async () => {
    ...
    const result = await finalizeSession({ ...deps, storage: null }, SESSION_ID);

    expect(result.empty).toBe(false);
    expect(sent).toHaveLength(1);
    expect(existsSync(dir)).toBe(true);
    expect(JSON.parse(store.getSession(SESSION_ID)!.stats_json!).minioUploaded).toBe(false);
  });
});
