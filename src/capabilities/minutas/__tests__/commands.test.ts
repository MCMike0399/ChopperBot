...
import { buildMinutasInteractionHandler, resolveInteractionAuthority } from '../commands.js';
...
function fakeInteraction(opts: FakeInteractionOpts) {
  ...
  const interaction = {
    ...
    guild: {
      members: { fetch: async () => ({ voice: { channel: opts.voiceChannel ?? null } }) },
      roles: { cache: new Map() },
    },
    member:
      opts.memberRoles === null
        ? null
        : {
            roles: { cache: new Collection((opts.memberRoles ?? []).map((r) => [r.id, r])) },
            permissions: new PermissionsBitField(opts.isAdmin ? PermissionFlagsBits.Administrator : 0n),
          },
    memberPermissions: new PermissionsBitField(opts.isAdmin ? PermissionFlagsBits.Administrator : 0n),
    options: { getString: (n: string) => (n === 'titulo' ? (opts.title ?? null) : null) },
    get replied() {
...
describe('minutas slash-command gate (calendar approver roles only)', () => {
  ...
  test.each([['join', JOIN_COMMAND], ['leave', LEAVE_COMMAND]] as const)(
    'calendar approver role passes the gate on /%s',
    async (_label, command) => {
      const { mem, store } = await newDb();
      store.setOutputChannelId(OUTPUT);
      const { handler, sessions, requestLeaveProcessing } = fakeDeps(store, mem);
      const { interaction, replies } = fakeInteraction({
        command,
      ...
      expect(replies.filter((r) => /moderación/.test(r.content))).toHaveLength(0);
      if (command === JOIN_COMMAND) expect(sessions.start).toHaveBeenCalledTimes(1);
...
describe('minutas slash-command flows', () => {
  ...
  test('leave with an active session → defer + process + ack', async () => {
    ...
    await handler(interaction);
    expect(requestLeaveProcessing).toHaveBeenCalledWith('g1', '/chopperbot-leave');
...
describe('resolveInteractionAuthority', () => {
  test('resolves GuildMember-shaped members (roles with names + permissions)', async () => {
    ...
    expect(auth.memberRoles).toEqual([{ id: MOD_ROLE, name: '⭐Administradora⭐' }]);
