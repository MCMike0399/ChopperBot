...
import { planBatches, readLedger, transcribeBatch, type PendingBurst } from './live.js';
import { buildTimeline, renderTranscript, type ChatNote, type SpeechBurst } from './draft.js';
import { generateMinutes, renderMinutesPost, type MinutesMeta } from './minutes.js';
...
export async function finalizeSession(
  ...
  if (!existsSync(dir)) throw new Error(`finalizeSession: no session dir ${dir}`);
  ...
  if (timeline.length === 0) {
    ...
    deps.store.updateSession(sessionId, {
      status: 'done',
      stats_json: JSON.stringify({ ...stats, empty: true, minioUploaded: uploaded }),
      minio_prefix: prefix,
    });
    log.info({ sessionId }, 'minutas.finalized_empty');
    return { sessionId, empty: true, publishedUrl: null, minioPrefix: prefix };
  }
  ...
  if (outputChannelId) {
    ...
    log.warn({ sessionId }, 'minutas.no_output_channel — minutes generated but not published');
  }
  ...
  if (uploaded) {
    ...
    log.warn({ sessionId, dir }, 'minutas.local_copy_kept (MinIO unavailable or partial upload)');
  }
  ...
  deps.store.updateSession(sessionId, {
    ...
    minio_prefix: prefix,
    stats_json: JSON.stringify({ ...stats, empty: false, minioUploaded: uploaded }),
  });
  log.info({ sessionId, publishedUrl, minioPrefix: prefix, stats }, 'minutas.finalized');
  return { sessionId, empty: false, publishedUrl, minioPrefix: prefix };
}

// ── Internals ───────────────────────────────────────────────────────────────

async function readManifest(dir: string, row: MinutasSessionRow): Promise<SessionManifest> {
  try {
    const raw = JSON.parse(await readFile(join(dir, ARTIFACTS.sessionMeta), 'utf8'));
    ...
    return {
      ...
      participants: JSON.parse(row.participants_json || '{}') as Record<string, string>,
    };
  }
}
...
async function transcribeBursts(
  ...
  if (pending.length > 0 && !deps.transcriber.isAvailable()) {
    log.warn({ dir, pending: pending.length }, 'minutas.transcriber_unavailable — bursts left untranscribed');
    for (const b of pending) {
      bursts.push({ userId: b.userId, speaker: b.speaker, startedAtMs: b.startedAtMs, segments: [] });
    }
  } else {
    for (const batch of planBatches(pending)) {
      try {
        ...
        log.warn({ err, batch: batch.name }, 'minutas.burst_transcribe_failed');
...
async function readChat(dir: string, sessionStartMs: number): Promise<ChatNote[]> {
  try {
    ...
    return raw
      ...
      .map((l) => ({ atMs: l.t - sessionStartMs, author: l.author, content: l.content }));
  } catch {
    return [];
  }
}
...
async function uploadAll(
  ...
  for (const rel of files) {
    try {
      ...
      ok = false;
      log.warn({ err, key: `${prefix}${rel}`, sessionId: row.id }, 'minutas.minio_upload_failed');
    }
  }
  log.info({ sessionId: row.id, prefix, files: files.length, ok }, 'minutas.minio_uploaded');
  return ok;
}
