...
export function recordOpusStreamToPcm(
   ...
   return new Promise((resolve, reject) => {
      ...
      ff.on("close", (code) => {
         ...
         try {
            resolve({
               bytes: statSync(outPcmPath).size,
               packets,
               decodeErrors,
            });
         } catch (err) {
         ...
         try {
            ...
            if (consecutiveErrors === DECODER_REBUILD_AFTER_ERRORS) {
               try {
                  ...
                  log.warn(
                     { outPcmPath, decodeErrors },
                     "minutas.opus_decoder_rebuilt",
                  );
               } catch (rebuildErr) {
                  log.error(
      ...
      opusStream.on("error", (err) => {
         log.warn(
            { err: String(err), outPcmPath },
            "minutas.opus_stream_error",
         );
         endInput();
      });
   });
   ...
   return new Promise((resolve, reject) => {
      ...
      ff.on("close", (code) =>
         code === 0
            ? resolve()
            : reject(
                 new Error(`ffmpeg exited ${code}: ${stderr.slice(0, 400)}`),
              ),
      );
   });
}
