 ...
 */
export function buildTimeline(bursts: SpeechBurst[], chat: ChatNote[]): DraftEntry[] {
  ...
  for (const note of chat) {
    ...
    if (!text) continue;
    entries.push({ atMs: note.atMs, kind: 'chat', speaker: note.author, text });
  }
  // Chronological; on a tie the spoken word lands before the typed comment.
  entries.sort((a, b) => a.atMs - b.atMs || (a.kind === b.kind ? 0 : a.kind === 'speech' ? -1 : 1));
