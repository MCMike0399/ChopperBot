...
export function buildMinutesSystemPrompt(): string {
  return `Eres ChopperBot redactando la MINUTA de una reunión de voz de la comunidad Revolución Z. Recibes la transcripción automática (por hablante, con marcas de tiempo) y los comentarios del chat del canal, y produces el acta en español.
...
- No inventes contenido: si algo no está en la transcripción, no existe. La transcripción es automática y puede tener errores; si un tramo es ambiguo, resume lo seguro.
- Las líneas con 💬 son comentarios del CHAT de texto: úsalos como contexto y en su propia sección, no como intervenciones habladas.
- Estructura EXACTA del acta (markdown de Discord):
  ## Resumen
  ...
  (solo lo que quedó acordado o decidido; si no hubo, escribe "Sin acuerdos formales.")
  ## Compromisos
  (quién se comprometió a qué; si no hubo, "Sin compromisos.")
  ## Comentarios del chat
  (SOLO comentarios con contenido: preguntas, datos, posturas, acuerdos. DESCARTA saludos, despedidas, monosílabos, muletillas y reacciones — «hola», «oki», «olo», «tqm», «jaja», emojis sueltos y variantes: en el acta son ruido, no registro. Si tras filtrar no queda nada sustancial, omite la sección entera)
...
export function buildMinutesUserPrompt(draft: string, meta: MinutesMeta): string {
  return `${renderMetaBlock(meta)}

Esta es la transcripción completa (borrador), con marcas de tiempo relativas al inicio:
...
export function buildBlockExtractionPrompt(block: string, index: number, total: number): string {
  return `Esta es la parte ${index} de ${total} de la transcripción de una reunión:

${block}

Extrae, en viñetas breves y en español: los temas discutidos, las posturas de cada persona (con su nombre), cualquier acuerdo/decisión y cualquier compromiso que aparezca EN ESTA PARTE. No redactes todavía el acta; solo notas fieles a la transcripción.`;
}

export function buildFinalFromNotesPrompt(notes: string, meta: MinutesMeta): string {
  return `${renderMetaBlock(meta)}
 ...
 */
export function renderMinutesPost(minutesBody: string, meta: MinutesMeta): string {
  return [
    `# 📜 Minuta — ${meta.title}`,
...
export function splitTranscriptIntoBlocks(text: string, maxChars = BLOCK_MAX_CHARS): string[] {
 ...
 */
export async function generateMinutes(draft: string, meta: MinutesMeta): Promise<string> {
  const system = buildMinutesSystemPrompt();
  if (draft.length <= SINGLE_PASS_MAX_CHARS) {
    return ask({
      system,
      messages: [{ role: 'user', content: buildMinutesUserPrompt(draft, meta) }],
  ...
  for (let i = 0; i < blocks.length; i++) {
    const note = await ask({
      system,
      messages: [{ role: 'user', content: buildBlockExtractionPrompt(blocks[i]!, i + 1, blocks.length) }],
      tools: composeToolSources([]),
      effort: 'medium',
    });
  ...
  return ask({
    system,
    messages: [{ role: 'user', content: buildFinalFromNotesPrompt(notes.join('\n\n'), meta) }],
    tools: composeToolSources([]),
    effort: 'medium',
  });
}
