...
export function minioPrefixFor(row: {
   id: string;
   guild_id: string;
   started_at: number;
}): string {
...
export class MinutasSessions {
   ...
   async start(params: {
      ...
      if (
         this.active.has(guild.id) ||
         this.deps.store.getActiveSessionForGuild(guild.id)
      ) {
      ...
      if (channel.isSendable()) {
         await channel
            ...
            .catch((err) =>
               log.warn({ err, sessionId: id }, "minutas.notice_failed"),
            );
      }
      log.info(
         {
   ...
   async end(guildId: string, reason: string): Promise<ClosedSession | null> {
      ...
      try {
         ...
         log.warn(
            ...
            "minutas.connection_destroy_failed",
         );
      }
      await Promise.race([
   ...
   private startBurst(
      ...
      const done = this.resolveSpeaker(session, userId).then((speaker) => {
         ...
         try {
            appendFileSync(
               join(session.dir, ARTIFACTS.bursts),
               JSON.stringify(line) + "\n",
            );
         } catch (err) {
            log.warn(
               { err, sessionId: session.id },
               "minutas.burst_manifest_failed",
            );
         }
         return recordOpusStreamToPcm(opusStream, join(session.dir, file))
            .then((result) => {
               ...
               this.noteBurstOutcome(
                  session,
                  !burstCapturedNothing(result),
                  file,
               );
            ...
            .catch((err) => {
               log.warn(
                  { err, sessionId: session.id, file },
                  "minutas.burst_failed",
               );
   ...
   handleVoiceStateUpdate(
      ...
      const session =
         [...this.active.values()].find(
            (s) =>
               s.channelId === oldState.channelId ||
      ...
      if (humans === 0 && !session.emptyTimer) {
         session.emptyTimer = setTimeout(() => {
            ...
            if (stillEmpty)
               this.closeAndReport(session.guild.id, "el canal quedó vacío");
         }, EMPTY_CHANNEL_GRACE_MS);
   ...
   handleScheduledEventEnd(
      event: Pick<GuildScheduledEvent, "channelId">,
   ): void {
      const session = [...this.active.values()].find(
         (s) => s.channelId === event.channelId,
      );
      if (!session) return;
   ...
   private writeManifest(session: ActiveSession): void {
      ...
      try {
         writeFileSync(
            join(session.dir, ARTIFACTS.sessionMeta),
            JSON.stringify(manifest, null, 2),
         );
      } catch (err) {
         log.warn(
            { err, sessionId: session.id },
            "minutas.manifest_write_failed",
         );
      }
   }
