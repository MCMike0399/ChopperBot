...
export function estimateWhisperSeconds(
   bursts: number,
   audioSeconds: number,
): number {
   return (
      WHISPER_FIXED_SEC_PER_BURST * bursts +
      WHISPER_SEC_PER_AUDIO_SEC * audioSeconds
   );
}
 ...
 */
export function measureSessionDir(dir: string): {
   bursts: number;
   audioSeconds: number;
} {
