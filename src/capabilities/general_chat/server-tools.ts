...
export function findChannel(
  ...
  const partial = channels.filter((c) => normalizeChannelQuery(c.name).includes(q));
  if (partial.length === 1) return { match: partial[0] };
  if (partial.length > 1 && partial.length <= 8) return { candidates: partial };
  return {};
}
...
export function groupByCategory(
  ...
  const sorted = [...channels].sort(
    (a, b) => a.categoryPosition - b.categoryPosition || a.position - b.position,
  );
  ...
  return [...groups.entries()].map(([category, chans]) => ({ category, channels: chans }));
}
  ...
  return {
    async listViewableChannels(): Promise<DirectoryChannel[]> {
      ...
      for (const channel of channels.values()) {
        ...
        const topicRaw =
          'topic' in channel && typeof channel.topic === 'string' ? channel.topic : null;
    ...
    return [
      ...
      },
      {
        ...
        inputSchema: {
          type: 'object',
          properties: {
            channel: { type: 'string', description: 'Mención <#id>, id, o nombre del canal.' },
          },
          required: ['channel'],
        },
      },
    ...
    try {
      ...
      switch (toolName) {
        ...
        case 'server_channel_info': {
          const query = String((input as Record<string, unknown>)?.channel ?? '').trim();
          if (!query) return { status: 'error', payload: { error: 'Falta `channel`.' } };
          const { match, candidates } = findChannel(channels, query);
          if (match) return { status: 'success', payload: { channel: channelPayload(match) } };
          if (candidates && candidates.length > 0) {
            return {
              status: 'success',
              payload: {
                ambiguous: true,
                candidates: candidates.map((c) => ({ mention: `<#${c.id}>`, name: c.name })),
              },
            };
          }
          ...
          return {
            status: 'error',
            payload: { error: `Ese canal no existe o la persona no puede verlo: ${query}` },
          };
        }
        default:
      ...
      return {
        status: 'error',
        payload: { error: 'No pude consultar el directorio del servidor en este momento.' },
      };
    }
  }
}
