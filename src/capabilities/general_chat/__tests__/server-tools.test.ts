...
const ch = (over: Partial<DirectoryChannel> & { id: string; name: string }): DirectoryChannel => ({
  type: 'texto',
  categoryName: null,
  categoryPosition: 0,
  position: 0,
...
const CHANNELS: DirectoryChannel[] = [
  ch({ id: ID1, name: '💬│general', categoryName: 'Comunidad', position: 0, topic: 'el chat principal' }),
  ch({ id: ID2, name: '🎲│fuera-de-tema', categoryName: 'Comunidad', position: 1 }),
  ch({ id: ID3, name: 'bienvenidx', categoryName: 'Escuela/trabajo', categoryPosition: 1, topic: 'reacciona 🎓' }),
  ch({ id: ID4, name: 'chat-poesía', categoryName: 'Clubs', categoryPosition: 2 }),
  ch({ id: ID5, name: 'chat-cineclub', categoryName: 'Clubs', categoryPosition: 2, position: 1 }),
];
...
describe('groupByCategory', () => {
  test('groups in server order', () => {
    ...
    expect(grouped.map((g) => g.category)).toEqual(['Comunidad', 'Escuela/trabajo', 'Clubs']);
...
describe('ServerDirectoryToolSource', () => {
  ...
  test('server_list_channels returns the grouped directory', async () => {
    ...
    const payload = res.payload as { total: number; categories: Array<{ category: string }> };
    expect(payload.total).toBe(5);
    expect(payload.categories.map((c) => c.category)).toContain('Escuela/trabajo');
  });

  test('server_channel_info resolves and returns the topic', async () => {
    const res = await source.handle('server_channel_info', { channel: 'bienvenidx' });
    expect(res.status).toBe('success');
    expect((res.payload as { channel: { topic?: string } }).channel.topic).toBe('reacciona 🎓');
  });

  test('a hidden-or-missing channel is a single indistinguishable error', async () => {
    const res = await source.handle('server_channel_info', { channel: 'canal-de-moderacion' });
    expect(res.status).toBe('error');
    expect(String((res.payload as { error: string }).error)).toContain('no existe o la persona no puede verlo');
  });
