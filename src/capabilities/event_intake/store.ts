...
export class EventIntakeStore {
   ...
   getWatchedCategories(): string[] {
      const row = this.db
         .prepare(
            "SELECT watched_categories_json FROM event_intake_settings WHERE id = 1",
         )
   ...
   setWatchedCategories(ids: string[]): void {
      this.db
         .prepare(
            "UPDATE event_intake_settings SET watched_categories_json = ?, updated_at = ? WHERE id = 1",
         )
   ...
   getModRoles(): string[] {
      const row = this.db
         .prepare(
            "SELECT mod_roles_json FROM event_intake_settings WHERE id = 1",
         )
   ...
   setModRoles(roles: string[]): void {
      this.db
         .prepare(
            "UPDATE event_intake_settings SET mod_roles_json = ?, updated_at = ? WHERE id = 1",
         )
   ...
   getAgitpropChannelId(): string | null {
      const row = this.db
         .prepare(
            "SELECT agitprop_channel_id FROM event_intake_settings WHERE id = 1",
         )
   ...
   setAgitpropChannelId(channelId: string | null): void {
      this.db
         .prepare(
            "UPDATE event_intake_settings SET agitprop_channel_id = ?, updated_at = ? WHERE id = 1",
         )
   ...
   getAgitpropRoles(): string[] {
      const row = this.db
         .prepare(
            "SELECT agitprop_roles_json FROM event_intake_settings WHERE id = 1",
         )
   ...
   setAgitpropRoles(roles: string[]): void {
      this.db
         .prepare(
            "UPDATE event_intake_settings SET agitprop_roles_json = ?, updated_at = ? WHERE id = 1",
         )
         .run(JSON.stringify(dedupeIds(roles)), Date.now());
   }
   ...
   getTicketByFlyerRequestMessage(messageId: string): TicketRow | undefined {
      return this.db
         .prepare(
            "SELECT * FROM event_intake_tickets WHERE flyer_request_message_id = ?",
         )
   ...
   recentTickets(limit: number): TicketRow[] {
      return this.db
         .prepare(
            "SELECT * FROM event_intake_tickets ORDER BY updated_at DESC LIMIT ?",
         )
         .all(limit) as TicketRow[];
   }
