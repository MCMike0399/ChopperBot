...
export function collectPairs(embed: EmbedLike): FormPair[] {
   const fromDesc = embed.description
      ? parsePairsFromDescription(embed.description)
      : [];
   if (fromDesc.length > 0) return fromDesc;
   if (embed.fields && embed.fields.length > 0) {
      return embed.fields
         .map((f) => ({
            question: f.name.trim(),
            answer: stripFences(f.value),
         }))
         .filter((p) => p.question);
   }
   return [];
...
export function mapPairs(pairs: FormPair[]): ParsedForm {
   ...
   for (const { question, answer } of pairs) {
      ...
      if (form.title === null && (q.includes("titulo") || q.includes("tema"))) {
         ...
         form.dayRaw === null &&
         (q.includes("dia") || q.includes("fecha"))
      ) {
         form.dayRaw = a;
      } else if (
         form.speaker === null &&
         (q.includes("ponente") || q.includes("facilitad"))
      ) {
         form.speaker = a;
      } else if (
         form.flyerSelf === null &&
         (q.includes("flyer") || q.includes("imagen") || q.includes("cartel"))
      ) {
...
export function isEventForm(
   ...
   return (
      parsed.title !== null &&
      (parsed.dayRaw !== null || parsed.timeRaw !== null)
   );
}
