...
export class FlyerService {
   ...
   async openFlyerJob(input: {
      ...
      if (!channelId) {
         log.warn(
            ...
            "event_intake.flyer.no_agitprop_channel",
         );
         await this.notifyTicket(
            input.ticketChannelId,
            renderTicketFlyerNotice("open_failed"),
         );
         return false;
      }
      ...
      const posted = await agitprop
         .send({
            content: body,
            allowedMentions: mentionPolicy(mentions.notifyIds),
         })
         .catch((err) => {
            log.warn(
               { err, channelId },
               "event_intake.flyer.request_post_failed",
            );
            return null;
         });
      if (!posted) {
      ...
      this.deps.store.markFlyerRequested(
         input.ticketChannelId,
         posted.id,
         input.notes ?? null,
      );
      ...
      await this.notifyTicket(
         input.ticketChannelId,
         renderTicketFlyerNotice("opened"),
      );
      log.info(
         {
            ticketChannelId: input.ticketChannelId,
            requestMessageId: posted.id,
         },
   ...
   async editFlyerCard(
      ...
      try {
         ...
         let body = card;
         const mentions = opts?.reping
            ? await this.deps.resolveAgitpropMentions(ch)
            : null;
         if (mentions?.text && opts?.reping) {
            const notify = shouldNotifyRoles(
               this.lastAgitpropPingAt.get(ticket.channel_id),
               this.now(),
               MOD_PING_COOLDOWN_MS,
            );
         ...
         log.warn(
            { err, ticketChannelId: ticket.channel_id },
            "event_intake.flyer.card_edit_failed",
         );
      }
   }

   async cancelFlyerJob(
      ticketChannelId: string,
      ...
      await this.editFlyerCard(
         { ...ticket, flyer_status: "cancelled" },
         "cancelled",
         parsed,
      );

      if (source === "ticket") {
         await this.notifyAgitprop(
            renderAgitpropFlyerNotice(ticketChannelId, "cancelled"),
         );
         await this.notifyTicket(
            ticketChannelId,
            renderTicketFlyerNotice("cancelled"),
         );
      } else {
         await this.notifyTicket(
            ticketChannelId,
   ...
   async updateFlyerJob(
      ...
      if (source === "ticket") {
         await this.notifyAgitprop(
            renderAgitpropFlyerNotice(ticketChannelId, "edited"),
         );
         await this.notifyTicket(
            ticketChannelId,
            renderTicketFlyerNotice("edited"),
         );
      } else {
         await this.notifyTicket(
            ticketChannelId,
   ...
   async fulfillFlyer(
      ...
      this.deps.store.markFlyerDelivered(
         ticketChannelId,
      ...
      if (source === "agitprop") {
         ...
         await this.notifyTicket(
            ticketChannelId,
            renderTicketFlyerNotice("delivered"),
         );
      } else {
         ...
         await this.notifyTicket(
            ticketChannelId,
            renderTicketFlyerNotice("delivered_in_ticket"),
         );
         await this.notifyAgitprop(
            renderAgitpropFlyerNotice(ticketChannelId, "delivered_in_ticket"),
         );
      }

      log.info(
         { ticketChannelId, source, messageId: imageMessage.id },
         "event_intake.flyer.delivered",
      );
      return true;
   }

   /** Resolve a fresh CDN URL for the stored flyer pointer (ticket row first). */
   async resolveFlyerImageUrl(
      ticketChannelId: string,
      eventId?: number | null,
   ): Promise<string | null> {
      const ticket = this.deps.store.getTicket(ticketChannelId);
      if (ticket?.flyer_image_channel_id && ticket.flyer_image_message_id) {
         const url = await this.urlFromPointer(
            ticket.flyer_image_channel_id,
            ticket.flyer_image_message_id,
         );
         if (url) return url;
      }
      if (eventId) {
         ...
         if (ev?.flyer_channel_id && ev.flyer_image_message_id) {
            return this.urlFromPointer(
               ev.flyer_channel_id,
               ev.flyer_image_message_id,
            );
         }
      }
      return null;
   ...
   private async applyFlyerToEvent(
      ticketChannelId: string,
      imageMessage: Message,
   ): Promise<void> {
      const ticket = this.deps.store.getTicket(ticketChannelId);
      ...
      this.deps.calendarStore.setFlyerPointer(
         eventId,
         imageMessage.channelId,
         imageMessage.id,
      );
      ...
      if (linked?.discord_event_id) {
         for (const row of this.deps.calendarStore.listAll()) {
            if (
               row.discord_event_id === linked.discord_event_id &&
               row.id !== eventId
            ) {
               this.deps.calendarStore.setFlyerPointer(
                  row.id,
   ...
   private async mirrorFlyerToTicket(
      ticketChannelId: string,
      imageMessage: Message,
   ): Promise<void> {
      ...
      await ch
         ...
         .catch((err) => {
            log.warn(
               { err, ticketChannelId },
               "event_intake.flyer.mirror_failed",
            );
         });
   }
   ...
   private async notifyTicket(
      ...
      await ch
         .send({ content: parts[0], allowedMentions: { parse: [] } })
         .catch(() => {});
   }
