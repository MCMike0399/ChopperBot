...
async function newWatcher(
   ...
   return new EventIntakeWatcher({
      ...
      getAgitpropChannelId: () =>
         opts && "agitpropChannelId" in opts
            ? (opts.agitpropChannelId ?? null)
            : AGITPROP,
...
test("flyerSelf=false opens Agitprop card and ticket notice; flyerSelf=true does not", async () => {
   ...
   expect(
      agitpropSent.some((s) => s.content.includes("Solicitud de flyer")),
   ).toBe(true);
   ...
   (msg2.channel as { send: ReturnType<typeof vi.fn> }).send = vi.fn(
      async (p: Sent) => {
         ticketSent.push(p);
...
test("image reply in Agitprop fulfills job and mirrors to ticket", async () => {
   ...
   expect(cardEdits.some((s) => s.content.includes("Flyer entregado"))).toBe(
      true,
   );
   mem.close();
});
...
test("Agitprop image reply replaces an already-delivered flyer instead of ignoring it", async () => {
   ...
   expect(row.flyer_status).toBe("delivered");
   expect(row.flyer_image_message_id).toBe("img-reply");
   expect(ticketSent.some((s) => s.content.includes("Flyer del evento"))).toBe(
      true,
   );
   mem.close();
});
...
test("approval uses stored flyer pointer instead of findLatestTicketImage", async () => {
   ...
   expect((syncMock.mock.calls[0]![1] as { imageUrl?: string }).imageUrl).toBe(
      FLYER_URL,
   );
   mem.close();
});
...
test("Agitprop author does not get calendar_create_event in Agitprop channel", async () => {
   ...
   askMock.mockImplementation(async (input) => {
      const names =
         input?.tools?.tools.map((t: { name: string }) => t.name) ?? [];
...
test("flyerSelf=false posts open_failed notice and leaves status none when Agitprop channel is missing", async () => {
   ...
   expect(ticketSent.some((s) => s.content.includes("No pude abrir"))).toBe(
      true,
   );
   mem.close();
});
...
test("flyer_request tool reports error (not success) when the Agitprop job fails to open", async () => {
   ...
   let toolResult:
      | { status?: string; payload?: { flyer_status?: string; error?: string } }
      | undefined;
   askMock.mockImplementation(async (input) => {
      toolResult = await input?.tools?.handle("flyer_request", {
         notes: "tema rojo",
      });
...
test("ticket-side image fulfill does not re-upload the flyer or attribute it to Agitprop", async () => {
   ...
   expect(ticketSent.some((s) => s.content.includes("Agitprop entregó"))).toBe(
      false,
   );
   expect(ticketSent.some((s) => s.content.includes("se subió aquí"))).toBe(
      true,
   );
   expect(agitpropSent.some((s) => s.content.includes("desde el ticket"))).toBe(
      true,
   );
   mem.close();
});
...
test("Agitprop @mention with no reply still binds the sole open flyer job", async () => {
   ...
   await watcher.handleMessage(
      agitpropMention({ sent: agitpropSent, refId: null }) as never,
   );
   expect(askMock).toHaveBeenCalled();
   mem.close();
});
