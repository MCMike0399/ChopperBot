...
const FLYER_URL =
   "https://cdn.discordapp.com/attachments/1534429008786227390/1/Caliban_y_la_Bruja.png";
      ...
      scheduledEvents: {
         create: async (payload: Record<string, unknown>) => {
            ...
            return {
               ...
               scheduledStartTimestamp: new Date(
                  payload.scheduledStartTime as Date,
               ).getTime(),
...
function makeApprovalMessage(opts: {
   sent: Sent[];
   history: unknown[];
}): Message {
   ...
   return {
      ...
      channel: {
         ...
         messages: {
            fetch: vi.fn(
               async () => new Map(opts.history.map((m, i) => [`h${i}`, m])),
            ),
         },
...
test("approval creates the Discord event WITH the ticket flyer as its cover", async () => {
   ...
   await watcher.handleMessage(
      makeApprovalMessage({ sent, history: [flyerMessage()] }),
   );

   expect(created).toHaveLength(1);
   expect(
      (created[0]!.image as string).startsWith("data:image/png;base64,"),
   ).toBe(true);
...
test("the bot's own and the ticket bot's images are never picked as the flyer", async () => {
   ...
   await watcher.handleMessage(
      makeApprovalMessage({ sent, history: [botImage] }),
   );
