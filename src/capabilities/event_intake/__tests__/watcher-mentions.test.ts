...
function makeMessage(opts: {
  ...
  return {
    ...
    guild: {
      ...
      roles: {
        cache: { size: roles.length, map: <T>(fn: (r: (typeof roles)[number]) => T) => roles.map(fn) },
        fetch: vi.fn(),
      },
    },
    channel: {
      ...
      permissionsFor: () => ({
        has: (flag: bigint) =>
          flag === PermissionFlagsBits.MentionEveryone ? (opts.canMentionAny ?? false) : true,
      }),
    },
    reply,
...
describe('proposal ping', () => {
  test('pings the mentionable approver roles and allows exactly those mentions', async () => {
    askMock.mockResolvedValue('Hola, tu solicitud llegó. Resumen para lxs mods: …');
  ...
  test('MentionEveryone lets it reach the unmentionable approver role too', async () => {
    ...
    expect(sent[0].allowedMentions?.roles).toEqual([ADMIN_ROLE, MOD_ROLE, TECH_ROLE]);
    mem.close();
  });
  ...
  test('a role mention the model invented is stripped before posting', async () => {
    ...
    const sent: Sent[] = [];
    await watcher.handleMessage(
      makeMessage({ authorId: TICKET_BOT, bot: true, embeds: [FORM_EMBED], sent }),
    );
