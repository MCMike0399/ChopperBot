...
describe('flyer notices', () => {
  ...
  test('agitprop notices reference the ticket channel', () => {
    expect(renderAgitpropFlyerNotice('ticket-1', 'cancelled')).toContain('<#ticket-1>');
    expect(renderAgitpropFlyerNotice('ticket-1', 'edited')).toContain('<#ticket-1>');
    expect(renderAgitpropFlyerNotice('ticket-1', 'delivered_in_ticket')).toContain('<#ticket-1>');
  });
});
