...
describe('ticket edit → Discord event repair', () => {
  test("an edit to the ticket's OWN event with no live card recreates it and appends the link", async () => {
    ...
    await watcher.handleMessage(
      makeMessage({ content: `<@${BOT}> reagéndalo al domingo 16`, memberRoles: [MOD_ROLE], sent }),
    );
    expect(syncMock).toHaveBeenCalledWith(event.id, expect.anything());
    expect(sent[0].content).toContain('📅 Ya creé también el **evento de Discord**');
  ...
  test('an edit to a LINKED event appends nothing (the update propagation refreshed it)', async () => {
    const { watcher, store, calendarStore, mem } = await newWatcher();
    const event = await seedApprovedEvent(store, calendarStore, { linked: true });
    askMock.mockImplementation(async (input) => {
      await input?.tools?.handle('calendar_update_event', { id: event.id, title: 'Conversatorio: Data Centers' });
      return 'Listo, corregí el título.';
    });
    const sent: Sent[] = [];
    await watcher.handleMessage(
      makeMessage({ content: `<@${BOT}> corrige el título`, memberRoles: [MOD_ROLE], sent }),
    );
  ...
  test('an edit to some OTHER event never spawns a card from the ticket', async () => {
    ...
    refreshMock.mockResolvedValue({ ok: true, action: 'not_linked' });
    askMock.mockImplementation(async (input) => {
      await input?.tools?.handle('calendar_update_event', { id: other.id, title: 'Otro título' });
      return 'Listo.';
    });
    const sent: Sent[] = [];
    await watcher.handleMessage(
      makeMessage({ content: `<@${BOT}> cambia el otro evento`, memberRoles: [MOD_ROLE], sent }),
    );
    expect(syncMock).not.toHaveBeenCalled();
    expect(sent[0].content).not.toContain('📅');
    mem.close();
  ...
  test('the mod bundle includes calendar_set_session_theme; a non-mod bundle never does', async () => {
    ...
    const sent: Sent[] = [];
    await watcher.handleMessage(
      makeMessage({ content: `<@${BOT}> hola`, memberRoles: [MOD_ROLE], sent }),
    );
    ...
    const msg = makeMessage({ content: `<@${BOT}> hola`, memberRoles: [], sent });
