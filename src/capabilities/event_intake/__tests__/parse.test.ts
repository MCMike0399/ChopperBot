...
describe("parseTicketForm", () => {
   ...
   test('flyer "Sí" → true', () => {
      ...
      const parsed = parseTicketForm(
         formMessage({ embeds: [{ description: desc }] }),
      )!;
   ...
   test("returns null for a message with no form embed", () => {
      expect(
         parseTicketForm(
            formMessage({ embeds: [{ description: "hola equipo" }] }),
         ),
      ).toBeNull();
   });
});
...
describe("isEventForm", () => {
   ...
   test("false when authored by someone else (even with the same embed)", () => {
      expect(isEventForm(formMessage({ authorId: "999" }), TICKET_BOT)).toBe(
         false,
      );
   });

   test("false for a normal chat message", () => {
      const chat: MessageLike = {
         authorId: "42",
         authorBot: false,
         content: "hola",
         embeds: [],
   ...
   test("false for a NON-event ticket form (report/support) — the guardrail", () => {
      ...
      expect(
         isEventForm(
            formMessage({ embeds: [{ description: report }] }),
            TICKET_BOT,
         ),
      ).toBe(false);
      ...
      expect(
         isEventForm(
            formMessage({ embeds: [{ description: titledReport }] }),
            TICKET_BOT,
         ),
      ).toBe(false);
   });
...
describe("extractRequesterId", () => {
   test("reads the first non-excluded mention from the welcome content", () => {
      expect(
         extractRequesterId(`Bienvenidx <@${REQUESTER}> :D`, [TICKET_BOT]),
      ).toBe(REQUESTER);
   });
   ...
   test("skips excluded ids (the bots)", () => {
      expect(
         extractRequesterId(
            `<@${TICKET_BOT}> abrió el ticket para <@${REQUESTER}>`,
            [TICKET_BOT],
         ),
      ).toBe(REQUESTER);
   });
