...
import {
   OutputChannelPublisher,
   type CalendarPublisher,
} from "../calendar/publisher.js";
...
import {
   EVENT_INTAKE_CAPABILITY_ID,
   DEFAULT_AGITPROP_CHANNEL_ID,
} from "./constants.js";
...
export class EventIntakeCapability implements Capability {
   ...
   private listener:
      ((message: OmitPartialGroupDMChannel<Message>) => void) | null = null;
   ...
   async init({ memory, projectRoot }: CapabilityInitDeps): Promise<void> {
      ...
      this.store.seedWatchedCategories(
         parseChannelIdEnv(config.EVENT_INTAKE_TICKET_CATEGORY_IDS),
      );
      ...
      this.store.seedAgitpropRoles(
         parseChannelIdEnv(config.EVENT_INTAKE_AGITPROP_ROLES),
      );
      log.info(
         {
            capability: this.id,
            watched: this.store.getWatchedCategories().length,
         },
   ...
   async start({ client }: CapabilityStartDeps): Promise<void> {
      if (!this.store || !this.db)
         throw new Error("EventIntakeCapability.start() before init()");
   ...
   isClaimedChannel(message: Message): boolean {
      ...
      if (
         ...
         this.watchedCache = {
            ids: new Set(this.store.getWatchedCategories()),
            at: now,
         };
      }
