...
export class FlyerToolSource implements ToolSource {
   ...
   tools(): ToolSpec[] {
      return [
            ...
            inputSchema: {
               ...
               properties: {
                  notes: {
                     type: "string",
                     description:
                        "Notas opcionales para Agitprop (tema visual, texto obligatorio, etc.).",
                  },
               },
            },
         },
            ...
            inputSchema: {
               ...
               properties: {
                  notes: {
                     type: "string",
                     description:
                        "Nuevas notas para Agitprop (reemplaza las anteriores si se pasan).",
                  },
               },
            },
         },
         {
            name: "flyer_cancel",
            description:
               "Cancela la solicitud de flyer abierta en Agitprop para este ticket.",
   ...
   async handle(toolName: string, input: unknown): Promise<ToolHandlerResult> {
      ...
      if (!ticket) {
         return {
            status: "error",
            payload: { error: "No hay ticket registrado para este canal." },
         };
      }
      ...
      if (!parsed) {
         return {
            status: "error",
            payload: { error: "No pude leer el formulario de este ticket." },
         };
      }
   ...
   private async handleRequest(
      ...
      const opened =
         (await this.deps.onFlyerAction?.("request", notes)) === true;
      if (!opened) {
         const current = this.deps.store.getTicket(ticket.channel_id);
         return {
            status: "error",
            payload: {
               error: "No pude abrir la solicitud de flyer en Agitprop. Revisa que el canal esté configurado y que pueda publicar ahí.",
   ...
   private async handleUpdate(
      ...
      const notes =
         typeof notesRaw === "string" ? notesRaw.trim() || null : null;
      if (notes !== null)
         this.deps.store.setFlyerNotes(ticket.channel_id, notes);
      await this.deps.onFlyerAction?.("update", notes);
      return {
         status: "success",
         payload: {
            message: "Solicitud de flyer actualizada en Agitprop.",
            flyer_status: "requested",
         },
      };
   }
   ...
   private async handleCancel(ticket: TicketRow): Promise<ToolHandlerResult> {
      ...
      return {
         status: "success",
         payload: {
            message: "Solicitud de flyer cancelada.",
            flyer_status: "cancelled",
         },
      };
   }
}
