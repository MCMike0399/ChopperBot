...
export function effectiveAgitpropTokens(
   tokens: readonly string[],
): readonly string[] {
   return effectiveModTokens(
      tokens.length > 0 ? tokens : DEFAULT_AGITPROP_ROLES,
   );
}
...
export function isAgitpropByRole(
   ...
   return (
      matchModRoles(memberRoles, effectiveAgitpropTokens(tokens)).length > 0
   );
}
