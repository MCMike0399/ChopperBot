...
export class VirusTotalClient {
   ...
   async lookupByHash(sha256: string): Promise<VerdictStats | null> {
      ...
      const raw = body?.data?.attributes?.last_analysis_stats as
         RawStats | undefined;
   ...
   async uploadFile(bytes: Uint8Array, fileName: string): Promise<string> {
      ...
      if (typeof id !== "string") {
         throw new Error(
            `VirusTotal upload returned no analysis id: ${JSON.stringify(body).slice(0, 200)}`,
         );
      }
      return id;
   }
   ...
   private async request(
      ...
      try {
         ...
         if (res.status === 401 || res.status === 403) {
            throw new VirusTotalAuthError(
               `VirusTotal rejected the API key (HTTP ${res.status})`,
            );
         }
         if (res.status === 429) {
            throw new VirusTotalRateLimitError(
               "VirusTotal rate limit hit (HTTP 429)",
            );
         }
         return res;
      } finally {
   ...
   private async parseOk(res: Response, op: string): Promise<any> {
      if (!res.ok) {
         ...
         throw new Error(
            `VirusTotal ${op} failed: HTTP ${res.status} ${text.slice(0, 200)}`,
         );
      }
      return res.json();
   }
