...
export class FileScannerStore {
   ...
   getWatchedChannels(): string[] {
      const row = this.db
         .prepare(
            "SELECT watched_channels_json FROM file_scanner_settings WHERE id = 1",
         )
   ...
   markBudgetAlert(nowMs: number): void {
      this.db
         .prepare(
            "UPDATE file_scanner_runtime SET last_budget_alert_at = ?, updated_at = ? WHERE id = 1",
         )
         .run(nowMs, nowMs);
   }
   ...
   private writeTimes(times: number[], nowMs: number): void {
      this.db
         .prepare(
            "UPDATE file_scanner_runtime SET request_times_json = ?, updated_at = ? WHERE id = 1",
         )
   ...
   recentScans(limit: number): ScanRecord[] {
      return this.db
         .prepare(
            "SELECT * FROM file_scanner_scans ORDER BY last_seen_at DESC LIMIT ?",
         )
