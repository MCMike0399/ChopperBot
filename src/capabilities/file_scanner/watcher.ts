...
export class FileScanWatcher {
   ...
   async handleMessage(message: Message): Promise<void> {
      ...
      const lines: FileLine[] = toScan.map((a) => ({
         fileName: a.name,
         status: { phase: "queued" },
      }));
   ...
   private async postInitial(
      ...
      if (this.canReplyWithReference(message)) {
         const reply = await message.reply(content).catch((err) => {
            log.warn(
               { err, channelId: message.channelId },
               "file_scanner.watcher.reply_failed",
            );
            return null;
         });
      ...
      return await message.channel.send(content).catch((err) => {
         log.warn(
            { err, channelId: message.channelId },
            "file_scanner.watcher.send_failed",
         );
         return null;
      });
   }
   ...
   private async scanOne(
      ...
      try {
         ...
         log.error(
            ...
            "file_scanner.watcher.download_failed",
         );
         return {
            kind: "error",
