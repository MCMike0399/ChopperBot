...
function fakeClient(
   handler: (commandName: string, input: Record<string, unknown>) => unknown,
) {
   const send = vi.fn(
      async (cmd: {
...
describe("MinioObjectStorage", () => {
   test("put sends a PutObject with bucket, key and bytes", async () => {
      ...
      await storageWith(client).put(
         "workshop/c1/a.txt",
         new TextEncoder().encode("hola"),
      );
      expect(send).toHaveBeenCalledTimes(1);
      const cmd = send.mock.calls[0]![0] as unknown as {
         constructor: { name: string };
         input: Record<string, unknown>;
      };
   ...
   test("get rethrows non-404 errors (the caller degrades on those)", async () => {
      ...
      await expect(storageWith(client).get("k")).rejects.toThrow(
         "connection refused",
      );
   });
   ...
   test("deletePrefix paginates and batch-deletes, returning the count", async () => {
      ...
      const { client } = fakeClient((name, input) => {
         if (name === "ListObjectsV2Command")
            return input.ContinuationToken ? pages[1] : pages[0];
         if (name === "DeleteObjectsCommand") {
            deleted.push(
               (
                  input.Delete as { Objects: Array<{ Key: string }> }
               ).Objects.map((o) => o.Key),
            );
            return {};
         }
   ...
   test("ensureReady: missing bucket → created", async () => {
      ...
      const { client } = fakeClient((name) => {
         ...
         if (name === "HeadBucketCommand")
            throw Object.assign(new Error("no"), { name: "NoSuchBucket" });
         return {};
      });
      expect(await storageWith(client).ensureReady()).toBe(true);
