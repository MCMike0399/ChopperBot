...
describe('isModCaller', () => {
  ...
  test('Administrator alone authorizes, whatever the roles say', () => {
    ...
    expect(isModCaller({ isAdministrator: true, memberRoles: rolesOf(['1', 'Miembro']) }, [])).toBe(true);
  });

  test('an approver role authorizes without Administrator', () => {
    expect(isModCaller({ memberRoles: rolesOf([MOD_ID, 'Moderación']), isAdministrator: false }, [])).toBe(true);
  });
  ...
  test('an empty role list is "no roles", not "unrestricted"', () => {
    expect(isModCaller({ memberRoles: [], isAdministrator: false }, [])).toBe(false);
  });
});
