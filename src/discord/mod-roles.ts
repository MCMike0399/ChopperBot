...
export function effectiveModTokens(
...
export function matchModRoles<T extends NamedRole>(
   ...
   return roles.filter(
      (r) => ids.has(r.id) || names.has(normalizeRoleName(r.name)),
   );
}
...
export function resolveModMentions(
   ...
   const notifiable = matched.filter(
      (r) => opts.canMentionAny || r.mentionable,
   );
...
export function mentionedRoleIds(
   ...
   const present = new Set(
      [...text.matchAll(ROLE_MENTION_RE)].map((m) => m[1]!),
   );
 ...
 */
export function sanitizeRoleMentions(
   text: string,
