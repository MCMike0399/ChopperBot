...
export function registerHandlers(client: Client, deps: HandlerDeps): void {
   client.once(Events.ClientReady, (c) => {
      log.info(
         {
            tag: c.user.tag,
            authorizedChannels: deps.router.allChannelIds().size,
         },
   ...
   client.on(Events.ShardDisconnect, (event: CloseEvent, id: number) => {
      log.warn(
         { shardId: id, closeCode: event.code, reason: event.reason },
            ...
            reply = await deps.turnQueue.run(
               ...
               async () => {
                  ...
                  let capability = capabilityId
                     ? deps.registry.get(capabilityId)
                     : undefined;
                  if (!capability) {
...
async function resolveAuthority(message: Message): Promise<TurnAuthority> {
   ...
   if (!member) {
      member = await message.guild.members
         ...
         .catch((err) => {
            log.warn(
               { err, user: message.author.id },
               "authority.member_fetch_failed",
            );
            return null;
         });
   }
...
export function shouldRespond(
   ...
   if (!client.user) {
      log.debug(
         { user: message.author.tag, reason: "client_not_ready" },
         "Ignoring message",
      );
      return false;
   }
   ...
   const plainTextMention =
      plainTextMentionPattern(client, message.guild)?.test(
         message.content ?? "",
      ) ?? false;
...
function plainTextMentionPattern(
   client: Client,
   guild: Guild | null,
): RegExp | null {
...
export function stripBotMention(
   ...
   const patterns = [new RegExp(`<@!?${client.user.id}>`, "g")];
