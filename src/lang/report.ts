 ...
 */
const UNLINTED_CAPABILITIES: ReadonlySet<string> = new Set([CONFIGURATION_CAPABILITY_ID]);
...
export function reportSpanishStyle(text: string, ctx: StyleReportContext): void {
  try {
    if (!text || UNLINTED_CAPABILITIES.has(ctx.capability)) return;
    const findings = lintSpanish(text, ctx.toolNames ? { toolNames: ctx.toolNames } : {});
