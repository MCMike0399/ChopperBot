...
import { renderGeneralChatPrompt, renderAssistantPrompt } from '../../capabilities/general_chat/preamble.js';
import { guildProfileFor, REVZ_GUILD_ID } from '../../capabilities/general_chat/profile.js';
...
async function calendarPrompts(): Promise<{ mod: string; readOnly: string }> {
  ...
  const base = { channelId: 'C1', guildId: null, userId: 'U1', userTag: 'mod', now: NOW };
...
describe('every community-facing prompt carries the voice contract', () => {
  ...
  test('general_chat — generic and community-assistant prompts', () => {
    ...
    expect(renderAssistantPrompt(profile!, NOW, [], 'general')).toContain(SPANISH_VOICE_RULES);
  });
  ...
  test('event_intake — proposal and ticket conversation', () => {
    expect(renderProposalPrompt(NOW, EMPTY_FORM, 'U1')).toContain(SPANISH_VOICE_RULES);
  ...
  test('instagram_monitor — moderator and read-only bundles', () => {
    expect(renderInstagramMonitorPrompt(NOW, true)).toContain(SPANISH_VOICE_RULES);
    expect(renderInstagramMonitorPrompt(NOW, false)).toContain(SPANISH_VOICE_RULES);
  });
...
describe('the contract and the linter say the same thing', () => {
  ...
  test('the examples the block gives as CORRECT are lint-clean', () => {
    for (const good of ['lxs', 'todxs', 'compañerxs', 'proponlo', 'ponlo', 'hazlo', 'dilo']) {
      expect(lintSpanish(`Ya quedó, ${good}.`), good).toEqual([]);
    }
  });

  test('the examples it gives as WRONG are all caught', () => {
    for (const bad of ['se le habla', 'elx', 'unx', 'estx', 'propónlo', '¿algo más?', 'quedo atento']) {
