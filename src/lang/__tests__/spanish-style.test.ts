...
import { lintSpanish, describeFindings, type SpanishStyleRuleId } from '../spanish-style.js';
import { KIMI_REPLIES, DEEPSEEK_REGRESSIONS } from './corpus.js';

function rules(text: string, toolNames?: readonly string[]): SpanishStyleRuleId[] {
...
describe('corpus — the regressions reported on the DeepSeek switch', () => {
  test('"se le habla … le pregunto UNA cosa … para elx" trips register, invented pronoun and scaffolding', () => {
    const reply = DEEPSEEK_REGRESSIONS.find((r) => r.id === '1537630365106315406');
  ...
  test('"Propónlo en la próxima asamblea" trips the enclitic-accent rule', () => {
    const reply = DEEPSEEK_REGRESSIONS.find((r) => r.id === '1537630259191480351');
    expect(reply).toBeDefined();
    const findings = lintSpanish(reply!.text);

    expect(findings.map((f) => f.rule)).toContain('enclitic_accent');
    expect(findings.find((f) => f.rule === 'enclitic_accent')?.match).toBe('Propónlo');
  });

  test('the community plural "ellxs" in the same reply is NOT flagged', () => {
    const reply = DEEPSEEK_REGRESSIONS.find((r) => r.id === '1537630259191480351')!;
    const invented = lintSpanish(reply.text).filter((f) => f.rule === 'inclusive_malformed');
    expect(invented).toEqual([]);
  });

  test('every flagged reply produces at least one finding', () => {
    for (const reply of DEEPSEEK_REGRESSIONS) {
      expect(lintSpanish(reply.text).length, `reply ${reply.id}`).toBeGreaterThan(0);
    }
  });
});
...
describe('inclusive language', () => {
  ...
  test.each(['un mensaje para elx', 'unx de lxs mods', 'estx es su ticket', 'se lo paso a ellx'])(
    'the invented pronoun in %j is flagged',
    (text) => {
  ...
  test('real words and names ending in x are never inclusive forms', () => {
    expect(lintSpanish('Marx, el códex y el fax siguen en el índex de Félix.')).toEqual([]);
  });
});

describe('scaffolding leaks', () => {
  test('a tool id in prose is a leak', () => {
    expect(rules('Voy a llamar a calendar_search_events para ubicarlo.')).toContain('scaffolding');
  });

  test('tool names passed by the turn are matched exactly', () => {
    expect(rules('Uso ig_pausar_cuenta y listo.', ['ig_pausar_cuenta'])).toContain('scaffolding');
  });

  test('a prompt parameter name is a leak', () => {
    expect(rules('Lo guardé con recurrence_freq semanal.')).toContain('scaffolding');
  });

  test('ALL-CAPS emphasis copied from the prompt is a leak', () => {
    expect(rules('Te pregunto UNA cosa a la vez.')).toContain('scaffolding');
    expect(rules('NUNCA creo un evento nuevo para eso.')).toContain('scaffolding');
  });

  test('talking about being configured is a leak', () => {
    expect(rules('Mis instrucciones dicen que pregunte una cosa a la vez.')).toContain('scaffolding');
  });

  test('code blocks, inline code and links are not prose', () => {
    expect(lintSpanish('Te dejo el script:\n```python\nstart_at_iso = 1\ndef pon_lo(): pass\n```')).toEqual([]);
    expect(lintSpanish('El campo `recurrence_freq` va en el archivo.')).toEqual([]);
    expect(lintSpanish('Quedó aquí: https://discord.com/events/1/2 y en <#1518328211165941912>')).toEqual([]);
  });

  test('ordinary capitalised words and acronyms are not leaks', () => {
    expect(lintSpanish('Ya se publicaron el PDF y el ICS del mes. ¡LISTO!')).toEqual([]);
  });
});
...
describe('closers', () => {
  ...
  test('a concrete offer is not a service closer', () => {
    expect(lintSpanish('¿Quieres que cree también el evento de Discord para que la gente se apunte?')).toEqual([]);
  });
});

describe('orthography', () => {
  test.each(['Propónlo en la asamblea.', 'Pónlo en la sala de eventos.', 'Házlo cuando puedas.'])(
    '%j is flagged',
    (text) => {
  ...
  test.each(['Proponlo en la asamblea.', 'Ponlo en la sala de eventos.', 'Hazlo cuando puedas.', 'Propónmelo mañana.'])(
    '%j is correct',
    (text) => {
      expect(lintSpanish(text)).toEqual([]);
    },
...
describe('describeFindings', () => {
  test('renders rule and snippet for the journal', () => {
    expect(describeFindings(lintSpanish('¿Desea que lo cree?'))).toBe('usted("¿Desea")');
  });

  test('is empty for clean text', () => {
    expect(describeFindings(lintSpanish('Listo, ya quedó el evento del jueves a las 8.'))).toBe('');
  });
});
