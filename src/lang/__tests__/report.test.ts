...
const warn = vi.fn();
vi.mock('../../log.js', () => ({
  log: { info: () => {}, warn: (...args: unknown[]) => warn(...args), error: () => {} },
}));

const { reportSpanishStyle } = await import('../report.js');

const DRIFTED = 'Si le hace falta algo, se le habla y le pregunto UNA cosa a la vez.';
...
describe('reportSpanishStyle', () => {
  ...
  test('stays silent on a reply in voice', () => {
    reportSpanishStyle('Listo, ya quedó el evento del jueves a las 8. Si me pasas el flyer, se lo pongo.', {
      capability: 'calendar',
      channelId: 'C1',
    });
  ...
  test('the admin console is exempt — naming a tool there is correct', () => {
    reportSpanishStyle('Corre config_system action:health para ver el estado.', {
