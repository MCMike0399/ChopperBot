...
describe("SqliteMemoryStore + migrations", () => {
   test("runs migrations in order and records them", async () => {
      ...
      await store.migrate("demo", [
         m(
            1,
            "CREATE TABLE IF NOT EXISTS demo_things (id INTEGER PRIMARY KEY, name TEXT NOT NULL)",
         ),
         m(
            2,
            "CREATE INDEX IF NOT EXISTS demo_things_name ON demo_things (name)",
         ),
      ]);
      const tables = store
         .db()
      ...
      const applied = store
         .db()
         .prepare(
            "SELECT version FROM _migrations WHERE capability = ? ORDER BY version",
         )
         .all("demo")
   ...
   test("adding a new migration version applies only the new one", async () => {
      ...
      const cols = store.db().prepare("PRAGMA table_info(demo_t)").all() as {
         name: string;
      }[];
   ...
   test("two capabilities migrate independently and do not collide", async () => {
      const store = new SqliteMemoryStore({ path: ":memory:" });
      await store.migrate("alpha", [
         m(1, "CREATE TABLE IF NOT EXISTS alpha_t (id INTEGER PRIMARY KEY)"),
      ]);
      await store.migrate("beta", [
         m(1, "CREATE TABLE IF NOT EXISTS beta_t (id INTEGER PRIMARY KEY)"),
      ]);

      const counts = store
         .db()
         .prepare(
            "SELECT capability, COUNT(*) as c FROM _migrations GROUP BY capability ORDER BY capability",
         )
   ...
   test("migrations run inside a transaction — failing migration rolls back", async () => {
      ...
      await expect(
         store.migrate("demo", [
            m(
               1,
               "CREATE TABLE demo_t (id INTEGER PRIMARY KEY); INSERT INTO nonexistent VALUES (1)",
            ),
         ]),
      ).rejects.toThrow();
...
describe("NamespacedMemory", () => {
   test("forwards db() and fixes capabilityId on migrate()", async () => {
      ...
      await calView.migrate("not-calendar", [
         {
            version: 1,
            up: "CREATE TABLE IF NOT EXISTS calendar_events (id INTEGER PRIMARY KEY)",
         },
      ]);
      const row = inner
         .db()
