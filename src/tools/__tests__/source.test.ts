...
describe("composeToolSources", () => {
   test("aggregates tools from all sources", () => {
      const a = makeSource("a", ["t1", "t2"], async () => ({
         status: "success",
         payload: {},
      }));
      const b = makeSource("b", ["t3"], async () => ({
         status: "success",
         payload: {},
      }));
   ...
   test("throws on tool-name collision", () => {
      const a = makeSource("a", ["shared"], async () => ({
         status: "success",
         payload: {},
      }));
      const b = makeSource("b", ["shared"], async () => ({
         status: "success",
         payload: {},
      }));
   ...
   test("routes a tool call to the source that owns it", async () => {
      const aHandler = vi
         .fn()
         .mockResolvedValue({ status: "success", payload: { from: "a" } });
      const bHandler = vi
         .fn()
         .mockResolvedValue({ status: "success", payload: { from: "b" } });
