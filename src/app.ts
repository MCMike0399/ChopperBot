...
export async function run(): Promise<void> {
   ...
   if (bootCheck.crashed) {
      log.warn(
         { suppressAlert: bootCheck.suppressAlert },
         "lifecycle.unclean_restart_detected",
      );
   }
   ...
   log.info(
      ...
      "Framework user directory ready",
   );
   ...
   const turnQueue = new TurnQueue({
      maxConcurrent: config.MAX_CONCURRENT_TURNS,
   });
   ...
   for (const cap of candidates) {
      try {
         ...
         log.info(
            { capability: cap.id, description: cap.description },
         ...
         log.error(
            { err, capability: cap.id },
            "Capability failed to init — skipping",
         );
      }
   }
         ...
         log.warn(
            {
               channelId,
               capabilityId,
               registered: registry.list().map((c) => c.id),
            },
   ...
   if (registry.has(workshopCap.id)) {
      claimGuards.push((message) =>
         workshopCap.isSessionChannel(message.channelId),
      );
      configCap.attachWorkshopAdmin({
         adminCloseSession: (channelId) =>
            workshopCap.adminCloseSession(channelId),
         adminEnsureWelcome: () => workshopCap.adminEnsureWelcome(),
      });
   }
   if (registry.has(minutasCap.id)) {
      configCap.attachMinutasAdmin({
         endActiveSession: (guildId) =>
            minutasCap.requestLeaveProcessing(
               guildId,
               "consola de configuración",
            ),
         transcriberAvailable: () =>
            minutasCap.adminStatus(null).transcriberAvailable,
