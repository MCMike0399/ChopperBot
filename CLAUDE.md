...
`src/capabilities/sancus_ops/` ships in the repo but is **intentionally NOT registered** here (removed from `app.ts`'s `candidates` 2026-08-06 — registering it leaks it into general_chat's capability snapshot and the LLM advertised it to users). Kept for its separate deploy; do not add it back.
