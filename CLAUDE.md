...
**Known residuals / open follow-ups (not yet fixed, ordered by impact — revisit if `requests_24h` still creeps toward 120 after 2026-06-02):**
1. **`activeFraction` under-counts ceiling-clamped accounts (~6–7 req/day).** The uniform `activeFraction` (≈0.73 = quiet-hours × tick-skip) assumes quiet hours proportionally reduce every account's polls. But an account pinned at the 12 h ceiling polls ~2×/day regardless — quiet hours only *time-shift* its poll, never drop it. With ~7 of 15 accounts clamped, the model under-projects them by ~3.8 polls/day ≈ 6.5 req/day. Fix would be a per-account quiet-aware polls/day term (`min(DAY/iv, (DAY−quietWindow)/iv + 1)`) inside `computeGovernorStretch`; deferred because the 25 % headroom absorbs it and stacking model changes makes live attribution harder.
2. **`callsPerPoll` measurement lag.** It's a trailing-24 h `requests ÷ polls` ratio used to project the *next* 24 h, so it lags real drift (measured **1.55** at one noon sweep vs ~1.7 steady-state). The per-tick recompute (2026-06-02) re-reads it every minute, which mitigates this, but it's still a backward-looking estimate.
3. **Account count drift:** the narrative above says "14-account deployment"; it's **15** as of 2026-06-02 (2 cold-start). Don't trust the count in prose — `SELECT count(*) FROM instagram_monitor_accounts;`.
