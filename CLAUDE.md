...
- `computeCadenceInterval()` (in `store.ts`, pure + unit-tested) takes an account's distinct recent post times (**deduped across channels** — `seen_posts` is per-channel, so the query `GROUP BY ig_post_id` or the median halves) and returns `interval = clamp(median_inter-post_gap × 0.5, 45 min, 6 h)`, with a recency-decay stretch toward MAX once an account goes quiet, and a coverage clamp. It returns `null` (→ cold-start) until there are ≥ `CADENCE_MIN_SAMPLES` (5) posts spanning ≥ `CADENCE_MIN_SPAN_MS` (**24 h**) — `MIN_SAMPLES` rejects tiny bursts; the 24 h span rejects a single-afternoon flurry while still accepting a real multi-day cadence (a 26-post/2.9-day account would be wrongly rejected at a 3-day threshold).
- **Cold start** (`CADENCE_COLD_START_INTERVAL_MS` = 6 h): accounts without a trusted cadence poll slowly *on purpose* — few posts ⇒ empirically a rare poster, so we don't burn budget on unknowns; they speed up automatically as history accrues. (This is why the default fallback is 6 h, not the old 60 min.)
- The interval is cached on `instagram_monitor_accounts` (`poll_interval_ms`, `posts_per_day`, `cadence_updated_at`; **v6**) and read by `dueAccounts()` via `effectiveBaseIntervalMs()` = `(poll_interval_ms ?? cold-start) × poll_stretch`, clamped to MAX. The failure-backoff cap is decoupled to `min(base × 2^failures, max(MAX_BACKOFF_MS, base))` so a base interval ≥ 6 h isn't silently capped.
- **Budget governor** (`computeGovernorStretch()`): a single global `poll_stretch` (runtime row, **v7**) raised above 1.0 so projected daily requests stay ≤ `IG_DAILY_REQUEST_BUDGET × CADENCE_BUDGET_HEADROOM` (**0.75**). `projected = Σ (activeFraction × day / interval) × callsPerPoll` (1.5 direct+auth — feed + ~50% warmup, pk cached). One multiplier preserves the active-vs-rare allocation; the hard budget gate stays the backstop. With several accounts at the 6 h MAX clamp the governor under-corrects slightly (clamped accounts can't stretch further) — the 25% headroom absorbs that plus the restart pk-resolve burst.
...
Net effect on the real 14-account deployment: active accounts (e.g. `yoxlas40horas` ~94 min, `kontragazeta` ~146 min) poll *faster* than the old budget-starved ~3–4 h, rare accounts sit at 6 h, 5 low-data accounts cold-start at 6 h; projected ~108 calls/day vs the 120 cap, `poll_stretch` ≈ 1.6 — the `121/120` pauses stop.
