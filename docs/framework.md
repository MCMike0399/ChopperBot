...
0b. **Perceived-progress UX in public channels (v1.12.0).** Typing is refreshed every **5 s** (`TYPING_REFRESH_MS`) and kept alive until the reply actually posts — at 8 s a busy Pi event loop let Discord's ~10 s indicator lapse mid-turn and a member read the stopped animation as "se trabó" (live 2026-08-06, `#chat-poesía`; the answer did arrive). Beyond that, a turn slower than **20 s** (`SLOW_TURN_STATUS_MS`) escalates to the same `LiveStatusMessage` talleres use — posted as a **reply** (so `buildHistory`'s reply-chain walk still finds the answer) and morphed into the reply when done. Fast turns post no extra message. `chunkBotReply` also caps a reply at `MAX_REPLY_CHUNKS` (5) with a Spanish truncation note: a broken generation can no longer occupy 8 messages of a channel.
