...
1. Message arrives. Bot responds only if addressed in an authorized channel: a real @-mention, a reply to one of its own messages, or — since **v1.17.1** — the **plain-text form `@ChopperBot`** (bot username or its guild nickname, case-insensitive, the `@` required, name must not continue into a longer word). Copy-pasting a mention from another message pastes it as literal text with no mention entity (`message.mentions` stays empty), so members "mentioning" the bot that way got silently ignored — live 2026-08-11 in the general channel ("wtff"). `stripBotMention` strips the plain-text form too (pass `message.guild` so the nickname is known); a bare `chopperbot` without `@` still does NOT trigger — that's someone talking *about* the bot.
...
3b. **Turn authority (added 2026-08-13).** `resolveAuthority()` snapshots WHO is asking — the author's guild roles (`memberRoles`) + whether they hold Discord's Administrator permission — onto `CapabilityTurnContext`. Roles come from the MESSAGE_CREATE partial member (no privileged GuildMembers intent: the ids resolve against the role cache the Guilds intent already fills; `message.member` missing → one `members.fetch`, and a failure leaves `memberRoles` **undefined**). Capabilities read it through **`isModTurn(db, ctx)`** (`src/capabilities/mod-authority.ts`), which resolves the approver tokens from event_intake's `mod_roles` setting — the same list that decides who may approve a ticket and who the calendar pings, so the three can't drift — and **fails closed**: `undefined` roles is "not a mod", never "no roles, therefore fine". `isModCaller` in `mod-roles.ts` is the pure gate underneath.
4. `capability.buildTurn(ctx)` returns `{ system, tools }` for *this* message. Capabilities decide whether to rebuild every turn (calendar embeds an upcoming-events snapshot, configuration embeds current time) or cache. **Privileged capabilities gate here, by omitting tools from the payload** rather than by asking the model to behave: configuration → zero tools for a non-mod, calendar → read-only slice, instagram_monitor → the mutating `monitor_*` tools removed (each also re-checks at the point of effect). A tool the model was never handed cannot be jailbroken into existence; a prompt rule can. Who passes today: `npx tsx scripts/verify-mod-authority.ts`.
...
## Mentions — the bot-wide policy (added 2026-08-13)

`createClient()` sets **`allowedMentions: { parse: ['users'], repliedUser: true }`**. Without it discord.js omits `allowed_mentions` entirely and Discord parses **every** mention class in whatever the model wrote — and since the bot was granted **Administrator** in the guild, every role is pingable, so one successful jailbreak (or one hostile Instagram caption echoed into a card) would ring the whole server. The rule against mass pings previously existed only as a sentence in `general_chat`'s preamble: a promise, not a gate.

Two consequences to keep in mind when writing a new send:
